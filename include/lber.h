 ...
 * in decode.c:
 */
typedef int (*BERDecodeCallback) (	BerElement *ber,	void *data,
	int mode );
...
LBER_F( struct berval * )
ber_mem2bv  (
	const char *, ber_len_t len, int duplicate, struct berval *bv);
...
LBER_F( ber_len_t )
ber_strnlen  (
	const char *s, ber_len_t len );

LBER_F( char * )
ber_strndup  (
	const char *s, ber_len_t l );
