...
int rewrite_parse(struct rewrite_info *info, const char *fname, int lineno, int argc, char **argv) {
  ...
  if (strcasecmp(argv[0], "rewriteEngine") == 0) {
    ...
    if (rewrite_int_curr_context == NULL) {
      rewrite_int_curr_context = rewrite_context_create(info, argv[1]);
    }
    if (rewrite_int_curr_context == NULL) {
      return -1;
    }
    ...
    if (argc > 2) {
      ...
      if (strcasecmp(argv[2], "alias") == 0) {
        ...
        aliased = rewrite_context_find(info, argv[3]);
        if (aliased == NULL) {
          Debug(LDAP_DEBUG_ANY,
                "[%s:%d] aliased"
                " rewriteContext '%s'"
    ...
    rc = rewrite_parse_builtin_map(info, fname, lineno, argc, argv);
    ...
    Debug(LDAP_DEBUG_ANY, "[%s:%d] unknown command '%s'\n", fname, lineno, "");
    return -1;
  }
 ...
 * Compares two maps
 */
static int rewrite_builtin_map_cmp(const void *c1, const void *c2) {
 ...
 * Duplicate map ?
 */
static int rewrite_builtin_map_dup(void *c1, void *c2) {
