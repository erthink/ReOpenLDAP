...
static int parse_line(char **argv, int *argc, int maxargs, char *buf) {
  char *p, *begin;
...
int rewrite_read(FILE *fin, struct rewrite_info *info) {
  char buf[1024];
  char *argv[11];
  int argc, lineno;
  ...
  for (lineno = 0; fgets(buf, sizeof(buf), fin); lineno++) {
    ...
    case 1:
      if (strncasecmp(argv[0], "rewrite", 7) == 0) {
        int rc;
        rc = rewrite_parse(info, "file", lineno, argc, argv);
