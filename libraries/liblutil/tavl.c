 ...
 */
int tavl_insert(TAvlnode **root, void *data, AVL_CMP fcmp, AVL_DUP fdup) {
  TAvlnode *t, *p, *s, *q, *r;
  int a, cmp, ncmp;
...
void *tavl_delete(TAvlnode **root, void *data, AVL_CMP fcmp) {
  TAvlnode *p, *q, *r, *top;
...
int tavl_free(TAvlnode *root, AVL_FREE dfree) {
  int nleft, nright;
 ...
 *				also set *ret = last comparison result, or -1 if
 *root
 *== NULL.
 */
...
TAvlnode *tavl_find2(TAvlnode *root, const void *data, AVL_CMP fcmp) {
  int cmp;
...
void *tavl_find(TAvlnode *root, const void *data, AVL_CMP fcmp) {
  int cmp;
...
TAvlnode *tavl_end(TAvlnode *root, int dir) {
  if (root) {
...
TAvlnode *tavl_next(TAvlnode *root, int dir) {
  if (root) {
