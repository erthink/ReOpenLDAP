...
int lutil_passwd_string64(const struct berval *sc, const struct berval *hash,
  ...
  if (salt) {
    ...
    memcpy(string.bv_val, hash->bv_val, hash->bv_len);
    memcpy(&string.bv_val[hash->bv_len], salt->bv_val, salt->bv_len);
...
static int DES_failed(des_data_block *encrypted) { return 0; }
