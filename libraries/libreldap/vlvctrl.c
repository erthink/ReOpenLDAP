...
/*---
   ...
   vlvinfop  (IN)  The address of an LDAPVLVInfo structure whose contents
                                   are used to construct the value of the
 ...
 ldctl_value member of an LDAPControl structure that contains the
                                   ...
                                   SHOULD be freed when it is no longer in use
 by calling ldap_memfree().
   ...
   Note:  The first time the VLV control is created, the ldvlv_context
                  ...
                  should be used as the context in the next
 ldap_create_vlv_control call.
...
/*---
 ...
 control that is created.
   ...
   ctrlp     (OUT) A result parameter that will be assigned the address
                                   ...
                                   VirtualListViewRequest control created by
 this function. The memory occupied by the LDAPControl structure SHOULD be freed
 when it is no longer in use by calling ldap_control_free().
...
/*---
   ...
   target_posp	(OUT)  This result parameter is filled in with the list
                                           index of the target entry.  If this
parameter is NULL, the target position is not returned.

   list_countp  (OUT)  This result parameter is filled in with the server's
                                           estimate of the size of the list.  If
this parameter is NULL, the size is not returned.

   contextp     (OUT)  This result parameter is filled in with the address
                                           of a struct berval that contains the
server- generated context identifier if one was returned by the server.  If the
server did not return a context identifier, this parameter will be set to NULL,
even if an error occured. The returned context SHOULD be used in the next call
                                           to create a VLV sort control.  The
struct berval returned SHOULD be disposed of by calling ber_bvfree() when it is
no longer needed.  If NULL is passed for contextp, the context identifier is not
returned.

   errcodep     (OUT)  This result parameter is filled in with the VLV
                                           result code.  If this parameter is
