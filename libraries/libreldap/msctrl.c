...
int ldap_create_dirsync_value(LDAP *ld, int flags, int maxAttrCount,
  ...
  if (ld == NULL || cookie == NULL || value == NULL) {
    if (ld) {
...
int ldap_parse_dirsync_control(LDAP *ld, LDAPControl *ctrl, int *continueFlag,
  ...
  if (ld == NULL || ctrl == NULL || continueFlag == NULL || cookie == NULL) {
    if (ld) {
      ld->ld_errno = LDAP_PARAM_ERROR;
    }
...
int ldap_create_show_deleted_control(LDAP *ld, LDAPControl **ctrlp) {
  assert(ld != NULL);
  assert(LDAP_VALID(ld));
  assert(ctrlp != NULL);
...
int ldap_create_extended_dn_value(LDAP *ld, int flag, struct berval *value) {
  BerElement *ber = NULL;
  ber_tag_t tag;
...
int ldap_create_extended_dn_control(LDAP *ld, int flag, LDAPControl **ctrlp) {
  struct berval value;
...
int ldap_create_sever_notification_control(LDAP *ld, LDAPControl **ctrlp) {
  assert(ld != NULL);
  assert(LDAP_VALID(ld));
  assert(ctrlp != NULL);
