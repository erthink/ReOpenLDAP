...
int ldap_int_decode_b64_inplace(struct berval *value) {
  char *p, *end, *byte;
  char nib;
  ...
  for (p = value->bv_val, value->bv_len = 0; p < end; p += 4, value->bv_len += 3) {
    int i;
    for (i = 0; i < 4; i++) {
      if (p[i] != '=' && (p[i] & 0x80 || b642nib[p[i] & 0x7f] > 0x3f)) {
        Debug(LDAP_DEBUG_ANY,
              "ldap_pvt_decode_b64_inplace: invalid base64 encoding"
              " char (%c) 0x%x\n",
              p[i], p[i]);
        return (-1);
      }
