...
static void ldap_pvt_set_errno(int err) { sock_errset(err); }
...
int ldap_connect_to_host(LDAP *ld, Sockbuf *sb, int proto, LDAPURLDesc *srv, int async) {
  ...
  if (!inet_aton(host, &in)) {
    int local_h_errno;
    rc = ldap_pvt_gethostbyname_a(host, &he_buf, &ha_buf, &hp, &local_h_errno);
  ...
  for (i = 0; !use_hp || (hp->h_addr_list[i] != 0); ++i, rc = -1) {
    ...
    if (use_hp) {
      ...
      memcpy(&sin.sin_addr, &in.s_addr, sizeof(sin.sin_addr));
    }
...
void ldap_free_select_info(void *sip) { LDAP_FREE(sip); }
