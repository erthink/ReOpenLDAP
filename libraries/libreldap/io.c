 ...
 */
ber_slen_t ber_write(BerElement *ber, const char *buf, ber_len_t len,
...
char *ber_start(BerElement *ber) { return ber->ber_buf; }

int ber_len(BerElement *ber) { return (ber->ber_end - ber->ber_buf); }

int ber_ptrlen(BerElement *ber) { return (ber->ber_ptr - ber->ber_buf); }
...
int ber_remaining(BerElement *ber) { return ber_pvt_ber_remaining(ber); }
