...
static int bind_prompt(LDAP *ld, const char *url, ber_tag_t request,
  ...
  if (authmethod == LDAP_AUTH_SIMPLE && dn[0] != '\0') {
    get_line(passwd, sizeof(passwd), stdin, "re-bind password? ");
  } else {
    passwd[0] = '\0';
  }
...
int main(int argc, char **argv) {
  ...
  char **exdn;
  static const char usage[] = "usage: %s [-u] [-h host] [-d level] [-s "
                              "dnsuffix] [-p port] [-t file] [-T file]\n";
  ...
  while (get_line(line, sizeof(line), stdin, "\ncommand? ") != NULL) {
    ...
    case 'a': /* add or abandon */
      ...
      case 'd': /* add */
        ...
        strcat(dn, dnsuffix);
        if ((attrs = get_modlist(NULL, "attr? ", "value? ")) == NULL)
          break;
    ...
    case 'b': /* async bind */
      ...
      if (method == LDAP_AUTH_SIMPLE && dn[0] != '\0')
        get_line(passwd, sizeof(passwd), stdin, "password? ");
      else
        passwd[0] = '\0';
    ...
    case 'B': /* synch bind */
      ...
      if (ldap_bind_s(ld, dn, passwd, method) != LDAP_SUCCESS) {
    ...
    case 'r': /* result or remove */
      switch (command3) {
      case 's': /* result */
        get_line(line, sizeof(line), stdin, "msgid (-1=>any)? ");
        if (line[0] == '\0')
          id = -1;
        else
          id = atoi(line);
        get_line(line, sizeof(line), stdin, "all (0=>any, 1=>all)? ");
        if (line[0] == '\0')
          all = 1;
        else
          all = atoi(line);
        if ((msgtype = ldap_result(ld, id, all, &timeout, &res)) < 1) {
...
static void print_search_entry(LDAP *ld, LDAPMessage *res) {
  ...
  if (res->lm_msgtype == LDAP_RES_SEARCH_RESULT || res->lm_chain != NULL)
