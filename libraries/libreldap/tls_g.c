...
static int tlsg_mutex_init(void **priv) {
  int err = 0;
...
static int tlsg_mutex_destroy(void **lock) {
 ...
 */
static void tlsg_destroy(void) { gnutls_global_deinit(); }

static tls_ctx *tlsg_ctx_new(struct ldapoptions *lo) {
  tlsg_ctx *ctx;
...
static void tlsg_ctx_ref(tls_ctx *ctx) {
...
static void tlsg_ctx_free(tls_ctx *ctx) {
  tlsg_ctx *c = (tlsg_ctx *)ctx;
  int refcount;
...
static int tlsg_getfile(const char *path, gnutls_datum_t *buf) {
  int rc = -1, fd;
  struct stat st;
...
static int tlsg_ctx_init(struct ldapoptions *lo, struct ldaptls *lt,
  ...
  if ((lo->ldo_tls_certfile && lo->ldo_tls_keyfile) ||
    ...
    if (lo->ldo_tls_key.bv_val) {
      ...
      rc = gnutls_x509_privkey_import(key, &buf, GNUTLS_X509_FMT_DER);
    } else {
    ...
    Debug(LDAP_DEBUG_ANY, "TLS: only one of cert and key specified\n");
    return -1;
  }
  ...
  if (is_server && lo->ldo_tls_dhfile) {
    ...
    Debug(LDAP_DEBUG_ANY, "TLS: warning: ignoring dhfile\n");
  }
...
static tls_session *tlsg_session_new(tls_ctx *ctx, int is_server) {
  tlsg_ctx *c = (tlsg_ctx *)ctx;
...
static int tlsg_session_my_dn(tls_session *session, struct berval *der_dn) {
  tlsg_session *s = (tlsg_session *)session;
...
static int tlsg_session_peer_dn(tls_session *session, struct berval *der_dn) {
  tlsg_session *s = (tlsg_session *)session;
...
static int tlsg_session_strength(tls_session *session) {
  tlsg_session *s = (tlsg_session *)session;
...
static const char *tlsg_session_version(tls_session *sess) {
  tlsg_session *s = (tlsg_session *)sess;
  return gnutls_protocol_get_name(gnutls_protocol_get_version(s->session));
}

static const char *tlsg_session_cipher(tls_session *sess) {
  tlsg_session *s = (tlsg_session *)sess;
  return gnutls_cipher_get_name(gnutls_cipher_get(s->session));
}

static int tlsg_session_peercert(tls_session *sess, struct berval *der) {
  tlsg_session *s = (tlsg_session *)sess;
  const gnutls_datum_t *peer_cert_list;
  unsigned int list_size;
...
static int tlsg_session_pinning(LDAP *ld, tls_session *sess, char *hashalg,
  ...
  if (gnutls_pubkey_export(pubkey, GNUTLS_X509_FMT_DER, key.data, &len) < 0) {
    goto done;
  }
...
static ssize_t tlsg_recv(gnutls_transport_ptr_t ptr, void *buf, size_t len) {
  struct tls_data *p;
...
static int tlsg_sb_setup(Sockbuf_IO_Desc *sbiod, void *arg) {
  struct tls_data *p;
...
static int tlsg_sb_remove(Sockbuf_IO_Desc *sbiod) {
  struct tls_data *p;
...
static int tlsg_sb_close(Sockbuf_IO_Desc *sbiod) {
  struct tls_data *p;
...
static int tlsg_sb_ctrl(Sockbuf_IO_Desc *sbiod, int opt, void *arg) {
  struct tls_data *p;
...
static int tlsg_cert_verify(tlsg_session *ssl) {
...
tls_impl ldap_int_tls_impl = {"GnuTLS",
