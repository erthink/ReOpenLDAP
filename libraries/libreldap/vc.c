 ...
 *
 * The request is an extended request with OID 1.3.6.1.4.1.4203.666.6.5 with
 *value of the BER encoding of:
 *
 * VCRequest ::= SEQUENCE {
 ...
 * }
 *
 * where ResultCode is the result code enumeration from RFC 4511, and LDAPString
...
int ldap_parse_verify_credentials(LDAP *ld, LDAPMessage *res, int *code, char **diagmsg, struct berval **cookie,
  ...
  if (rc != LDAP_SUCCESS) {
    ldap_debug_perror(ld, "ldap_parse_verify_credentials");
    return rc;
  }
  ...
  if (retdata) {
    ...
    if (diagmsg != NULL) {
      *diagmsg = LDAP_MALLOC(diagmsg_bv.bv_len + 1);
      memcpy(*diagmsg, diagmsg_bv.bv_val, diagmsg_bv.bv_len);
...
int ldap_verify_credentials(LDAP *ld, struct berval *cookie, const char *dn, const char *mechanism, struct berval *cred,
  ...
  if (mechanism == LDAP_SASL_SIMPLE) {
    ...
    rc = ber_printf(ber, "{stO" /*"}"*/, dn, LDAP_AUTH_SIMPLE, cred);
