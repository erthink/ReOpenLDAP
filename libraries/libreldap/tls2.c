...
int ldap_pvt_tls_connect(LDAP *ld, Sockbuf *sb, const char *host) {
...
int ldap_pvt_tls_inplace(Sockbuf *sb) { return HAS_TLS(sb) ? 1 : 0; }
...
int ldap_pvt_tls_config(LDAP *ld, int option, const char *arg) {
  ...
  case LDAP_OPT_X_TLS_PROTOCOL_MIN: {
    ...
    if (l < 0 || l > 0xff || next == arg || (*next != '\0' && *next != '.'))
      return -1;
    i = l << 8;
    if (*next == '.') {
      arg = next + 1;
...
int ldap_pvt_tls_get_unique(void *s, struct berval *buf, int is_server) {
  tls_session *session = s;
  return tls_imp->ti_session_unique(session, buf, is_server);
}

const char *ldap_pvt_tls_get_version(void *s) {
  tls_session *session = s;
  return tls_imp->ti_session_version(session);
}

const char *ldap_pvt_tls_get_cipher(void *s) {
  tls_session *session = s;
  return tls_imp->ti_session_cipher(session);
}

int ldap_pvt_tls_get_peercert(void *s, struct berval *der) {
  tls_session *session = s;
