...
int ber_sockbuf_ctrl(Sockbuf *sb, int opt, void *arg) {
  ...
  case LBER_SB_OPT_UNGET_BUF:
    ...
    if (sb->sb_ungetlen <= sizeof(sb->sb_ungetbuf)) {
      memcpy(sb->sb_ungetbuf, ((struct berval *)arg)->bv_val, sb->sb_ungetlen);
      ret = 1;
    } else {
      sb->sb_ungetlen = 0;
      ret = -1;
...
int ber_sockbuf_remove_io(Sockbuf *sb, Sockbuf_IO *sbio, int layer) {
  ...
  while (*q != NULL) {
    ...
    if (layer == p->sbiod_level && p->sbiod_io == sbio) {
      if (p->sbiod_io->sbi_remove != NULL && p->sbiod_io->sbi_remove(p) < 0) {
        return -1;
      }
      *q = p->sbiod_next;
      LBER_FREE(p);
...
int ber_int_sb_destroy(Sockbuf *sb) {
  ...
  while (sb->sb_iod) {
    ...
    ber_sockbuf_remove_io(sb, sb->sb_iod->sbiod_io, sb->sb_iod->sbiod_level);
    sb->sb_iod = p;
  }
