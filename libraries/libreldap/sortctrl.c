...
/* ---------------------------------------------------------------------------
   ...
   pNextKey    (IN/OUT) Points to the next key in the sortkey string to parse.
                                                The pointer is updated to point
   to the next character after the sortkey being parsed.

   key         (OUT)    Points to the address of an LDAPSortKey structure
                                                which has been allocated by this
   routine and initialized with information from the next sortkey.
   ---------------------------------------------------------------------------*/
...
/* ---------------------------------------------------------------------------
   ...
   sortKeyList    (OUT) Points to a null-terminated array of pointers to
                                                LDAPSortKey structures allocated
   by this routine. This memory SHOULD be freed by the calling program using
...
/* ---------------------------------------------------------------------------
   ...
   keyList     (IN) Points to a null-terminated array of pointers to
                                        LDAPSortKey structures, containing a
   description of each of the sort keys to be used.  The description consists of
   an attribute name, ascending/descending flag, and an optional matching rule
   (OID) to use.
...
/* ---------------------------------------------------------------------------
   ...
   isCritical  (IN) 0 - Indicates the control is not critical to the operation.
                                        non-zero - The control is critical to
   the operation.
...
/* ---------------------------------------------------------------------------
   ...
   returnCode (OUT) This result parameter is filled in with the sort control
                                        result code.  This parameter MUST not be
   NULL.

   attribute  (OUT) If an error occured the server may return a string
                                        indicating the first attribute in the
   sortkey list that was in error.  If a string is returned, the memory should
   be freed with ldap_memfree.  If this parameter is NULL, no string is
   returned.
   ...
   Ber encoding for sort control
         ...
         SortResult ::= SEQUENCE {
                sortResult  ENUMERATED {
                        ...
                        operationsError           (1), -- server internal
   failure timeLimitExceeded         (3), -- timelimit reached before
                                                                                   -- sorting was completed
                        strongAuthRequired        (8), -- refused to return
   sorted
                                                                                   -- results via insecure
                                                                                   -- protocol
                        adminLimitExceeded       (11), -- too many matching
   entries
