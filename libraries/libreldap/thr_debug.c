...
/* Use native malloc - the ReOpenLDAP wrappers may defeat malloc debuggers */
#undef malloc
#undef calloc
#undef realloc
#undef free
...
static void get_options(void) {
  ...
  if (s != NULL) {
    while (*(s += strspn(s, ", \t\r\n")) != '\0') {
      ...
      while (oi->name && (strncasecmp(oi->name, s, optlen) || oi->name[optlen]))
        oi++;
      if (oi->name)
        *oi->var = oi->val;
      else
...
static void check_usage(const ldap_debug_usage_info_t *usage, const char *msg) {
  ...
  case ldap_debug_state_destroyed:
    MEMERROR_IF(Is_destroyed, msg, { PEEK(DUMMY_ADDR(usage)); });
    break;
  default:
...
static void destroy_usage(ldap_debug_usage_info_t *usage) {
...
static void remove_thread_info(ldap_debug_thread_t *t, const char *msg) {
...
static char *thread_name(char *buf, int bufsize, ldap_pvt_thread_t thread) {
  int i;
  --bufsize;
...
static void adjust_count(int which, int adjust) {
  int rc;
  switch (count) {
  case Count_no:
    break;
...
int ldap_debug_thread_initialize(void) {
  int i, rc, rc2;
  if (!options_done)
    get_options();
...
int ldap_debug_thread_destroy(void) {
  int rc;
...
static void thread_exiting(const char *how, const char *msg) {
...
static void *ldap_debug_thread_wrapper(void *arg) {
  void *ret;
...
int ldap_pvt_thread_create(ldap_pvt_thread_t *thread, int detach, void *(*start_routine)(void *), void *arg) {
  ...
  if (wrap_threads) {
    ldap_debug_thread_call_t *call = malloc(sizeof(ldap_debug_thread_call_t));
    assert(call != NULL);
...
int ldap_pvt_thread_join(ldap_pvt_thread_t thread, void **thread_return) {
  ...
  if (rc) {
    ...
    if (threadID)
      with_thread_info_lock(remove_thread_info(t, "ldap_pvt_thread_join"));
    adjust_count(Idx_unjoined_thread, -1);
  }
...
void *ldap_pvt_thread_pool_context(void) {
#if 0 /* Function is used by ch_free() via slap_sl_contxt() in slapd */
