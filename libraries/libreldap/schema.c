...
const char *ldap_syntax2name(LDAPSyntax *syn) {
  if (!syn)
    return NULL;
  return (syn->syn_oid);
}

const char *ldap_matchingrule2name(LDAPMatchingRule *mr) {
  if (!mr)
    return NULL;
  return (choose_name(mr->mr_names, mr->mr_oid));
}

const char *ldap_matchingruleuse2name(LDAPMatchingRuleUse *mru) {
  if (!mru)
    return NULL;
  return (choose_name(mru->mru_names, mru->mru_oid));
}

const char *ldap_attributetype2name(LDAPAttributeType *at) {
  if (!at)
    return NULL;
  return (choose_name(at->at_names, at->at_oid));
}

const char *ldap_objectclass2name(LDAPObjectClass *oc) {
  if (!oc)
    return NULL;
  return (choose_name(oc->oc_names, oc->oc_oid));
}

const char *ldap_contentrule2name(LDAPContentRule *cr) {
  if (!cr)
    return NULL;
  return (choose_name(cr->cr_names, cr->cr_oid));
}

const char *ldap_nameform2name(LDAPNameForm *nf) {
  if (!nf)
    return NULL;
  return (choose_name(nf->nf_names, nf->nf_oid));
}

const char *ldap_structurerule2name(LDAPStructureRule *sr) {
  if (!sr)
    return NULL;
  ...
  while (1) {
    ...
    case TK_BAREWORD:
      if (!strcasecmp(sval, "NAME")) {
        ...
        seen_syntax = 1;
        parse_whsp(&ss);
        mr->mr_syntax_oid = ldap_int_parse_numericoid(&ss, code, flags);
        if (!mr->mr_syntax_oid) {
          *errp = ss;
          ldap_matchingrule_free(mr);
          return NULL;
...
void ldap_matchingruleuse_free(LDAPMatchingRuleUse *mru) {
  ...
  if (mru->mru_applies_oids)
    ldap_charray_free(mru->mru_applies_oids);
  ...
  while (1) {
    ...
    case TK_BAREWORD:
      if (!strcasecmp(sval, "NAME")) {
        ...
        parse_whsp(&ss);
        savepos = ss;
        at->at_syntax_oid = parse_noidlen(&ss, code, &at->at_syntax_len, flags);
        if (!at->at_syntax_oid) {
