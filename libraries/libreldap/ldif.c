...
int ldif_countlines(const char *buf) {
  char *nl;
  int ret = 0;
 ...
 */
int ldif_must_b64_encode_register(const char *name, const char *oid) {
  int i;
  ber_len_t len;
  ...
  if (must_b64_encode == default_must_b64_encode) {
    ...
    tmp = ber_memrealloc(must_b64_encode, sizeof(must_b64_encode_s) * (i + 2));
    if (tmp == NULL) {
      return 1;
    }
    must_b64_encode = tmp;
 ...
 */
static int ldif_must_b64_encode(const char *s) {
  int i;
  struct berval bv;
...
/* NOTE: only preserved for binary compatibility */
char *ldif_put(int type, const char *name, const char *val, ber_len_t vlen) {
...
int ldif_is_not_printable(const char *val, ber_len_t vlen) {
  if (vlen == 0 || val == NULL) {
    return -1;
  }
...
LDIFFP *ldif_open(const char *file, const char *mode) {
...
LDIFFP *ldif_open_mem(char *ldif, size_t size, const char *mode) {
