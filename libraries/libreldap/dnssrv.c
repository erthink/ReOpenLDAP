...
int ldap_dn2domain(const char *dn_in, char **domainp) {
  int i, j;
  char *ndomain;
  LDAPDN dn = NULL;
  LDAPRDN rdn = NULL;
      ...
      for (j = 0; rdn[j] != NULL; j++) {
        ...
        if (rdn[j + 1] == NULL && (ava->la_flags & LDAP_AVA_STRING) && ava->la_value.bv_len &&
          ...
          if (domain.bv_len == 0) {
            ...
            memcpy(domain.bv_val, ava->la_value.bv_val, ava->la_value.bv_len);
...
int ldap_domain2dn(const char *domain_in, char **dnp) {
...
static void srv_srand(int seed) { srv_seed = (float)seed / (float)RAND_MAX; }
 ...
 */
int ldap_domain2hostlist(const char *domain, char **list) {
#ifdef HAVE_RES_QUERY
  char *request;
  char *hostlist = NULL;
...
#define T_SRV 33
  ...
  if (len >= 0) {
    ...
    while (p < reply + len) {
      int type, size;
      int class __maybe_unused, ttl __maybe_unused;
      status = dn_expand(reply, reply + len, p, host, sizeof(host));
      if (status < 0) {
        goto out;
      }
