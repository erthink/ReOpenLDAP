...
void ldap_ldif_record_done(LDIFRecord *lr) {
  int i;
  ...
  for (i = lr->lr_lines - 1; i >= 0; i--)
    if (lr->lr_freeval[i])
      ber_memfree_x(lr->lr_vals[i].bv_val, lr->lr_ctx);
...
/*
 * ldap_parse_ldif_record_x() will convert an LDIF record read with
 * ldif_read_record() into an array of LDAPMod* and an array of LDAPControl*,
 * suitable for passing directly to any other LDAP API function that takes
 * LDAPMod** and LDAPControl** arguments, such as ldap_modify_s().
 *
 * rbuf - the ldif record buffer returned from ldif_read_record - rbuf.bv_val
 * must be writable - will use ldif_getline to read from it linenum - the ldif
 * line number returned from ldif_read_record
 *         - used for logging errors (e.g. error at line N)
 * lr - holds the data to return
 * errstr - a string used for logging (usually the program name e.g.
 * "ldapmodify" flags - 0 or some combination of LDIF_DEFAULT_ADD
...
int ldap_parse_ldif_record_x(struct berval *rbuf, unsigned long linenum,
  ...
  int version;
  LDAPControl **pctrls;
  int i, j, k, idn = -1, nmods;
  struct berval **bvl, bv = {0};
  ...
  while (rc == 0 && (line = ldif_getline(&rbuf->bv_val)) != NULL) {
    ...
    if (dn == NULL) {
      if (linenum + i == 1 && BV_CASEMATCH(lr->lr_btype + i, &BV_VERSION)) {
        /* lutil_atoi() introduces a dependence of libreldap
