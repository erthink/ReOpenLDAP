/* $ReOpenLDAP$ */
/* Copyright 2006-2018 ReOpenLDAP AUTHORS: please see AUTHORS file.
 * All rights reserved.
 *
...
static void usage(char *name) {
  fprintf(stderr,
          ...
          "[-R %%:lo:hi] [-f <filter-template>] [-n <threads>] [-D <bindDN>] "
          "[-i <seconds>]\n",
          name);
  exit(EXIT_FAILURE);
}
...
static void *my_task(void *my_num) {
  ...
  {
    int version = LDAP_VERSION3;
    (void)ldap_set_option(sld, LDAP_OPT_PROTOCOL_VERSION, &version);
  }
