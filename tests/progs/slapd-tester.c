...
int main(int argc, char **argv) {
  ...
  while ((i = getopt(argc, argv, "AB:CD:d:FH:h:Ii:j:L:l:NP:p:r:St:Ww:y:")) !=
    ...
    case 'B': {
      char **p, **b = ldap_str2charray(optarg, ",");
      extra_t **epp;
    ...
    case 'l': /* the number of loops per client */
      if (!isdigit((unsigned char)optarg[0])) {
        char **p, **l = ldap_str2charray(optarg, ",");

        for (p = l; p[0]; p++) {
          struct {
            struct berval type;
            char *buf;
          } types[] = {{BER_BVC("add="), aloops},
                       {BER_BVC("bind="), bloops},
  ...
  if (nloops[0] == '\0')
    snprintf(nloops, sizeof(nloops), "%d", loops);
  if (mloops[0] == '\0')
  ...
  sanum = 0;
  snprintf(scmd, sizeof scmd, "%s" LDAP_DIRSEP SEARCHCMD, progdir);
  sargs[sanum++] = scmd;
  if (uri) {
    sargs[sanum++] = "-H";
    sargs[sanum++] = uri;
  ...
  ranum = 0;
  snprintf(rcmd, sizeof rcmd, "%s" LDAP_DIRSEP READCMD, progdir);
  rargs[ranum++] = rcmd;
  if (uri) {
    rargs[ranum++] = "-H";
    rargs[ranum++] = uri;
  ...
  nanum = 0;
  snprintf(ncmd, sizeof ncmd, "%s" LDAP_DIRSEP MODRDNCMD, progdir);
  nargs[nanum++] = ncmd;
  if (uri) {
    nargs[nanum++] = "-H";
    nargs[nanum++] = uri;
  ...
  manum = 0;
  snprintf(mcmd, sizeof mcmd, "%s" LDAP_DIRSEP MODIFYCMD, progdir);
  margs[manum++] = mcmd;
  if (uri) {
    margs[manum++] = "-H";
    margs[manum++] = uri;
  ...
  aanum = 0;
  snprintf(acmd, sizeof acmd, "%s" LDAP_DIRSEP ADDCMD, progdir);
  aargs[aanum++] = acmd;
  if (uri) {
    aargs[aanum++] = "-H";
    aargs[aanum++] = uri;
  ...
  banum = 0;
  snprintf(bcmd, sizeof bcmd, "%s" LDAP_DIRSEP BINDCMD, progdir);
  bargs[banum++] = bcmd;
  if (!noinit) {
...
static char *get_file_name(char *dirname, char *filename) {
  ...
  snprintf(buf, sizeof buf, "%s" LDAP_DIRSEP "%s", dirname, filename);
  return (strdup(buf));
}
