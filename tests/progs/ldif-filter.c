...
static unsigned get_flags(const char *backend, const char *spec) {
...
static void build_entry(Strings *ss, size_t n, unsigned flags, size_t new_len) {
...
static void flush_entries(Strings *ss, const char *sep, unsigned flags) {
  size_t i, end = ss->len;
  const char *prefix = "";
...
static void filter_stdin(unsigned flags) {
  char line[256];
