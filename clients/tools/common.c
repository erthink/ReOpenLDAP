      ...
      if (strcasecmp(control, "nettimeout") == 0) {
        ...
        if (cvalue == 0) {
          ...
          if (lutil_atou(&u, cvalue)) {
            fprintf(stderr, _("Unable to parse ldif-wrap=\"%s\"\n"), cvalue);
            exit(EXIT_FAILURE);
          }
...
void tool_bind(LDAP *ld) {
  ...
  if (authmethod == LDAP_AUTH_SASL) {
    ...
    rc =
        ldap_sasl_interactive_bind_s(ld, binddn, sasl_mech, sctrlsp, NULL,
    ...
    if (rc != LDAP_SUCCESS) {
      char *msg = NULL;
      ldap_get_option(ld, LDAP_OPT_DIAGNOSTIC_MESSAGE, (void *)&msg);
      tool_perror("ldap_sasl_interactive_bind_s", rc, NULL, NULL, msg, NULL);
      ldap_memfree(msg);
      tool_exit(ld, rc);
    }
#else
    ...
    if (result) {
      rc = ldap_parse_result(ld, result, &err, &matched, &info, &refs, &ctrls,
                             1);
      if (rc != LDAP_SUCCESS) {
    ...
    if (ctrls && ppolicy) {
      ...
      ctrl =
          ldap_control_find(LDAP_CONTROL_PASSWORDPOLICYRESPONSE, ctrls, NULL);
