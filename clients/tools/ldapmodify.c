...
static int process_ldif_rec(char *rbuf, unsigned long linenum) {
  ...
  while (rc == 0 && (line = ldif_getline(&rbuf)) != NULL) {
    ...
    if (dn == NULL) {
      if (linenum + i == 1 && BV_CASEMATCH(btype + i, &BV_VERSION)) {
        int v;
        if (vals[i].bv_len == 0 || lutil_atoi(&v, vals[i].bv_val) != 0 ||
            v != 1) {
  ...
  if (BV_CASEMATCH(btype + i, &BV_CONTROL)) {
    ...
    if (rc != 0) {
      fprintf(stderr, _("%s: Error processing %s line, line %lu: %s\n"), prog,
              BV_CONTROL.bv_val, linenum + i, ldap_err2string(rc));
    }
    i++;
    if (i >= lines) {
    short_input:
  ...
  if (new_entry) {
     ...
     */
    lm =
        ber_memalloc(nmods * sizeof(LDAPMod) + (nmods + 1) * sizeof(LDAPMod *) +
  ...
  if (rc == 0) {
    if (pctrls) {
      ...
      if (defctrls) {
        ...
        newctrls =
            ber_memrealloc(pctrls, (npc + ndefc + 1) * sizeof(LDAPControl *));
...
static int dorename(const char *dn, const char *newrdn, const char *newsup,
  ...
  if (!dont) {
    rc =
        ldap_rename(ld, dn, newrdn, newsup, deleteoldrdn, pctrls, NULL, &msgid);
    if (rc != LDAP_SUCCESS) {
