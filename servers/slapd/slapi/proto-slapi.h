...
LDAP_SLAPI_F (Modifications *) slapi_int_ldapmods2modifications( Operation *op, LDAPMod ** );
LDAP_SLAPI_F (int) slapi_int_count_controls( LDAPControl **ctrls );
LDAP_SLAPI_F (char **) slapi_get_supported_extended_ops(void);
LDAP_SLAPI_F (int) slapi_int_access_allowed(Operation *op, Entry *entry, AttributeDescription *desc, struct berval *val, slap_access_t access, AccessControlState *state );

/* slapi_ops.c */
LDAP_SLAPI_F (int) slapi_int_response( Slapi_Operation *op, SlapReply *rs );
LDAP_SLAPI_F (void) slapi_int_connection_init_pb( Slapi_PBlock *pb, ber_tag_t OpType );
...
LDAP_SLAPI_F (int) slapi_int_call_plugins(Backend *be, int funcType, Slapi_PBlock * pPB);
LDAP_SLAPI_F (int) slapi_int_get_plugins(Backend *be, int functype, SLAPI_FUNC **ppFuncPtrs);
LDAP_SLAPI_F (int) slapi_int_register_extop(Backend *pBE, ExtendedOp **opList, Slapi_PBlock *pPB);
LDAP_SLAPI_F (int) slapi_int_get_extop_plugin(struct berval  *reqoid, SLAPI_FUNC *pFuncAddr );
LDAP_SLAPI_F (struct berval *) slapi_int_get_supported_extop( int );
LDAP_SLAPI_F (int) slapi_int_read_config (Backend *be, const char *fname, int lineno,
		int argc, char **argv );
...
LDAP_SLAPI_F (int) slapi_int_free_object_extensions(int objecttype, void *object);
LDAP_SLAPI_F (int) slapi_int_create_object_extensions(int objecttype, void *object);
LDAP_SLAPI_F (int) slapi_int_clear_object_extensions(int objecttype, void *object);
