...
static char *pblock_get_authtype(AuthorizationInformation *authz, int is_tls) {
  char *authType;
...
static int pblock_set_default(Slapi_PBlock *pb, int param, void *value) {
  slapi_pblock_class_t pbClass;
  int i;
...
static int pblock_be_call(Slapi_PBlock *pb, int (*bep)(Operation *)) {
  BackendDB *be_orig;
  Operation *op;
  int rc;
...
static int pblock_get(Slapi_PBlock *pb, int param, void **value) {
...
static int pblock_add_control(Slapi_PBlock *pb, LDAPControl *control) {
...
static int pblock_set(Slapi_PBlock *pb, int param, void *value) {
  int rc = PBLOCK_SUCCESS;
...
static void pblock_clear(Slapi_PBlock *pb) { pb->pb_nParams = 1; }

static int pblock_delete_param(Slapi_PBlock *p, int param) {
  int i;
...
static void pblock_destroy(Slapi_PBlock *pb) {
  LDAPControl **controls = NULL;
  LDAPMod **mods = NULL;
  char **attrs = NULL;
 ...
 * OpenLDAP extension
 */
int slapi_int_pblock_get_first(Backend *be, Slapi_PBlock **pb) {
  assert(pb != NULL);
 ...
 * OpenLDAP extension
 */
int slapi_int_pblock_get_next(Slapi_PBlock **pb) {
  assert(pb != NULL);
