 ...
 *
 * Description:        insert the slapi_pblock structure to the end of the
 *plugin list
 *
 * Input:              a pointer to a plugin slapi_pblock structure to be added
 *to the list
 *
 * Output:             none
 *
 ...
 *                     in all the plugins
 *
 * Input:              the type of the functions to get, such as
 *pre-operation,etc.
 *
 * Output:             none
 *
...
int slapi_int_get_plugins(Backend *be, int functype, SLAPI_FUNC **ppFuncPtrs) {
  ...
  while (pCurrentPB != NULL && rc == LDAP_SUCCESS) {
    ...
    if (rc == LDAP_SUCCESS) {
      ...
      rc = slapi_pblock_get(pCurrentPB, SLAPI_IBM_PBLOCK, &pCurrentPB);
    }
  }
 ...
 *					   asscoiated with a particular extended
 *operation plugin.
 *
...
void slapi_int_unregister_extop(Backend *pBE, ExtendedOp **opList, Slapi_PBlock *pPB) {
  ...
  for (i = 0; pTmpOIDs[i] != NULL; i++) {
    ...
    for (; pTmpExtOp != NULL; pTmpExtOp = pTmpExtOp->ext_next) {
      int rc;
      rc = strcasecmp(pTmpExtOp->ext_oid.bv_val, pTmpOIDs[i]);
      if (rc == 0) {
        if (backExtOp == NULL) {
          *opList = pTmpExtOp->ext_next;
        } else {
          backExtOp->ext_next = pTmpExtOp->ext_next;
        }
 ...
 *                     in the extended op module and put the extended op
 *function address in the structure. The function will not be executed in this
 *routine.
 *
 * Input:              pBE - pointer to a backend structure
 *                     opList - pointer to a linked list of extended
 ...
 *
 * Description:        This routine gets the function address for a given
 *function name.
 *
 * Input:
 *                     funcName - name of the extended op function, ie. an OID.
 *
 * Output:             pFuncAddr - the function address of the requested
 *function name.
 *
...
/***************************************************************************
 * This function is similar to slapi_int_get_extop_plugin above. except it
 *returns one OID per call. It is called from root_dse_info (root_dse.c). The
 *function is a modified version of get_supported_extop (file extended.c).
 ***************************************************************************/
struct berval *slapi_int_get_supported_extop(int index) {
  ExtendedOp *ext;

  for (ext = pGExtendedOps; ext != NULL && --index >= 0; ext = ext->ext_next) {
    ; /* empty */
  }
 ...
 *
 * Description:        This routine loads the specified solib, gets and executes
 *the init function if requested.
 *
 * Input:
 *                     pPlugin - a pointer to a Slapi_PBlock struct which will
 *be passed to the solib init function. path - path name of the solib to be
 *load. initfunc - either the solib initialization function or an OID of the
 *                                loaded extended operation.
 *                     doInit - if it is TRUE, execute the init function,
 *otherwise, save the function address but not execute it.
 *
 * Output:             pInitFunc - the function address of the loaded function.
 *This param should be not be null if doInit is FALSE. pLdHandle - handle
 *returned by lt_dlopen()
 *
