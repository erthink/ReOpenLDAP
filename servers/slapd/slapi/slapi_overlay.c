...
static Slapi_PBlock *slapi_over_pblock_new(Operation *op, SlapReply *rs) {
  Slapi_PBlock *pb;
...
static int slapi_over_aux_operational(Operation *op, SlapReply *rs) {
 ...
 */
static int slapi_over_call_plugins(Slapi_PBlock *pb, int type) {
...
static int slapi_over_search(Operation *op, SlapReply *rs, int type) {
  int rc;
  Slapi_PBlock *pb;
...
static int slapi_op_bind_callback(Operation *op, SlapReply *rs, int prc) {
  switch (prc) {
...
static int slapi_op_search_callback(Operation *op, SlapReply *rs, int prc) {
  Slapi_PBlock *pb = SLAPI_OPERATION_PBLOCK(op);
...
static int slapi_over_merge_controls(Operation *op, SlapReply *rs) {
  Slapi_PBlock *pb = SLAPI_OPERATION_PBLOCK(op);
...
static int slapi_over_unmerge_controls(Operation *op, SlapReply *rs) {
  Slapi_PBlock *pb = SLAPI_OPERATION_PBLOCK(op);
...
static int slapi_op_func(Operation *op, SlapReply *rs) {
  Slapi_PBlock *pb;
...
static int slapi_over_extended(Operation *op, SlapReply *rs) {
  Slapi_PBlock *pb;
  SLAPI_FUNC callback;
  int rc;
  int internal_op;
...
static int slapi_over_db_open(BackendDB *be, ConfigReply *cr) {
  Slapi_PBlock *pb;
  int rc;
...
static int slapi_over_db_close(BackendDB *be, ConfigReply *cr) {
  Slapi_PBlock *pb;
  int rc;
