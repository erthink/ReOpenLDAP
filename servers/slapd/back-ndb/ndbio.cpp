...
extern "C" NdbAttrInfo *ndb_ai_find(struct ndb_info *ni, AttributeType *at) {
  NdbAttrInfo atmp;
...
extern "C" NdbAttrInfo *ndb_ai_get(struct ndb_info *ni, struct berval *aname) {
  NdbAttrInfo atmp, *ai;
  atmp.na_name = *aname;
...
static int ndb_oc_create(struct ndb_info *ni, NdbOcInfo *oci, int create) {
  ...
  if (create) {
    ptr = buf + sprintf(buf,
                        "CREATE TABLE `%s` (eid bigint unsigned NOT NULL, vid "
...
extern "C" int ndb_aset_create(struct ndb_info *ni, NdbOcInfo *oci) {
  char buf[4096], *ptr;
  NdbAttrInfo *ai;
  int i;

  ptr = buf + sprintf(buf,
                      "CREATE TABLE IF NOT EXISTS `%s` (eid bigint unsigned "
                      "NOT NULL, vid int unsigned NOT NULL",
                      oci->no_table.bv_val);
...
extern "C" int ndb_entry_put_data(BackendDB *be, NdbArgs *NA) {
  struct ndb_info *ni = (struct ndb_info *)be->be_private;
  Attribute *aoc;
 ...
 */
extern "C" int ndb_entry_get_data(Operation *op, NdbArgs *NA, int update) {
...
extern "C" int ndb_entry_del_data(BackendDB *be, NdbArgs *NA) {
  struct ndb_info *ni = (struct ndb_info *)be->be_private;
...
extern "C" int ndb_dn2rdns(struct berval *dn, NdbRdns *rdns) {
  char *beg, *end;
  int i, len;
...
extern "C" int ndb_entry_put_info(BackendDB *be, NdbArgs *NA, int update) {
  struct ndb_info *ni = (struct ndb_info *)be->be_private;
  const NdbDictionary::Dictionary *myDict = NA->ndb->getDictionary();
...
extern "C" struct berval *ndb_ref2oclist(const char *ref, void *ctx) {
  char *implied;
...
extern "C" int ndb_entry_del_info(BackendDB *be, NdbArgs *NA) {
...
extern "C" int ndb_next_id(BackendDB *be, Ndb *ndb, ID *id) {
  struct ndb_info *ni __maybe_unused = (struct ndb_info *)be->be_private;
  const NdbDictionary::Dictionary *myDict = ndb->getDictionary();
...
extern "C" int ndb_thread_handle(Operation *op, Ndb **ndb) {
  struct ndb_info *ni = (struct ndb_info *)op->o_bd->be_private;
  void *data;
...
extern "C" void ndb_trans_backoff(int num_retries) {
  int i;
  int delay = 0;
  int pow_retries = 1;
  unsigned long key = 0;
...
extern "C" void ndb_check_referral(Operation *op, SlapReply *rs, NdbArgs *NA) {
