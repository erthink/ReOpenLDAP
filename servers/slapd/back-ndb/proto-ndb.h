...
extern int ndb_entry_put_data(BackendDB *be, NdbArgs *args);
...
extern int ndb_dn2rdns(struct berval *dn, NdbRdns *buf);

extern NdbAttrInfo *ndb_ai_find(struct ndb_info *ni, AttributeType *at);
...
extern int ndb_next_id(BackendDB *be, Ndb *ndb, ID *id);

extern int ndb_thread_handle(Operation *op, Ndb **ndb);

extern int ndb_back_init_cf(BackendInfo *bi);
