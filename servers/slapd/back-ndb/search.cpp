...
static int ndb_dn2bound(NdbIndexScanOperation *myop, NdbRdns *rdns) {
  int i;
 ...
 *
 * If any of the filter terms are indexed, then only an IndexScan of the
 * OL_index will be performed. If none are indexed, but all the terms reside in
 * a single table, a Scan can be performed with the LDAP filter transformed into
 * a ScanFilter.
 *
...
static int ndb_oc_search(Operation *op, SlapReply *rs, Ndb *ndb,
  ...
  struct berval *ocs;
  NdbRecAttr *scanID, *scanOC __maybe_unused, *scanDN[NDB_MAX_RDNS];
  char dnBuf[2048], *ptr;
  NdbRdns rdns;
  NdbArgs NA;
...
extern "C" int ndb_back_search(Operation *op, SlapReply *rs) {
  struct ndb_info *ni = (struct ndb_info *)op->o_bd->be_private;
  NdbTransaction *txn;
  NdbIndexScanOperation *scan;
  NdbScanFilter *sf = NULL;
...
extern "C" int ndb_has_children(NdbArgs *NA, int *hasChildren) {
  NdbIndexScanOperation *scan;
  char idbuf[2 * sizeof(ID)];
  int rc;
 ...
 */
extern "C" int ndb_operational(Operation *op, SlapReply *rs) {
  Attribute **ap;
