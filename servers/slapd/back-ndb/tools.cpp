...
extern "C" int ndb_tool_entry_open(BackendDB *be, int mode) {
...
extern "C" int ndb_tool_entry_close(BackendDB *be) {
  if (myPutTxn) {
...
extern "C" ID ndb_tool_entry_next(BackendDB *be) {
  char *ptr;
  ID id;
  int i;
...
extern "C" ID ndb_tool_entry_first(BackendDB *be) {
  int i;
...
extern "C" ID ndb_tool_dn2id_get(Backend *be, struct berval *dn) {
  NdbArgs NA;
  NdbRdns rdns;
  Entry e;
  char text[1024];
...
extern "C" Entry *ndb_tool_entry_get(BackendDB *be, ID id) {
  NdbArgs NA;
  int rc;
  char text[1024];
  Operation op = {0};
...
static struct berval glueval[] = {BER_BVC("glue"), BER_BVNULL};

static int ndb_dnid_cmp(const void *v1, const void *v2) {
  struct dn_id *dn1 = (struct dn_id *)v1, *dn2 = (struct dn_id *)v2;
...
extern "C" ID ndb_tool_entry_put(BackendDB *be, Entry *e, struct berval *text) {
