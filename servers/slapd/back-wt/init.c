...
static int wt_db_init(BackendDB *be, ConfigReply *cr) {
  struct wt_info *wi;
...
static int wt_db_open(BackendDB *be, ConfigReply *cr) {
  ...
  rc = wiredtiger_open(wi->wi_home, NULL, wi->wi_config, &wi->wi_conn);
  if (rc) {
    Debug(LDAP_DEBUG_ANY,
          "wt_db_open: database \"%s\": "
  ...
  rc = session->create(session, WT_TABLE_ID2ENTRY,
                       "key_format=Q,"
                       "value_format=Su,"
                       "columns=(id,dn,entry)");
  if (rc) {
  ...
  rc = session->create(session, WT_TABLE_DN2ID,
                       "key_format=S,"
                       "value_format=SQQ,"
  ...
  rc = cache_session->create(cache_session, WT_TABLE_IDLCACHE,
                             "key_format=Sb,"
                             "value_format=u,"
...
static int wt_db_close(BackendDB *be, ConfigReply *cr) {
  struct wt_info *wi = (struct wt_info *)be->be_private;
  int rc;
  ...
  if (rc) {
    Debug(LDAP_DEBUG_ANY, "wt_db_close: cannot close database (%d).\n", errno);
    return -1;
  }
...
static int wt_db_destroy(Backend *be, ConfigReply *cr) {
  struct wt_info *wi = (struct wt_info *)be->be_private;
...
int wt_back_initialize(BackendInfo *bi) {
  static const char *controls[] = {LDAP_CONTROL_ASSERT,
