 ...
 */
int wt_attr_slot(struct wt_info *wi, AttributeDescription *ad, int *ins) {
...
static int ainfo_insert(struct wt_info *wi, AttrInfo *a) {
  int x = INT_MAX;
  ...
  wi->wi_attrs = ch_realloc(wi->wi_attrs, (wi->wi_nattrs + 1) * sizeof(AttrInfo *));
  if (x < wi->wi_nattrs)
    memmove(&wi->wi_attrs[x + 1], &wi->wi_attrs[x], (wi->wi_nattrs - x) * sizeof(AttrInfo *));
  wi->wi_attrs[x] = a;
  wi->wi_nattrs++;
  return 0;
}
...
int wt_attr_index_config(struct wt_info *wi, const char *fname, int lineno, int argc, char **argv,
  ...
  if (!mask) {
    if (c_reply) {
      snprintf(c_reply->msg, sizeof(c_reply->msg), "no indexes selected");
      fprintf(stderr, "%s: line %d: %s\n", fname, lineno, c_reply->msg);
    }
    rc = LDAP_PARAM_ERROR;
    goto done;
...
static int wt_attr_index_unparser(void *v1, void *v2) {
  AttrInfo *ai = v1;
  BerVarray *bva = v2;
  struct berval bv;
  char *ptr;
  ...
  if (bv.bv_len) {
    ...
    bv.bv_val = lutil_strcopy(ptr, (const char *)ai->ai_desc->ad_cname.bv_val);
    *bv.bv_val++ = ' ';
...
void wt_attr_info_free(AttrInfo *ai) {
#ifdef LDAP_COMP_MATCH
  free(ai->ai_cr);
#endif
  free(ai);
