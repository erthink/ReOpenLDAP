...
static int index_at_values(Operation *op, wt_ctx *wc, AttributeDescription *ad, AttributeType *type,
  ...
  if (type->sat_sup) {
    /* recurse */
    rc = index_at_values(op, wc, NULL, type->sat_sup, tags, vals, id, opid);
  ...
  if (type->sat_ad) {
    ...
    if (ai) {
      ...
      if (mask) {
        rc = indexer(op, wc, ad, &type->sat_cname, vals, id, ixop, mask);
        if (rc)
          return rc;
      }
    }
    ...
    if (desc) {
      ...
      if (ai) {
        if (opid == WT_INDEX_UPDATE_OP)
          ...
          mask = ai->ai_newmask ? ai->ai_newmask : ai->ai_indexmask;
        if (mask) {
          rc = indexer(op, wc, desc, &desc->ad_cname, vals, id, ixop, mask);
...
int wt_index_entry(Operation *op, wt_ctx *wc, int opid, Entry *e) {
  int rc;
