...
int wt_delete(Operation *op, SlapReply *rs) {
  ...
  if (!SLAP_SHADOW(op->o_bd)) {
    ...
    vals[0] = op->o_csn;
    BER_BVZERO(&vals[1]);
    rs->sr_err = wt_index_values(op, wc, slap_schema.si_ad_entryCSN, vals, 0, SLAP_INDEX_ADD_OP);
