...
static int over_access_allowed(Operation *op, Entry *e,
  ...
  for (; on; on = on->on_next) {
    ...
    if (on->on_bi.bi_access_allowed) {
      ...
      rc = on->on_bi.bi_access_allowed(op, e, desc, val, access, state, maskp);
      if (rc != SLAP_CB_CONTINUE)
        break;
    }
  }
...
static int over_acl_group(Operation *op, Entry *e, struct berval *gr_ndn,
  ...
  for (; on; on = on->on_next) {
    ...
    if (on->on_bi.bi_acl_group) {
      ...
      op->o_bd->bd_info = (BackendInfo *)on;
      rc = on->on_bi.bi_acl_group(op, e, gr_ndn, op_ndn, group_oc, group_at);
      if (rc != SLAP_CB_CONTINUE)
        break;
    }
  }
      ...
      for (i = 0; on->on_bi.bi_obsolete_names[i] != NULL; i++) {
        if (strcmp(on->on_bi.bi_obsolete_names[i], over_type) == 0) {
          Debug(LDAP_DEBUG_ANY,
                "overlay_find(\"%s\"): "
                "obsolete name for \"%s\".\n",
                on->on_bi.bi_obsolete_names[i], on->on_bi.bi_type);
          goto foundit;
        }
      }
    }
...
int overlay_is_over(BackendDB *be) { return be->bd_info->bi_type == overtype; }
