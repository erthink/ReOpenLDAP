...
int dnssrv_back_bind(Operation *op, SlapReply *rs) {
  ...
  if (!BER_BVISNULL(&op->orb_cred) && !BER_BVISEMPTY(&op->orb_cred)) {
    /* simple bind */
    Statslog(LDAP_DEBUG_STATS, "%s DNSSRV BIND dn=\"%s\" provided cleartext passwd\n", op->o_log_prefix,
             BER_BVISNULL(&op->o_req_dn) ? "" : op->o_req_dn.bv_val);

    send_ldap_error(op, rs, LDAP_UNWILLING_TO_PERFORM, "you shouldn't send strangers your password");
    ...
    send_ldap_error(op, rs, LDAP_UNWILLING_TO_PERFORM, "anonymous bind expected");
  }
