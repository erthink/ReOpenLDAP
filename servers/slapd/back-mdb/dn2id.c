 ...
 */
int mdb_dn2id(Operation *op, MDB_txn *txn, MDB_cursor *mc, struct berval *in,
...
int mdb_id2name(Operation *op, MDB_txn *txn, MDB_cursor **cursp, ID id,
...
int mdb_dn2id_walk(Operation *op, IdScopes *isc) {
  ...
  for (;;) {
    ...
    if (!rc) {
      ...
      isc->rdns[n].bv_len =
          data.mv_size - sizeof(diskNode) - isc->nrdns[n].bv_len - sizeof(ID);
