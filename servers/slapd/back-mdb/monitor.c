...
static int mdb_monitor_idx_entry_add(struct mdb_info *mdb, Entry *e);
...
static struct {
  char *name;
  char *oid;
} s_oid[] = {{"olmMDBAttributes", "olmDatabaseAttributes:1"},
...
static int mdb_monitor_free(Entry *e, void **priv) {
  ...
  *priv = NULL;
  ...
  int rc = modify_delete_values(e, &mod, 1, &text, textbuf, sizeof(textbuf));
...
static int mdb_monitor_initialize(void) {
  ...
  for (i = 0; s_oid[i].name; i++) {
    ...
    if (parse_oidm(&c, 0, NULL) != 0) {
      Debug(LDAP_DEBUG_ANY,
            LDAP_XSTRING(mdb_monitor_initialize) ": unable to add "
  ...
  for (i = 0; s_at[i].desc != NULL; i++) {
    ...
    if (code != LDAP_SUCCESS) {
      Debug(LDAP_DEBUG_ANY,
            LDAP_XSTRING(mdb_monitor_initialize) ": register_at failed for "
                                                 "attributeType (%s)\n",
            s_at[i].desc);
      return 3;
  ...
  for (i = 0; s_oc[i].desc != NULL; i++) {
    ...
    if (code != LDAP_SUCCESS) {
      Debug(LDAP_DEBUG_ANY,
            LDAP_XSTRING(mdb_monitor_initialize) ": register_oc failed for "
                                                 "objectClass (%s)\n",
            s_oc[i].desc);
      return 4;
...
int mdb_monitor_db_open(BackendDB *be) {
...

  {
    ...
    char *fname = mdb->mi_dbenv_home, *ptr;
...
static int mdb_monitor_bitmask2key(slap_mask_t bitmask) {
  int key;
...
static int mdb_monitor_idx_apply(void *v_idx, void *v_valp) {
  ...
  ber_len_t count_len[MDB_MONITOR_IDX_TYPES], idx_len;
  int i, num = 0;
...
static int mdb_monitor_idx_entry_add(struct mdb_info *mdb, Entry *e) {
  ...
  avl_apply(mdb->mi_idx, mdb_monitor_idx_apply, &vals, -1, AVL_INORDER);
