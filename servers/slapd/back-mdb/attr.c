 ...
 */
int mdb_attr_slot(struct mdb_info *mdb, AttributeDescription *ad, int *ins) {
...
static int ainfo_insert(struct mdb_info *mdb, AttrInfo *a) {
  int x = 0;
...
int mdb_attr_dbs_open(BackendDB *be, MDBX_txn *tx0, ConfigReply *cr) {
...
static int mdb_attr_index_unparser(void *v1, void *v2) {
  AttrInfo *ai = v1;
  BerVarray *bva = v2;
  struct berval bv;
  char *ptr;
...
int mdb_attr_multi_config(struct mdb_info *mdb, const char *fname, int lineno, int argc, char **argv,
  ...
  if (lo > hi) {
  badval:
    snprintf(c_reply->msg, sizeof(c_reply->msg), "invalid hi/lo thresholds");
    fprintf(stderr, "%s: line %d: %s\n", fname, lineno, c_reply->msg);
    return LDAP_PARAM_ERROR;
  }
...
static int mdb_attr_multi_unparser(void *v1, void *v2) {
  AttrInfo *ai = v1;
  BerVarray *bva = v2;
  struct berval bv;
