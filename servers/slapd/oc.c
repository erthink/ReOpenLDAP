...
int is_entry_objectclass(Entry *e, ObjectClass *oc, unsigned flags) {
  ...
  for (bv = attr->a_vals; bv->bv_val; bv++) {
    ...
    if (!(flags & SLAP_OCF_SET_FLAGS)) {
      ...
      if ((flags & SLAP_OCF_CHECK_SUP) && is_object_subclass(oc, objectClass)) {
        return 1;
      }
    }
...
static LDAP_STAILQ_HEAD(OCList, ObjectClass) oc_list = LDAP_STAILQ_HEAD_INITIALIZER(oc_list);
...
static LDAP_STAILQ_HEAD(OCUList, ObjectClass) oc_undef_list = LDAP_STAILQ_HEAD_INITIALIZER(oc_undef_list);
...
static int oc_create_allowed(ObjectClass *soc, char **attrs, int *op, const char **err) {
  ...
  if (attrs) {
    ...
    while (*attrs1) {
      ...
      if (at_find_in_list(sat, soc->soc_required) < 0 && at_find_in_list(sat, soc->soc_allowed) < 0) {
...
static int oc_add_sups(ObjectClass *soc, char **sups, int *op, const char **err) {
  ...
  if (sups) {
    if (!soc->soc_sups) {
      ...
      soc->soc_sups = (ObjectClass **)ch_calloc(nsups, sizeof(ObjectClass *));
    }
    ...
    while (*sups1) {
       ...
       */
      if (soc->soc_kind != soc1->soc_kind && soc1->soc_kind != LDAP_SCHEMA_ABSTRACT) {
        *err = *sups1;
...
static void oc_delete_names(ObjectClass *oc) {
  ...
  while (*names) {
    ...
    tmpoir.oir_oc = oc;
    oir = (struct oindexrec *)avl_delete(&oc_index, (caddr_t)&tmpoir, oc_index_cmp);
    assert(oir != NULL);
    ldap_memfree(oir);
    names++;
  }
...
static int oc_insert(ObjectClass **roc, ObjectClass *prev, const char **err) {
  ...
  if ((names = soc->soc_names)) {
    while (*names) {
      ...
      if (avl_insert(&oc_index, (caddr_t)oir, oc_index_cmp, avl_dup_error)) {
        ObjectClass *old_soc;
        int rc;
