...
int slapmodify(int argc, char **argv) {
  char *buf = NULL;
  const char *text;
  ...
  for (lineno = 1;
    ...
    if (enable_meter)
      lutil_meter_update(&meter, ftell(ldiffp->fp), 0);
    ...
    if (BER_BVISEMPTY(&ndn) && !BER_BVISEMPTY(be->be_nsuffix)) {
      fprintf(stderr,
              "%s: line %lu: "
    ...
    if (lr.lrop_mods) {
      for (n = 0; lr.lrop_mods && lr.lrop_mods[n] != NULL; n++) {
        ...
        switch (mods.sm_op) {
        case LDAP_MOD_ADD:
          local_rc = modify_add_values(e, &mods, 0, &text, textbuf, textlen);
          break;

        case LDAP_MOD_DELETE:
          local_rc = modify_delete_values(e, &mods, 0, &text, textbuf, textlen);
          break;
    ...
    if (SLAP_LASTMOD(be) && e != NULL) {
      ...
      slap_timestamp(ldap_time_steady(), &timestamp);
    ...
    if (!dryrun) {
      ...
      if (rc != LDAP_SUCCESS) {
        fprintf(stderr,
                "%s: could not %s entry dn=\"%s\" "
                "(line=%lu): %s\n",
                progname, request, ndn.bv_val, lineno, bvtext.bv_val);
        rc = EXIT_FAILURE;
        goto cleanup;
      }
  ...
  cleanup:;
    ...
    if (rc != LDAP_SUCCESS && !continuemode)
      break;
  }
