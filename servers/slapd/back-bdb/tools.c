...
static int bdb_tool_entry_get_int(BackendDB *be, ID id, Entry **ep);
...
ID bdb_tool_dn2id_get(Backend *be, struct berval *dn) {
  Operation op = {0};
  Opheader ohdr = {0};
  EntryInfo *ei = NULL;
  int rc;
...
static int bdb_tool_index_add(Operation *op, DB_TXN *txn, Entry *e) {
...
ID bdb_tool_entry_put(BackendDB *be, Entry *e, struct berval *text) {
  int rc;
  struct bdb_info *bdb;
  DB_TXN *tid = NULL;
  Operation op = {0};
...
int bdb_tool_entry_reindex(BackendDB *be, ID id, AttributeDescription **adv) {
...
ID bdb_tool_entry_modify(BackendDB *be, Entry *e, struct berval *text) {
  int rc;
  struct bdb_info *bdb;
  DB_TXN *tid = NULL;
  Operation op = {0};
...
#ifdef BDB_TOOL_IDL_CACHING
static int bdb_tool_idl_cmp(const void *v1, const void *v2) {
...
static int bdb_tool_idl_flush_one(void *v1, void *arg) {
...
static int bdb_tool_idl_flush_db(DB *db, bdb_tool_idl_cache *ic) {
...
static void *bdb_tool_trickle_task(void *ctx, void *ptr) {
  ...
  while (1) {
    ldap_pvt_thread_cond_wait(&bdb_tool_trickle_cond, &bdb_tool_trickle_mutex);
    if (slapd_shutdown)
      break;
...
static void *bdb_tool_index_task(void *ctx, void *ptr) {
  ...
  while (1) {
    ...
    ldap_pvt_thread_cond_wait(&bdb_tool_index_cond_work, &bdb_tool_index_mutex);
    if (slapd_shutdown) {
      bdb_tool_index_tcount--;
      if (!bdb_tool_index_tcount)
        ldap_pvt_thread_cond_signal(&bdb_tool_index_cond_main);
