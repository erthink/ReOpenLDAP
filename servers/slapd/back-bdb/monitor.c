...
static AttributeDescription *ad_olmBDBEntryCache, *ad_olmBDBDNCache, *ad_olmBDBIDLCache, *ad_olmDbDirectory;

#ifdef BDB_MONITOR_IDX
static int bdb_monitor_idx_entry_add(struct bdb_info *bdb, Entry *e);
...
static struct {
  char *name;
  char *oid;
} s_oid[] = {{"olmBDBAttributes", "olmDatabaseAttributes:1"},
...
static int bdb_monitor_initialize(void) {
  ...
  for (i = 0; s_oid[i].name; i++) {
    ...
    if (parse_oidm(&c, 0, NULL) != 0) {
      Debug(LDAP_DEBUG_ANY,
            LDAP_XSTRING(bdb_monitor_initialize) ": unable to add "
  ...
  for (i = 0; s_at[i].desc != NULL; i++) {
    ...
    if (code != LDAP_SUCCESS) {
      Debug(LDAP_DEBUG_ANY,
            LDAP_XSTRING(bdb_monitor_initialize) ": register_at failed for "
                                                 "attributeType (%s)\n",
            s_at[i].desc);
      return 3;
  ...
  for (i = 0; s_oc[i].desc != NULL; i++) {
    ...
    if (code != LDAP_SUCCESS) {
      Debug(LDAP_DEBUG_ANY,
            LDAP_XSTRING(bdb_monitor_initialize) ": register_oc failed for "
                                                 "objectClass (%s)\n",
            s_oc[i].desc);
      return 4;
...
int bdb_monitor_db_open(BackendDB *be) {
  ...
  if (!mbe->is_configured()) {
    ...
    if (warning++ == 0) {
      Debug(LDAP_DEBUG_ANY, LDAP_XSTRING(bdb_monitor_db_open) ": monitoring disabled; "
...

  {
    ...
    char *fname = bdb->bi_dbenv_home, *ptr;
  ...
  if (rc == 0) {
    rc = mbe->register_entry_attrs(&bdb->bi_monitor.bdm_ndn, a, cb, NULL, 0, NULL);
  }
...
int bdb_monitor_db_close(BackendDB *be) {
  ...
  if (!BER_BVISNULL(&bdb->bi_monitor.bdm_ndn)) {
    ...
    if (mi && mi->bi_extra) {
      mbe = mi->bi_extra;
      mbe->unregister_entry_callback(&bdb->bi_monitor.bdm_ndn, (monitor_callback_t *)bdb->bi_monitor.bdm_cb, NULL, 0,
                                     NULL);
    }
...
static int bdb_monitor_bitmask2key(slap_mask_t bitmask) {
  int key;

  for (key = 0; key < 8 * (int)sizeof(slap_mask_t) && !(bitmask & 0x1U); key++)
    bitmask >>= 1;
...
static ber_len_t bdb_monitor_idx2len(monitor_idx_t *idx) {
  int i;
  ber_len_t len = 0;
...
static int monitor_idx_cmp(const void *p1, const void *p2) {
...
static int monitor_idx_dup(void *p1, void *p2) {
...
int bdb_monitor_idx_add(struct bdb_info *bdb, AttributeDescription *desc, slap_mask_t type) {
  ...
  idx = (monitor_idx_t *)avl_find(bdb->bi_idx, (caddr_t)&idx_dummy, monitor_idx_cmp);
  if (idx == NULL) {
    idx = (monitor_idx_t *)ch_calloc(sizeof(monitor_idx_t), 1);
    idx->idx_ad = desc;
    idx->idx_count[key] = 1;

    switch (avl_insert(&bdb->bi_idx, (caddr_t)idx, monitor_idx_cmp, monitor_idx_dup)) {
    case 0:
      break;
...
static int bdb_monitor_idx_apply(void *v_idx, void *v_valp) {
  ...
  ber_len_t count_len[BDB_MONITOR_IDX_TYPES], idx_len;
  int i, num = 0;
  ...
  bv.bv_len += idx->idx_ad->ad_cname.bv_len + num + idx_len;
...
static int bdb_monitor_idx_entry_add(struct bdb_info *bdb, Entry *e) {
  ...
  avl_apply(bdb->bi_idx, bdb_monitor_idx_apply, &vals, -1, AVL_INORDER);
