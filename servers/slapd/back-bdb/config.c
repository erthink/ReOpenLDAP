...
static ConfigTable bdbcfg[] = {
    ...
    {"cachesize", "size", 2, 2, 0, ARG_ULONG | ARG_OFFSET,
     ...
     "SYNTAX OMsInteger SINGLE-VALUE )",
     NULL, NULL},
    {"checkpoint", "kbyte> <min", 3, 3, 0, ARG_MAGIC | BDB_CHKPT, bdb_cf_gen,
    ...
    {"dbconfig", "DB_CONFIG setting", 1, 0, 0, ARG_MAGIC | BDB_CONFIG,
     ...
     "SYNTAX OMsIA5String X-ORDERED 'VALUES' )",
     NULL, NULL},
    {"dbnosync", NULL, 1, 2, 0, ARG_ON_OFF | ARG_MAGIC | BDB_NOSYNC, bdb_cf_gen,
     "( OLcfgDbAt:1.4 NAME 'olcDbNoSync' "
     "DESC 'Disable synchronous database writes' "
     "EQUALITY booleanMatch "
     "SYNTAX OMsBoolean SINGLE-VALUE )",
     NULL, NULL},
    {"dbpagesize", "db> <size", 3, 3, 0, ARG_MAGIC | BDB_PGSIZE, bdb_cf_gen,
    ...
    {"linearindex", NULL, 1, 2, 0, ARG_ON_OFF | ARG_OFFSET,
     ...
     "SYNTAX OMsBoolean SINGLE-VALUE )",
     NULL, NULL},
    {"lockdetect", "policy", 2, 2, 0, ARG_MAGIC | BDB_LOCKD, bdb_cf_gen,
     "( OLcfgDbAt:1.8 NAME 'olcDbLockDetect' "
     "DESC 'Deadlock detection algorithm' "
     "EQUALITY caseIgnoreMatch "
     "SYNTAX OMsDirectoryString SINGLE-VALUE )",
     NULL, NULL},
    {"mode", "mode", 2, 2, 0, ARG_MAGIC | BDB_MODE, bdb_cf_gen,
...
static void *bdb_online_index(void *ctx, void *arg) {
  struct re_s *rtask = arg;
...
static int bdb_cf_cleanup(ConfigArgs *c) {
...
static int bdb_cf_gen(ConfigArgs *c) {
  struct bdb_info *bdb = c->be->be_private;
  int rc;
  ...
  if (c->op == SLAP_CONFIG_EMIT) {
    ...
    case BDB_CONFIG:
      if (!(bdb->bi_flags & BDB_IS_OPEN) && !bdb->bi_db_config) {
  ...
  case BDB_NOSYNC:
    if (c->value_int)
      ...
      bdb->bi_dbenv_xflags &= ~DB_TXN_NOSYNC;
    if (bdb->bi_flags & BDB_IS_OPEN) {
      bdb->bi_dbenv->set_flags(bdb->bi_dbenv, DB_TXN_NOSYNC, c->value_int);
    }
    break;
