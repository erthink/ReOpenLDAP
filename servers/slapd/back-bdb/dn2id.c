 ...
 */
int hdb_dup_compare(DB *db, const DBT *usrkey, const DBT *curkey) {
  diskNode *un, *cn;
  int rc;
 ...
 */
int hdb_fix_dn(Entry *e, int checkit) {
  EntryInfo *ei;
  int rlen = 0, nrlen = 0;
  char *ptr, *nptr;
  int max = 0;
  ...
  if (ptr > e->e_name.bv_val)
    ptr[-1] = '\0';
  if (nptr > e->e_nname.bv_val)
    nptr[-1] = '\0';
 ...
 */
int hdb_dn2id_add(Operation *op, DB_TXN *txn, EntryInfo *eip, Entry *e) {
  struct bdb_info *bdb = (struct bdb_info *)op->o_bd->be_private;
  DB *db = bdb->bi_dn2id->bdi_db;
  DBT key, data;
  ID nid;
...
int hdb_dn2id_delete(Operation *op, DB_TXN *txn, EntryInfo *eip, Entry *e) {
  struct bdb_info *bdb = (struct bdb_info *)op->o_bd->be_private;
  DB *db = bdb->bi_dn2id->bdi_db;
  DBT key, data;
  DBC *cursor;
...
int hdb_dn2id_parent(Operation *op, DB_TXN *txn, EntryInfo *ei, ID *idp) {
  struct bdb_info *bdb = (struct bdb_info *)op->o_bd->be_private;
  DB *db = bdb->bi_dn2id->bdi_db;
  DBT key, data;
  DBC *cursor;
  ...
  if (rc == 0) {
    if (d->nrdnlen[0] & 0x80) {
      ...
      ei->bei_rdn.bv_len = data.size - sizeof(diskNode) - ei->bei_nrdn.bv_len;
...
int hdb_dn2id_children(Operation *op, DB_TXN *txn, Entry *e) {
  struct bdb_info *bdb = (struct bdb_info *)op->o_bd->be_private;
  DB *db = bdb->bi_dn2id->bdi_db;
  DBT key, data;
  DBC *cursor;
...
static int apply_func(void *data, void *arg) {
  EntryInfo *ei = data;
  ID *idl = arg;
...
static int hdb_dn2idl_internal(struct dn2id_cookie *cx) {
  BDB_IDL_ZERO(cx->tmp);
  ...
  if (cx->ei->bei_ckids + 1 != cx->ei->bei_dkids) {
    ...
    cx->rc = cx->db->cursor(cx->db, NULL, &cx->dbc, cx->bdb->bi_db_opflags);
    if (cx->rc)
      goto done_one;
  ...
  if (!BDB_IDL_IS_ZERO(cx->tmp)) {
    if (cx->prefix == DN_SUBTREE_PREFIX) {
      ...
      if (!(cx->ei->bei_state & CACHE_ENTRY_NO_GRANDKIDS)) {
        ...
        EntryInfo *ei = cx->ei;
        int nokids = 1;
        save = cx->op->o_tmpalloc(BDB_IDL_SIZEOF(cx->tmp), cx->op->o_tmpmemctx);
