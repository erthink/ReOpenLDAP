...
static int bdb_id2entry_put(BackendDB *be, DB_TXN *tid, Entry *e, int flag) {
...
int bdb_entry_release(Operation *op, Entry *e, int rw) {
...
return_results:
      ...
      *ent = e;
      ...
      if (op) {
        ...
        if (!boi->boi_txn) {
          ...
          bli = op->o_tmpalloc(sizeof(struct bdb_lock_info), op->o_tmpmemctx);
  ...
  Debug(LDAP_DEBUG_TRACE, "bdb_entry_get: rc=%d\n", rc);
  return (rc);
}
