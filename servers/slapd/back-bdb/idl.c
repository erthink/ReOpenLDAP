...
static int bdb_idl_entry_cmp(const void *v_idl1, const void *v_idl2) {
  ...
  if ((rc = SLAP_PTRCMP(idl1->db, idl2->db)))
    return rc;
  if ((rc = idl1->kstr.bv_len - idl2->kstr.bv_len))
    return rc;
...
static char *bdb_show_key(DBT *key, char *buf) {
...
void bdb_idl_cache_put(struct bdb_info *bdb, DB *db, DBT *key, ID *ids,
  ...
  ee = (bdb_idl_cache_entry_t *)ch_malloc(sizeof(bdb_idl_cache_entry_t));
  ee->db = db;
  ...
  if (bdb->bi_idl_cache_size >= bdb->bi_idl_cache_max_size) {
    ...
    assert(bdb->bi_idl_lru_tail != NULL || bdb->bi_idl_lru_head == NULL);
  }
  bdb->bi_idl_cache_size++;
...
#define BDB_ENOUGH 1
  ...
  Debug(LDAP_DEBUG_ARGS, "bdb_idl_fetch_key: %s\n", bdb_show_key(key, keybuf));
...
int bdb_idl_insert_key(BackendDB *be, DB *db, DB_TXN *tid, DBT *key, ID id) {
  struct bdb_info *bdb = (struct bdb_info *)be->be_private;
  int rc;
  DBT data;
  DBC *cursor;
...
int bdb_idl_delete_key(BackendDB *be, DB *db, DB_TXN *tid, DBT *key, ID id) {
  struct bdb_info *bdb = (struct bdb_info *)be->be_private;
  int rc;
  DBT data;
  DBC *cursor;
...
int bdb_idl_union(ID *a, ID *b) {
  ...
  over:
    ida = IDL_MIN(BDB_IDL_FIRST(a), BDB_IDL_FIRST(b));
