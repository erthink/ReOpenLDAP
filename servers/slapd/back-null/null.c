...
static ConfigOCs nullocs[] = {{"( OLcfgDbOc:8.1 "
                               "NAME 'olcNullConfig' "
...
static int null_back_db_open(BackendDB *be, ConfigReply *cr) {
  ...
  if (ni->ni_dosearch) {
    ...
    bv[1].bv_len = bv[0].bv_len - (bv[1].bv_val - bv[0].bv_val);
...
static int null_back_respond(Operation *op, SlapReply *rs, int rc) {
  ...
  LDAPControl **preread_ctrl = NULL, **postread_ctrl = NULL;
    ...
    case LDAP_REQ_DELETE:
      ...
      *preread_ctrl = NULL;
      ...
      if (slap_read_controls(op, rs, &e, &slap_pre_read_bv, preread_ctrl)) {
        ...
        Debug(LDAP_DEBUG_TRACE, "<=- null_back_respond: pre-read "
                                "failed!\n");
    ...
    case LDAP_REQ_RENAME:
      ...
      *postread_ctrl = NULL;
      ...
      if (slap_read_controls(op, rs, &e, &slap_post_read_bv, postread_ctrl)) {
        ...
        Debug(LDAP_DEBUG_TRACE, "<=- null_back_respond: post-read "
                                "failed!\n");
...
static int null_back_entry_release(Operation *op, Entry *e, int rw) {
...
static int null_tool_entry_open(BackendDB *be, int mode) { return 0; }
...
static Entry *null_tool_entry_get(BackendDB *be, ID id) {
