  ...
  default:
    ...
    if (rs->sr_ref) {
      ...
      if (!j) {
        ...
        new = ch_realloc(gs->refs, (j + i + 1) * sizeof(struct berval));
      }
      for (k = 0; k < i; j++, k++) {
    ...
    if (rs->sr_ctrls) {
      ...
      if (!j) {
        ...
        if (get_pagedresults(op) > SLAP_CONTROL_IGNORED) {
          ...
          if (newpage) {
            for (k = 0; k < j; k++) {
              if (!strcmp(gs->ctrls[k]->ldctl_oid, LDAP_CONTROL_PAGEDRESULTS)) {
...
static int glue_open(BackendInfo *bi) {
  ...
  if (slapMode & SLAP_TOOL_MODE) {
    for (i = 0; i < gi->gi_nodes; i++) {
      ...
      if (gi->gi_n[i].gn_be->bd_info->bi_open == on->on_info->oi_orig->bi_open)
        bsame = 1;
      ...
      if (!same && gi->gi_n[i].gn_be->bd_info->bi_open)
        rc = gi->gi_n[i].gn_be->bd_info->bi_open(gi->gi_n[i].gn_be->bd_info);
...
static ID glue_tool_entry_first_x(BackendDB *b0, struct berval *base, int scope, Filter *f) {
  ...
  rc = glueBack->be_entry_first_x(glueBack, glue_base, glue_scope, glue_filter);
  while (rc == NOID) {
    if (glueBack && glueBack->be_entry_close)
      glueBack->be_entry_close(glueBack);
    for (i = 0; i < gi->gi_nodes; i++) {
    ...
    if (i == 0) {
      ...
      glueBack = gi->gi_n[i - 1].gn_be;
      rc = glue_tool_entry_first_x(b0, glue_base, glue_scope, glue_filter);
      if (glueBack == GLUEBACK_DONE) {
        break;
      }
    }
