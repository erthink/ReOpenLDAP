...
int backsql_compare(Operation *op, SlapReply *rs) {
  ...
  for (a = attrs_find(e.e_attrs, op->oq_compare.rs_ava->aa_desc); a != NULL;
...
return_results:;
  ...
  default:
    if (!BER_BVISNULL(&e.e_nname) && !access_allowed(op, &e, slap_schema.si_ad_entry, NULL, ACL_DISCLOSE, NULL)) {
