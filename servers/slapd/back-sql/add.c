...
int backsql_modify_delete_all_values(Operation *op, SlapReply *rs, SQLHDBC dbh,
  ...
  if (rc != SQL_SUCCESS) {
    Debug(LDAP_DEBUG_TRACE,
          ...
          "\"%s\"\n",
          at->bam_query);
    backsql_PrintErrors(bi->sql_db_env, dbh, asth, rc);
  ...
  if (rc != SQL_SUCCESS) {
    Debug(LDAP_DEBUG_TRACE, "   backsql_modify_delete_all_values(): "
  ...
  for (rc = SQLFetch(asth); BACKSQL_SUCCESS(rc); rc = SQLFetch(asth)) {
    int i;
    /* first parameter no, parameter order */
    SQLUSMALLINT pno = 0, po = 0;
    ...
    for (i = 0; i < row.ncols; i++) {
      ...
      Debug(LDAP_DEBUG_TRACE,
            "   backsql_modify_delete_all_values() "
            "arg(%d)=" BACKSQL_IDFMT "\n",
            pno + 1 + po, BACKSQL_IDARG(e_id->eid_keyval));
      ...
      Debug(LDAP_DEBUG_TRACE,
            "   backsql_modify_delete_all_values(): "
            "arg(%d)=%s; executing \"%s\"\n",
            pno + 2 - po, row.cols[i], at->bam_delete_proc);
      rc = SQLExecute(sth);
...
int backsql_modify_internal(Operation *op, SlapReply *rs, SQLHDBC dbh,
  ...
  for (ml = modlist; ml != NULL; ml = ml->sml_next) {
    ...
    add_only:;
      ...
      for (i = 0, at_val = sm_values; !BER_BVISNULL(at_val); i++, at_val++) {
        SQLHSTMT sth = SQL_NULL_HSTMT;
        /* first parameter position, parameter order */
        SQLUSMALLINT pno = 0, po;
        /* procedure return code */
        int prc = LDAP_SUCCESS;
...
static int backsql_add_attr(Operation *op, SlapReply *rs, SQLHDBC dbh,
  ...
  for (i = 0, at_val = &at->a_vals[i]; !BER_BVISNULL(at_val);
    ...
    currpos = pno + 1 + po;
    rc = backsql_BindParamNumID(sth, currpos, SQL_PARAM_INPUT, &new_keyval);
    if (rc != SQL_SUCCESS) {
      Debug(LDAP_DEBUG_TRACE,
            "   backsql_add_attr(): "
    ...
    if (DebugTest(LDAP_DEBUG_TRACE)) {
      ...
      Debug(LDAP_DEBUG_TRACE,
            "   backsql_add_attr(\"%s\"): "
            "executing \"%s\" %s\n",
            op->ora_e->e_name.bv_val, at_rec->bam_add_proc, logbuf);
    }
...
int backsql_add(Operation *op, SlapReply *rs) {
  backsql_info *bi = (backsql_info *)op->o_bd->be_private;
  ...
  Entry p = {0}, *e = NULL;
  Attribute *at, *at_objectClass = NULL;
  ...
  if (oc == NULL) {
    Debug(LDAP_DEBUG_TRACE,
          ...
          op->ora_e->e_name.bv_val, scname.bv_val);
    rs->sr_err = LDAP_UNWILLING_TO_PERFORM;
  ...
  } else if (BACKSQL_CREATE_NEEDS_SELECT(bi) && oc->bom_create_keyval == NULL) {
    Debug(LDAP_DEBUG_TRACE,
          "   backsql_add(\"%s\"): "
  ...
  if (rs->sr_err != LDAP_SUCCESS) {
    Debug(LDAP_DEBUG_TRACE,
          ...
          op->ora_e->e_name.bv_val);
    rs->sr_text = (rs->sr_err == LDAP_OTHER) ? "SQL-backend error" : NULL;
    e = NULL;
    goto done;
  }
  ...
  if (BACKSQL_IS_ADD(oc->bom_expect_return)) {
    ...
    if (rc != SQL_SUCCESS) {
      Debug(LDAP_DEBUG_TRACE,
            ...
            "for objectClass %s\n",
            op->ora_e->e_name.bv_val, oc->bom_oc->soc_cname.bv_val);
      backsql_PrintErrors(bi->sql_db_env, dbh, sth, rc);
      SQLFreeStmt(sth, SQL_DROP);
  ...
  if (oc->bom_create_hint) {
    ...
    if (at && at->a_vals) {
      ...
      backsql_BindParamStr(sth, colnum, SQL_PARAM_INPUT, "", 0);
      Debug(LDAP_DEBUG_TRACE,
            "backsql_add(): "
  ...
  if (!BER_BVISNULL(&realdn) && realdn.bv_val != op->ora_e->e_name.bv_val) {
