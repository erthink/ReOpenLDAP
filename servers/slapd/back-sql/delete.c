...
static int backsql_delete_attr_f(void *v_at, void *v_bda) {
...
static int backsql_tree_delete_search_cb(Operation *op, SlapReply *rs) {
...
int backsql_delete(Operation *op, SlapReply *rs) {
  ...
  if (!access_allowed(op, &d, slap_schema.si_ad_entry, NULL, ACL_WDEL, NULL)) {
    Debug(LDAP_DEBUG_TRACE, "   backsql_delete(): "
