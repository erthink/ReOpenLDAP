...
static int create_baseObject(BackendDB *be, const char *fname, int lineno);

static int read_baseObject(BackendDB *be, const char *fname);
...
static ConfigTable sqlcfg[] = {
    ...
    {"baseobject", "[file]", 1, 2, 0, ARG_STRING | ARG_MAGIC | BSQL_BASE_OBJECT,
     ...
     "SYNTAX OMsDirectoryString SINGLE-VALUE )",
     NULL, NULL},
    {"sqllayer", "name", 2, 0, 0, ARG_MAGIC | BSQL_LAYER, (void *)sql_cf_gen,
...
#define WHATSLEFT ((ber_len_t)(&buf[sizeof(buf)] - ptr))
  ...
  case BSQL_ALIASING_KEYWORD:
    ...
    ber_str2bv(c->argv[1], strlen(c->argv[1]) + 1, 1, &bi->sql_aliasing);
...
static int read_baseObject(BackendDB *be, const char *fname) {
  ...
  if (bi->sql_baseObject == NULL) {
    Debug(LDAP_DEBUG_ANY, "read_baseObject_file: entry_alloc failed");
    ldif_close(fp);
...
static int create_baseObject(BackendDB *be, const char *fname, int lineno) {
  ...
  for (iAVA = 0; rdn[iAVA]; iAVA++) {
    ...
    attr_merge_normalize_one(bi->sql_baseObject, ad, &ava->la_value, NULL);
  }
