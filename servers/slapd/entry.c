...
Entry *str2entry(char *s, int *rc) { return str2entry2(s, 1, rc); }

#define bvcasematch(bv1, bv2) (ber_bvstrcasecmp(bv1, bv2) == 0)

Entry *str2entry2(char *s, int checkvals, int *rc) {
  int dummy;
  if (!rc)
    rc = &dummy;
        ...
        *rc = slap_bv2ad(type + i, &ad, &text);
        ...
        if (slap_syntax_is_binary(ad->ad_type->sat_syntax) &&
          ...
          Debug(LDAP_DEBUG_ANY,
                ...
                ad->ad_cname.bv_val, 0, ad->ad_type->sat_syntax->ssyn_oid);
          *rc = LDAP_INVALID_SYNTAX;
          goto fail;
        }
      }
        ...
        if (atail->a_desc->ad_type->sat_flags & SLAP_AT_SORTED_VAL) {
          *rc = slap_sort_vals((Modifications *)atail, &text, &j, NULL);
          if (*rc == LDAP_SUCCESS) {
            ...
            Debug(LDAP_DEBUG_ANY,
                  "str2entry: attributeType %s value #%d provided more than "
                  "once\n",
...
#define entry_lenlen(l)                                                        \
  (((l) < 0x80)                                                                \
       ? 1                                                                     \
       : ((l) < 0x100) ? 2 : ((l) < 0x10000) ? 3 : ((l) < 0x1000000) ? 4 : 5)
...
int entry_encode(Entry *e, struct berval *bv) {
  ...
  *ptr++ = '\0';
  ...
  for (a = e->e_attrs; a; a = a->a_next) {
    ...
    memcpy(ptr, a->a_desc->ad_cname.bv_val, a->a_desc->ad_cname.bv_len);
  ...
  while ((i = entry_getlen(&ptr))) {
    ...
    if (a->a_desc->ad_type->sat_flags & SLAP_AT_SORTED_VAL) {
      ...
      if (rc == LDAP_SUCCESS) {
        ...
        /* should never happen */
        Debug(LDAP_DEBUG_ANY,
              "entry_decode: attributeType %s value #%d provided more than "
              "once\n",
