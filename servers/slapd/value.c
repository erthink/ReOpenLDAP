...
int value_add_one(BerVarray *vals, const struct berval *addval) {
  int n;
  BerVarray v2;
...
int value_add_one_str(BerVarray *vals, const char *str) {
  struct berval one;
...
int value_add_one_int(BerVarray *vals, int x) {
  char buf[16];
  struct berval v;
...
int value_join_str(BerVarray vals, const char *comma, BerValue *dest) {
  size_t len = 0;
  int i;
