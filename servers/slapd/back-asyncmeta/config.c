...
static ConfigTable a_metacfg[] = {
    ...
    {"", "", 0, 0, 0, ARG_IGNORED, NULL,
     "( OLcfgDbAt:3.116 NAME 'olcAsyncMetaSub' "
...
static int asyncmeta_rwi_init(struct rewrite_info **rwm_rw) {
  char *rargv[3];
...
static int asyncmeta_back_new_target(a_metatarget_t **mtp) {
  a_metatarget_t *mt;
...
int asyncmeta_subtree_free(a_metasubtree_t *ms) {
  switch (ms->ms_type) {
...
int asyncmeta_subtree_destroy(a_metasubtree_t *ms) {
  if (ms->ms_next) {
...
static void asyncmeta_filter_free(metafilter_t *mf) {
  regfree(&mf->mf_regex);
  ber_memfree(mf->mf_regex_pattern.bv_val);
  ch_free(mf);
}

void asyncmeta_filter_destroy(metafilter_t *mf) {
  if (mf->mf_next)
...
static int asyncmeta_subtree_config(a_metatarget_t *mt, ConfigArgs *c) {
...
static int asyncmeta_back_cf_gen(ConfigArgs *c) {
  a_metainfo_t *mi = (a_metainfo_t *)c->be->be_private;
  ...
  case LDAP_BACK_CFG_REWRITE: {
    ...
    if (ix < cnt) {
      int i;
      for (i = /* LY: guess this */ ix; i < cnt; i++) {
        ca.line = mt->mt_rwmap.rwm_bva_rewrite[i].bv_val;
        ca.argc = 0;
        config_fp_parse_line(&ca);
  ...
  case LDAP_BACK_CFG_MAP: {
    ...
    /* add the new mapping */
    rc = asyncmeta_map_config(c, &mt->mt_rwmap.rwm_oc, &mt->mt_rwmap.rwm_at);
    if (rc) {
      goto map_fail;
    }
  ...
  if (is_oc) {
    if (src[0] != '\0') {
      if (oc_bvfind(&mapping[0].src) == NULL) {
        Debug(LDAP_DEBUG_ANY,
              "%s: warning, source objectClass '%s' should be defined in "
              "schema\n",
              c->log, src);
    ...
    if (oc_bvfind(&mapping[0].dst) == NULL) {
      Debug(LDAP_DEBUG_ANY,
            "%s: warning, destination objectClass '%s' is not defined in "
            "schema\n",
            c->log, dst);
    }
  } else {
    ...
    if (src[0] != '\0') {
      ...
      if (rc != LDAP_SUCCESS) {
        Debug(LDAP_DEBUG_ANY,
              "%s: warning, source attributeType '%s' should be defined in "
              "schema\n",
              c->log, src);
    ...
    if (rc != LDAP_SUCCESS) {
      Debug(LDAP_DEBUG_ANY,
            "%s: warning, destination attributeType '%s' is not defined in "
            "schema\n",
            c->log, dst);
