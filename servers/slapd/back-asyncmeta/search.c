...
static void asyncmeta_handle_onerr_stop(Operation *op, SlapReply *rs, a_metaconn_t *mc, bm_context_t *bc, int candidate,
                                        slap_callback *cb) {
...
meta_search_candidate_t asyncmeta_back_search_start(Operation *op, SlapReply *rs, a_metaconn_t *mc, bm_context_t *bc,
  ...
  if (msc->msc_ld == NULL) {
    Debug(LDAP_DEBUG_ANY, "%s: asyncmeta_back_search_start candidate=%d ld=NULL%s.\n", op->o_log_prefix, candidate,
   ...
   * Maps filter
   */
  rc = asyncmeta_filter_map_rewrite(&dc, op->ors_filter, &mfilter, BACKLDAP_MAP, NULL);
  switch (rc) {
  case LDAP_SUCCESS:
    break;
   ...
   */
  rc = asyncmeta_map_attrs(op, &mt->mt_rwmap.rwm_at, op->ors_attrs, BACKLDAP_MAP, &mapped_attrs);
  if (rc != LDAP_SUCCESS) {
    /*
     * this target is no longer candidate
     */
...
retry:;
   ...
   * Starts the search
   */
  ber = ldap_build_search_req(msc->msc_ld, mbase.bv_val, realscope, mfilter.bv_val, mapped_attrs, op->ors_attrsonly,
                              ctrls, NULL, timelimit, op->ors_slimit, op->ors_deref, &msgid);
  if (ber) {
    candidates[candidate].sr_msgid = msgid;
    rc = ldap_send_initial_request(msc->msc_ld, LDAP_REQ_SEARCH, mbase.bv_val, ber, msgid);
    if (rc == msgid)
      rc = LDAP_SUCCESS;
    else
      rc = LDAP_SERVER_DOWN;
...
int asyncmeta_back_search(Operation *op, SlapReply *rs) {
  a_metainfo_t *mi = (a_metainfo_t *)op->o_bd->be_private;
  time_t stoptime = (time_t)(-1), timeout = 0;
  int rc = 0;
  ...
  for (i = 0; i < mi->mi_ntargets; i++) {
    ...
    case META_SEARCH_CANDIDATE:
      ...
      Debug(LDAP_DEBUG_TRACE,
            "%s asyncmeta_back_search: IS_CANDIDATE "
            "cnd=\"%ld\"\n",
            op->o_log_prefix, i);
    ...
    case META_SEARCH_NOT_CANDIDATE:
      Debug(LDAP_DEBUG_TRACE,
            "%s asyncmeta_back_search: NOT_CANDIDATE "
            "cnd=\"%ld\"\n",
            op->o_log_prefix, i);
      candidates[i].sr_msgid = META_MSGID_IGNORE;
      break;
    ...
    case META_SEARCH_CONNECTING:
      Debug(LDAP_DEBUG_TRACE,
            "%s asyncmeta_back_search: NEED_BIND "
            "cnd=\"%ld\" %p\n",
    ...
    case META_SEARCH_BINDING:
      Debug(LDAP_DEBUG_TRACE,
            "%s asyncmeta_back_search: BINDING "
            "cnd=\"%ld\" %p\n",
            op->o_log_prefix, i, &mc->mc_conns[i]);
      ncandidates++;
    ...
    case META_SEARCH_ERR:
      Debug(LDAP_DEBUG_TRACE,
            "%s asyncmeta_back_search: SEARCH_ERR "
            "cnd=\"%ldd\"\n",
            op->o_log_prefix, i);
      candidates[i].sr_msgid = META_MSGID_IGNORE;
  ...
  if (DebugTest(LDAP_DEBUG_TRACE)) {
    ...
    Debug(LDAP_DEBUG_TRACE,
          "%s asyncmeta_back_search: ncandidates=%d "
          "cnd=\"%s\"\n",
