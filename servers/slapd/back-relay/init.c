...
static ConfigTable relaycfg[] = {{"relay", "relay", 2, 2, 0, ARG_MAGIC | ARG_DN | ARG_QUOTE, relay_back_cf,
                                  "( OLcfgDbAt:5.1 "
                                  "NAME 'olcRelay' "
                                  "DESC 'Relay DN' "
                                  "SYNTAX OMsDN "
...
static ConfigOCs relayocs[] = {{"( OLcfgDbOc:5.1 "
                                "NAME 'olcRelayConfig' "
  ...
  if (c->op == SLAP_CONFIG_EMIT) {
    ...
    if (bd == NULL) {
      snprintf(c->cr_msg, sizeof(c->cr_msg),
               ...
               "in \"olcRelay <dn>\"\n",
               c->value_dn.bv_val);
      Log(LDAP_DEBUG_CONFIG, LDAP_LEVEL_ERR, "%s: %s.\n", c->log, c->cr_msg);
...
int relay_back_db_open(Backend *be, ConfigReply *cr) {
  ...
  if (!BER_BVISNULL(&ri->ri_realsuffix)) {
    ...
    if (ri->ri_bd == NULL) {
      ...
      Log(LDAP_DEBUG_ANY, LDAP_LEVEL_ERR, "relay_back_db_open: %s.\n", cr->msg);

      return 1;
    }

    /* inherit controls */
    memcpy(be->bd_self->be_ctrls, ri->ri_bd->be_ctrls, sizeof(be->be_ctrls));

  } else {
    /* inherit all? */
    memcpy(be->bd_self->be_ctrls, frontendDB->be_ctrls, sizeof(be->be_ctrls));
  }
