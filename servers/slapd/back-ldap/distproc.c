...
static ReferenceType_t ldap_distproc_str2rt(const char *s) {
  struct berval bv;
...
static SearchScope_t ldap_distproc_str2ss(const char *s) {
  struct berval bv;
...
static int ldap_distproc_uri_cmp(const void *c1, const void *c2) {
...
static int ldap_distproc_uri_dup(void *c1, void *c2) {
...
static int ldap_distproc_operational(Operation *op, SlapReply *rs) {
...
static int ldap_distproc_cfadd_apply(void *datum, void *arg) {
...
static int distproc_cfgen(ConfigArgs *c) {
...
static int ldap_distproc_db_init(BackendDB *be, ConfigReply *cr) {
...
static int ldap_distproc_db_apply(void *datum, void *arg) {
  ldapinfo_t *li = (ldapinfo_t *)datum;
...
static int ldap_distproc_db_func(BackendDB *be, enum db_which which) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  ldap_distproc_t *lc = (ldap_distproc_t *)on->on_bi.bi_private;
...
static int ldap_distproc_conn_apply(void *datum, void *arg) {
  ldapinfo_t *li = (ldapinfo_t *)datum;
...
static int ldap_distproc_connection_destroy(BackendDB *be, Connection *conn) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  ldap_distproc_t *lc = (ldap_distproc_t *)on->on_bi.bi_private;
...
static int ldap_distproc_parse_returnContRef_ctrl(Operation *op, SlapReply *rs,
  ...
  if (op->o_pagedresults != SLAP_CONTROL_NONE) {
    rs->sr_text = "returnContinuationReference control specified with "
