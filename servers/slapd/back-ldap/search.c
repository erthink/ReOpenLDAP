 ...
 */
static int ldap_back_munge_filter(Operation *op, struct berval *filter) {
  char *ptr;
  int gotit = 0;
  ...
  for (ptr = strchr(filter->bv_val, '('); ptr; ptr = strchr(ptr, '(')) {
    ...
    if (filter->bv_val == op->ors_filterstr.bv_val) {
      filter->bv_val = op->o_tmpalloc(filter->bv_len + 1, op->o_tmpmemctx);
...
int ldap_back_search(Operation *op, SlapReply *rs) {
  ...
  LDAPMessage *res, *e;
  int rc = 0, msgid;
  struct berval match = BER_BVNULL, filter = BER_BVNULL;
  int i, x;
  char **attrs = NULL;
  ...
  int remove_unknown_schema = LDAP_BACK_OMIT_UNKNOWN_SCHEMA(li);
  ...
  if (i > 0 || x > 0) {
    ...
    attrs = op->o_tmpalloc((i + x + 1) * sizeof(char *), op->o_tmpmemctx);
    if (attrs == NULL) {
...
retry:
  ...
  for (rc = -2; rc != -1;
    ...
    if (rc == LDAP_RES_SEARCH_ENTRY) {
      ...
      do_retry = 0;
      rc = ldap_parse_reference(lc->lc_ld, res, &references, &rs->sr_ctrls, 1);
