...
int ldap_back_extended(Operation *op, SlapReply *rs) {
  int i;
...
static int ldap_back_exop_passwd(Operation *op, SlapReply *rs,
  ...
  int do_retry = 1;
  char *text = NULL;
  struct berval dn = op->o_req_dn, ndn = op->o_req_ndn;
  ...
  if (BER_BVISNULL(&ndn) && op->ore_reqdata != NULL) {
    ...
    if (!BER_BVISEMPTY(&tmpid)) {
      ...
      rs->sr_err = dnPrettyNormal(NULL, &tmpid, &dn, &ndn, op->o_tmpmemctx);
    ...
    if (ldap_result(lc->lc_ld, msgid, LDAP_MSG_ALL, &tv, &res) == -1) {
      ...
      if (rc == LDAP_SUCCESS) {
        if (rs->sr_err == LDAP_SUCCESS) {
           ...
           */
          rc = ldap_parse_passwd(lc->lc_ld, res, &newpw);
          if (rc == LDAP_SUCCESS && !BER_BVISNULL(&newpw)) {
