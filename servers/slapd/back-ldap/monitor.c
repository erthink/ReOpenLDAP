...
struct ldap_back_monitor_ops_t {
  struct berval rdn;
} ldap_back_monitor_op[] = {{BER_BVC("cn=Bind")},
                            {BER_BVC("cn=Unbind")},
                            {BER_BVC("cn=Search")},
                            {BER_BVC("cn=Compare")},
                            {BER_BVC("cn=Modify")},
...
static struct {
  unsigned flag;
  struct berval name;
} s_flag[] = {{LDAP_BACK_FCONN_ISBOUND, BER_BVC("bound")},
...
static struct {
  char *name;
  char *oid;
} s_oid[] = {{"olmLDAPAttributes", "olmDatabaseAttributes:2"},
...
static struct {
  char *name;
  ObjectClass **oc;
} s_moc[] = {{"monitorContainer", &oc_monitorContainer},
...
static int ldap_back_monitor_modify(Operation *op, SlapReply *rs, Entry *e,
  ...
  Modifications *ml, *ml_olmDbURIList = NULL;
...
static int ldap_back_monitor_conn_peername(LDAP *ld, struct berval *bv) {
  Sockbuf *sockbuf;
  ber_socket_t socket;
  Sockaddr sa;
  ...
  case AF_INET: {
#if defined(HAVE_GETADDRINFO) && defined(HAVE_INET_NTOP)
    peeraddr = inet_ntop(AF_INET, &sa.sa_in_addr.sin_addr, addr, sizeof(addr));
#else  /* ! HAVE_GETADDRINFO || ! HAVE_INET_NTOP */
...
static int ldap_back_monitor_conn_init(BackendDB *be, monitor_subsys_t *ms) {
  ...
  if (e == NULL) {
    Debug(LDAP_DEBUG_ANY,
          ...
          li->li_monitor_info.lmi_conn_rdn.bv_val, ms->mss_ndn.bv_val);
    return (-1);
  }
...
static void ldap_back_monitor_ops_dispose(void **priv) {
...
static int ldap_back_monitor_ops_free(Entry *e, void **priv) {
...
static int ldap_back_monitor_ops_init(BackendDB *be, monitor_subsys_t *ms) {
  ldapinfo_t *li = (ldapinfo_t *)ms->mss_private;
  ...
  if (parent == NULL) {
    Debug(LDAP_DEBUG_ANY,
          ...
          "unable to create entry \"%s,%s\"\n",
          li->li_monitor_info.lmi_ops_rdn.bv_val, ms->mss_ndn.bv_val);
    return (-1);
  }
  ...
  for (op = 0; op < SLAP_OP_LAST; op++) {
    ...
    if (e == NULL) {
      Debug(LDAP_DEBUG_ANY,
            "ldap_back_monitor_ops_init: "
            "unable to create entry \"%s,%s\"\n",
            ldap_back_monitor_op[op].rdn.bv_val, parent->e_nname.bv_val);
      return (-1);
    }
...
static int ldap_back_monitor_initialize(void) {
  ...
  for (i = 0; s_at[i].desc != NULL; i++) {
    ...
    if (code != LDAP_SUCCESS) {
      Debug(LDAP_DEBUG_ANY,
            "ldap_back_monitor_initialize: register_at failed for "
            "attributeType (%s)\n",
            s_at[i].desc);
      return 3;
  ...
  for (i = 0; s_oc[i].desc != NULL; i++) {
    ...
    if (code != LDAP_SUCCESS) {
      Debug(LDAP_DEBUG_ANY,
            "ldap_back_monitor_initialize: register_oc failed for objectClass "
            "(%s)\n",
            s_oc[i].desc);
      return 4;
...
int ldap_back_monitor_db_open(BackendDB *be) {
  ...
  if (BER_BVISNULL(&li->li_monitor_info.lmi_conn_rdn)) {
    ber_str2bv("cn=Connections", 0, 1, &li->li_monitor_info.lmi_conn_rdn);
  }
  if (BER_BVISNULL(&li->li_monitor_info.lmi_ops_rdn)) {
    ber_str2bv("cn=Operations", 0, 1, &li->li_monitor_info.lmi_ops_rdn);
  }
...
int ldap_back_monitor_db_close(BackendDB *be) {
  ...
  if (li && !BER_BVISNULL(&li->li_monitor_info.lmi_ndn)) {
    ...
    if (mss) {
       ...
       * Workaround for todo4recovery://erased_by_github/erthink/ReOpenLDAP/issues/15 */
