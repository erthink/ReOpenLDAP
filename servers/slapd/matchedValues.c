...
int filter_matched_values(Operation *op, Attribute *a, char ***e_flags) {
  ...
  for (vrf = op->o_vrFilter; vrf != NULL; vrf = vrf->vrf_next) {
    ...
    case LDAP_FILTER_GE:
      rc = test_ava_vrFilter(op, a, vrf->vrf_ava, LDAP_FILTER_GE, e_flags);
      if (rc == -1)
        return rc;
      break;

    case LDAP_FILTER_LE:
      rc = test_ava_vrFilter(op, a, vrf->vrf_ava, LDAP_FILTER_LE, e_flags);
      if (rc == -1)
        return rc;
      break;
...
static int test_ava_vrFilter(Operation *op, Attribute *a,
  ...
  for (i = 0; a != NULL; a = a->a_next, i++) {
    ...
    for (j = 0; !BER_BVISNULL(bv); bv++, j++) {
      ...
      rc = value_match(&match, a->a_desc, mr, 0, bv, &ava->aa_value, &text);
      if (rc != LDAP_SUCCESS)
        return rc;
