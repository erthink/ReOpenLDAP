...
int shell_back_compare(Operation *op, SlapReply *rs) {
  ...
  if (si->si_compare == NULL) {
    send_ldap_error(op, rs, LDAP_UNWILLING_TO_PERFORM, "compare not implemented");
    return (-1);
  }
  ...
  if (!access_allowed(op, &e, entry, NULL, ACL_READ, NULL)) {
