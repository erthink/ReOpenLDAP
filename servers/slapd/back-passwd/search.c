...
static int pw2entry(Backend *be, struct passwd *pw, Entry *ep);

int passwd_back_search(Operation *op, SlapReply *rs) {
  struct passwd *pw;
  ...
  if (be_issuffix(op->o_bd, &op->o_req_ndn)) {
    ...
    if (op->ors_scope != LDAP_SCOPE_ONELEVEL) {
       ...
       */
      if (ldap_bv2rdn(&op->o_req_dn, &rdn, &next, LDAP_DN_FORMAT_LDAP)) {
...
static int pw2entry(Backend *be, struct passwd *pw, Entry *e) {
  size_t pwlen;
  struct berval val;
  struct berval bv;
  ...
  sprintf(val.bv_val, "uid=%s,%s", pw->pw_name, be->be_suffix[0].bv_val);
