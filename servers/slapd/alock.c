...
static int alock_grab_lock(int fd, int slot) {
  int res;
...
static int alock_release_lock(int fd, int slot) {
  int res;
...
static int alock_share_lock(int fd, int slot) {
  int res;
...
static int alock_test_lock(int fd, int slot) {
  int res;
...
static unsigned long int alock_read_iattr(unsigned char *bufptr) {
...
static void alock_write_iattr(unsigned char *bufptr, unsigned long int val) {
  int count;
...
static int alock_read_slot(alock_info_t *info, alock_slot_t *slot_data) {
  ...
  res = lseek(info->al_fd, (off_t)(ALOCK_SLOT_SIZE * info->al_slot), SEEK_SET);
  if (res == -1)
    return -1;

  size_total = 0;
  while (size_total < ALOCK_SLOT_SIZE) {
    size = read(info->al_fd, slotbuf + size_total, ALOCK_SLOT_SIZE - size_total);
    if (size == 0)
      return -1;
    if (size < 0) {
      err = errno;
...
static int alock_write_slot(alock_info_t *info, alock_slot_t *slot_data) {
  unsigned char slotbuf[ALOCK_SLOT_SIZE];
  int res, size, size_total, err;
  ...
  size_total = 0;
  while (size_total < ALOCK_SLOT_SIZE) {
    size = write(info->al_fd, slotbuf + size_total, ALOCK_SLOT_SIZE - size_total);
    if (size == 0)
      return -1;
    if (size < 0) {
      err = errno;
...
static int alock_query_slot(alock_info_t *info) {
  int res, nosave;
...
int alock_open(alock_info_t *info, const char *appname, const char *envdir, int locktype) {
  struct stat statbuf;
  ...
  for (scan_info.al_slot = 1; scan_info.al_slot < max_slot; ++scan_info.al_slot) {
    if (scan_info.al_slot != info->al_slot) {
      ...
      if (res == ALOCK_UNLOCKED && info->al_slot == 0) {
        info->al_slot = scan_info.al_slot;

      } else if (res == ALOCK_LOCKED) {
        ++live_count;

      } else if (res == ALOCK_UNIQUE && ((locktype & ALOCK_SMASK) == ALOCK_UNIQUE || nosave)) {
        close(info->al_fd);
        ber_memfree(slot_data.al_appname);
        return ALOCK_BUSY;
  ...
  if (info->al_slot == 0)
    info->al_slot = max_slot + 1;
  res = alock_grab_lock(info->al_fd, info->al_slot);
  if (res == -1) {
    close(info->al_fd);
    ber_memfree(slot_data.al_appname);
    return ALOCK_UNSTABLE;
...
int alock_scan(alock_info_t *info) {
  struct stat statbuf;
  alock_info_t scan_info;
  int res, max_slot;
  int dirty_count, live_count, nosave;
...
int alock_close(alock_info_t *info, int nosave) {
  alock_slot_t slot_data;
  int res;
...
int alock_recover(alock_info_t *info) {
  struct stat statbuf;
  alock_slot_t slot_data;
  alock_info_t scan_info;
  int res, max_slot;
  ...
  for (scan_info.al_slot = 1; scan_info.al_slot < max_slot; ++scan_info.al_slot) {
    if (scan_info.al_slot != info->al_slot) {
      ...
      if (res == ALOCK_LOCKED || res == ALOCK_UNIQUE) {
