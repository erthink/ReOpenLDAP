...
int rwm_map_config(struct ldapmap *oc_map, struct ldapmap *at_map, const char *fname, int lineno, int argc,
  ...
  if (argc < 3 || argc > 4) {
    Debug(LDAP_DEBUG_ANY,
          "%s: line %d: syntax is \"map {objectclass | attribute} [<local> | "
          "*] {<foreign> | *}\"\n",
          fname, lineno);
    return 1;
  }
  ...
  if (mapping == NULL) {
    Debug(LDAP_DEBUG_ANY, "%s: line %d: out of memory\n", fname, lineno);
    return 1;
  }
  ...
  if (is_oc) {
    ...
    if (src[0] != '\0') {
      rc = slap_bv2ad(&mapping[0].m_src, &mapping[0].m_src_ad, &text);
      if (rc != LDAP_SUCCESS) {
        Debug(LDAP_DEBUG_ANY,
...
static char *rwm_suffix_massage_regexize(const char *s) {
  char *res, *ptr;
  const char *p, *r;
  int i;
...
static char *rwm_suffix_massage_patternize(const char *s, const char *p) {
  ber_len_t len;
  char *res, *ptr;
