...
static int retcode_cleanup_cb(Operation *op, SlapReply *rs) {
  rs_send_cleanup(rs);
...
static int retcode_send_onelevel(Operation *op, SlapReply *rs) {
...
static int retcode_cb_response(Operation *op, SlapReply *rs) {
...
static int retcode_op_internal(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
...
static int retcode_op_func(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
  retcode_t *rd = (retcode_t *)on->on_bi.bi_private;
...
static int retcode_op2str(ber_tag_t op, struct berval *bv) {
  switch (op) {
  case LDAP_REQ_BIND:
    BER_BVSTR(bv, "bind");
    return 0;
...
static int retcode_entry_response(Operation *op, SlapReply *rs, BackendInfo *bi, Entry *e) {
  ...
  if (rs->sr_err != LDAP_SUCCESS && !LDAP_API_ERROR(rs->sr_err)) {
    BackendDB db = *op->o_bd, *o_bd = op->o_bd;
    ...
    if (rs->sr_err == LDAP_REFERRAL) {
      ...
      if (a != NULL) {
        struct berval oid = BER_BVNULL, data = BER_BVNULL;
        ber_int_t msgid = op->o_msgid;
...
static int retcode_response(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
  retcode_t *rd = (retcode_t *)on->on_bi.bi_private;
...
static int retcode_db_init(BackendDB *be, ConfigReply *cr) {
...
static void retcode_item_destroy(retcode_item_t *rdi) {
...
enum { RC_PARENT = 1, RC_ITEM };

static ConfigDriver rc_cf_gen;

static ConfigTable rccfg[] = {
    {"retcode-parent", "dn", 2, 2, 0, ARG_MAGIC | ARG_DN | RC_PARENT, rc_cf_gen,
...
static ConfigOCs rcocs[] = {{"( OLcfgOvOc:20.1 "
...
static int rc_cf_gen(ConfigArgs *c) {
  ...
  if (c->op == SLAP_CONFIG_EMIT) {
    ...
    case RC_PARENT:
      if (!BER_BVISEMPTY(&rd->rd_pdn)) {
        ...
        if (rc == 0) {
          rc = value_add_one(&c->rvalue_nvals, &rd->rd_npdn);
        }
        return rc;
      }
      rc = 0;
    ...
    if (c->argc > 3) {
      for (i = 3; i < c->argc; i++) {
        if (strncasecmp(c->argv[i], "op=", STRLENOF("op=")) == 0) {
          ...
          if (!BER_BVISNULL(&rdi.rdi_text)) {
            snprintf(c->cr_msg, sizeof(c->cr_msg), "\"text\" already provided");
            Debug(LDAP_DEBUG_CONFIG, "%s: retcode: %s\n", c->log, c->cr_msg);
            return ARG_BAD_CONF;
          }
          ...
          if (rdi.rdi_ref != NULL) {
            snprintf(c->cr_msg, sizeof(c->cr_msg), "\"ref\" already provided");
            Debug(LDAP_DEBUG_CONFIG, "%s: retcode: %s\n", c->log, c->cr_msg);
            return ARG_BAD_CONF;
          }
          ...
          if (strcasecmp(arg, "disconnect") == 0) {
            ...
            snprintf(c->cr_msg, sizeof(c->cr_msg), "unknown flag \"%s\"", arg);
            Debug(LDAP_DEBUG_CONFIG, "%s: retcode: %s\n", c->log, c->cr_msg);
            return ARG_BAD_CONF;
          }
...
static int retcode_db_open(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  retcode_t *rd = (retcode_t *)on->on_bi.bi_private;
  ...
  for (rdi = rd->rd_item; rdi; rdi = rdi->rdi_next) {
    ...
    /* RDN avas */
    rc = ldap_bv2rdn(&rdi->rdi_dn, &rdn, (char **)&p, LDAP_DN_FORMAT_LDAP);
    ...
    for (j = 0; rdn[j]; j++) {
      ...
      attr_merge_normalize_one(&rdi->rdi_e, ad, &ava->la_value, NULL);
    }
...
static int retcode_db_destroy(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  retcode_t *rd = (retcode_t *)on->on_bi.bi_private;
...
int retcode_over_initialize(void) {
  int i, code;

  static struct {
    char *desc;
    AttributeDescription **ad;
  } retcode_at[] = {{"( 1.3.6.1.4.1.4203.666.11.4.1.1 "
                     "NAME ( 'errCode' ) "
  ...
  static struct {
    char *desc;
    ObjectClass **oc;
  } retcode_oc[] = {{"( 1.3.6.1.4.1.4203.666.11.4.3.0 "
                     "NAME ( 'errAbsObject' ) "
                     "SUP top ABSTRACT "
                     "MUST ( errCode ) "
                     "MAY ( "
  ...
  for (i = 0; retcode_at[i].desc != NULL; i++) {
    ...
    if (code) {
      Debug(LDAP_DEBUG_ANY, "retcode: register_at failed\n");
      return code;
    }
  ...
  for (i = 0; retcode_oc[i].desc != NULL; i++) {
    ...
    if (code) {
      Debug(LDAP_DEBUG_ANY, "retcode: register_oc failed\n");
      return code;
    }
