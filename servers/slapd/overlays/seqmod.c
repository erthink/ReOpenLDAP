...
static int sm_avl_cmp(const void *c1, const void *c2) {
...
static int seqmod_op_cleanup(Operation *op, SlapReply *rs) {
...
static int seqmod_op_mod(Operation *op, SlapReply *rs) {
...
static int seqmod_op_extended(Operation *op, SlapReply *rs) {
  if (exop_is_write(op))
    return seqmod_op_mod(op, rs);
  else
    return SLAP_CB_CONTINUE;
}

static int seqmod_db_open(BackendDB *be, ConfigReply *cr) {
...
static int seqmod_db_close(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
