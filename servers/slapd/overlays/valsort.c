...
static int valsort_cf_func(ConfigArgs *c) {
  ...
  is_numeric = (vitmp.vi_ad->ad_type->sat_syntax == syn_numericString ||
                vitmp.vi_ad->ad_type->sat_syntax == slap_schema.si_syn_integer)
                   ? 1
                   : 0;
...
static void do_sort(Operation *op, Attribute *a, int beg, int num,
  ...
  if (sort & VALSORT_NUMERIC) {
    long *numbers = op->o_tmpalloc(num * sizeof(long), op->o_tmpmemctx), idx;
    for (i = 0; i < num; i++)
...
static int valsort_response(Operation *op, SlapReply *rs) {
  slap_overinst *on;
  valsort_info *vi;
  Attribute *a;
   ...
   */
  if (rs->sr_type != REP_SEARCH || (_SCM(op->o_sync) > SLAP_CONTROL_IGNORED) ||
      ...
      for (i = 0; i < n; i++) {
        ...
        if (!ptr) {
          Debug(LDAP_DEBUG_TRACE,
                ...
                "in entry %s\n",
                vi->vi_ad->ad_cname.bv_val, rs->sr_entry->e_name.bv_val);
          break;
        }
        char *end = NULL;
...
static int valsort_add(Operation *op, SlapReply *rs) {
...
static int valsort_modify(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
  valsort_info *vi = on->on_bi.bi_private;
...
static int valsort_db_open(BackendDB *be, ConfigReply *cr) {
  return overlay_register_control(be, LDAP_CONTROL_VALSORT);
}

static int valsort_destroy(BackendDB *be, ConfigReply *cr) {
...
static int valsort_parseCtrl(Operation *op, SlapReply *rs, LDAPControl *ctrl) {
  ber_tag_t tag;
