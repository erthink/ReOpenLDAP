...
static int deref_parseCtrl(Operation *op, SlapReply *rs, LDAPControl *ctrl) {
  ber_tag_t tag;
...
static int deref_cleanup(Operation *op, SlapReply *rs) {
...
static int deref_response(Operation *op, SlapReply *rs) {
...
static int deref_op_search(Operation *op, SlapReply *rs) {
  if (op->o_deref) {
    slap_callback *sc;
    deref_cb_t *dc;
...
static int deref_db_init(BackendDB *be, ConfigReply *cr) {
  if (ov_count == 0) {
    int rc;
...
int deref_over_initialize(void) {
