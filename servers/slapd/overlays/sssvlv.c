 ...
 */
static struct berval *select_value(Attribute *attr, sort_key *key) {
...
static int find_session_by_so(int svi_max_percon, int conn_id, sort_op *so) {
  assert(so != NULL);
  if (so == NULL)
    return -1;
...
static int find_next_session(int svi_max_percon, int conn_id) {
  int sess_id;
...
static void send_list(Operation *op, SlapReply *rs, sort_op *so) {
...
static void send_result(Operation *op, SlapReply *rs, sort_op *so) {
  LDAPControl *ctrls[3];
  int rc, i = 0;
...
static int sssvlv_op_response(Operation *op, SlapReply *rs) {
  ...
  if (rs->sr_type == REP_SEARCH) {
    ...
    for (i = 0; i < sc->sc_nkeys; i++) {
      Attribute *a = attr_find(rs->sr_entry->e_attrs, sc->sc_keys[i].sk_ad);
      if (a) {
        if (a->a_numvals > 1) {
    ...
    memcpy(sn2->sn_vals, sn->sn_vals, sc->sc_nkeys * sizeof(struct berval));

    ptr = (char *)(sn2->sn_vals + sc->sc_nkeys);
    sn2->sn_dn.bv_val = ptr;
    sn2->sn_dn.bv_len = rs->sr_entry->e_nname.bv_len;
    memcpy(ptr, rs->sr_entry->e_nname.bv_val, rs->sr_entry->e_nname.bv_len);
...
static int sssvlv_op_search(Operation *op, SlapReply *rs) {
  ...
  if (matchrule && matchrule->bv_val) {
    ...
    if (mr == NULL) {
      ...
      Debug(LDAP_DEBUG_TRACE,
            "%s: no ordering rule specified and no default ordering rule for "
            "attribute %s\n",
...
static int build_key(BerElement *ber, SlapReply *rs, sort_key *key) {
  struct berval attr;
 ...
 */
static int sss_parseCtrl(Operation *op, SlapReply *rs, LDAPControl *ctrl) {
  BerElementBuffer berbuf;
  BerElement *ber;
  ber_tag_t tag;
  ber_len_t len;
...
static int vlv_parseCtrl(Operation *op, SlapReply *rs, LDAPControl *ctrl) {
  BerElementBuffer berbuf;
  BerElement *ber;
  ber_tag_t tag;
  ber_len_t len;
...
static int sssvlv_db_open(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  sssvlv_info *si = on->on_bi.bi_private;
  int rc;
  int conn_index;
...
static int sssvlv_db_destroy(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
