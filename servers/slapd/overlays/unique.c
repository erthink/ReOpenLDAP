...
enum { UNIQUE_BASE = 1, UNIQUE_IGNORE, UNIQUE_ATTR, UNIQUE_STRICT, UNIQUE_URI };
...
static void unique_free_domain(unique_domain *domain) {
...
static int unique_new_domain_uri(unique_domain_uri **urip,
  ...
  if (attr_str) {
    for (i = 0; attr_str[i]; ++i) {
      unique_attrs *attr;
      ad = NULL;
      if (slap_str2ad(attr_str[i], &ad, &text) == LDAP_SUCCESS) {
        attr = ch_calloc(1, sizeof(unique_attrs));
        attr->attr = ad;
        attr->next = uri->attrs;
        uri->attrs = attr;
      } else {
...
static int unique_new_domain(unique_domain **domainp, char *domain_spec,
  ...
  if (strncasecmp(uri_start, "ignore ", STRLENOF("ignore ")) == 0) {
    domain->ignore = 1;
  ...
  for (url_desc = url_descs; url_desc; url_desc = url_descs->lud_next) {
    rc = unique_new_domain_uri(&domain->uri, url_desc, c);
    if (rc) {
      rc = ARG_BAD_CONF;
      goto exit;
    }
...
static int unique_cf_base(ConfigArgs *c) {
  BackendDB *be = (BackendDB *)c->be;
  ...
  case SLAP_CONFIG_EMIT:
    ...
    if (legacy && legacy->uri && legacy->uri->dn.bv_val) {
      rc = value_add_one(&c->rvalue_vals, &legacy->uri->dn);
      if (rc)
        return rc;
      rc = value_add_one(&c->rvalue_nvals, &legacy->uri->ndn);
      if (rc)
        return rc;
    }
    break;
  ...
  case SLAP_CONFIG_ADD:
    ...
    if (!legacy) {
      unique_new_domain(&private->legacy, UNIQUE_DEFAULT_URI, c);
...
static int unique_cf_attrs(ConfigArgs *c) {
  slap_overinst *on = (slap_overinst *)c->bi;
  unique_data *private = (unique_data *)on->on_bi.bi_private;
  unique_domain *domains = private->domains;
  unique_domain *legacy = private->legacy;
  ...
  case SLAP_CONFIG_EMIT:
    if (legacy && (c->type == UNIQUE_IGNORE) == legacy->ignore && legacy->uri)
      for (attr = legacy->uri->attrs; attr; attr = attr->next)
        value_add_one(&c->rvalue_vals, &attr->attr->ad_cname);
    rc = 0;
    break;
  case LDAP_MOD_DELETE:
    if (legacy && (c->type == UNIQUE_IGNORE) == legacy->ignore && legacy->uri &&
      ...
      if (!legacy->uri->attrs && !legacy->uri->dn.bv_val) {
        unique_free_domain_uri(legacy->uri);
        legacy->uri = NULL;
      }
      if (!legacy->uri && !private->legacy_strict_set) {
  ...
  case SLAP_CONFIG_ADD:
    ...
    for (attrp = &legacy->uri->attrs; *attrp; attrp = &(*attrp)->next)
      ;
    *attrp = reverse_attrs;
...
static int unique_cf_strict(ConfigArgs *c) {
  slap_overinst *on = (slap_overinst *)c->bi;
  unique_data *private = (unique_data *)on->on_bi.bi_private;
  unique_domain *domains = private->domains;
  unique_domain *legacy = private->legacy;
  ...
  case SLAP_CONFIG_EMIT:
    ...
    if (private->legacy_strict_set) {
      ...
      bv.bv_len = legacy->strict ? STRLENOF("TRUE") : STRLENOF("FALSE");
...
static int unique_cf_uri(ConfigArgs *c) {
  slap_overinst *on = (slap_overinst *)c->bi;
  unique_data *private = (unique_data *)on->on_bi.bi_private;
  unique_domain *domains = private->domains;
  unique_domain *legacy = private->legacy;
  ...
  switch (c->op) {
  case SLAP_CONFIG_EMIT:
    for (domain = domains; domain; domain = domain->next) {
      rc = value_add_one(&c->rvalue_vals, &domain->domain_spec);
      if (rc)
        break;
    }
    break;
  ...
  case LDAP_MOD_ADD:
    ...
    rc = 0;
    if (c->line)
      rc = unique_new_domain(&domain, c->line, c);
    else
      rc = unique_new_domain(&domain, c->argv[1], c);
    if (rc)
      break;
    assert(domain->next == NULL);
    for (domainp = &private->domains; *domainp; domainp = &(*domainp)->next)
      ;
    *domainp = domain;
...
static int unique_db_init(BackendDB *be, ConfigReply *cr) {
...
static int unique_db_destroy(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  unique_data **privatep = (unique_data **)&on->on_bi.bi_private;
...
static int count_attr_cb(Operation *op, SlapReply *rs) {
  unique_counter *uc;
...
static int count_filter_len(unique_domain *domain, unique_domain_uri *uri,
  ...
  while (!is_at_operational(ad->ad_type)) {
    if (uri->attrs) {
      ...
      if ((domain->ignore && attr) || (!domain->ignore && !attr)) {
        break;
      }
    }
    if (b && b[0].bv_val) {
...
static int unique_search(Operation *op, Operation *nop, struct berval *dn,
  ...
  if (nop->ors_filter == NULL) {
    ...
    send_ldap_error(op, rs, LDAP_OTHER, "unique_search invalid filter");
    return (rs->sr_err);
  }
...
static int unique_add(Operation *op, SlapReply *rs) {
  ...
  for (domain = legacy ? legacy : domains; domain; domain = domain->next) {
    unique_domain_uri *uri;

    for (uri = domain->uri; uri; uri = uri->next) {
      int len;
      int ks = 0;

      if (uri->ndn.bv_val && !dnIsSuffix(&op->o_req_ndn, &uri->ndn))
        continue;
      ...
      if (!(a = op->ora_e->e_attrs)) {
        ...
        for (; a; a = a->a_next) {
          ks += count_filter_len(domain, uri, a->a_desc, a->a_vals);
        }
      }
...
static int unique_modify(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
  unique_data *private = (unique_data *)on->on_bi.bi_private;
  unique_domain *domains = private->domains;
  unique_domain *legacy = private->legacy;
    ...
    for (uri = domain->uri; uri; uri = uri->next) {
      ...
      for (m = op->orm_modlist; m; m = m->sml_next)
        if ((m->sml_op & LDAP_MOD_OP) != LDAP_MOD_DELETE)
          ks += count_filter_len(domain, uri, m->sml_desc, m->sml_values);
...
static int unique_modrdn(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
  unique_data *private = (unique_data *)on->on_bi.bi_private;
  unique_domain *domains = private->domains;
  unique_domain *legacy = private->legacy;
  ...
  for (domain = legacy ? legacy : domains; domain; domain = domain->next) {
    ...
    for (uri = domain->uri; uri; uri = uri->next) {
      ...
      for (i = 0; newrdn[i]; i++) {
        ...
        ks += count_filter_len(domain, uri, newrdn[i]->la_private, bv);
      }
