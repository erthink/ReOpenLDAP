...
enum { REFINT_ATTRS = 1, REFINT_NOTHING, REFINT_MODIFIERSNAME };
...
static ConfigOCs refintocs[] = {{"( OLcfgOvOc:11.1 "
...
static int refint_cf_gen(ConfigArgs *c) {
  ...
  case SLAP_CONFIG_EMIT:
    ...
    case REFINT_ATTRS:
      ip = dd->attrs;
      while (ip) {
        value_add_one(&c->rvalue_vals, &ip->attr->ad_cname);
        ip = ip->next;
      }
      rc = 0;
      break;
    case REFINT_NOTHING:
      if (!BER_BVISEMPTY(&dd->nothing)) {
        ...
        if (rc)
          return rc;
        rc = value_add_one(&c->rvalue_nvals, &dd->nnothing);
        return rc;
      }
      rc = 0;
      break;
    case REFINT_MODIFIERSNAME:
      if (!BER_BVISEMPTY(&dd->refint_dn)) {
        rc = value_add_one(&c->rvalue_vals, &dd->refint_dn);
        if (rc)
          return rc;
        rc = value_add_one(&c->rvalue_nvals, &dd->refint_ndn);
        return rc;
      }
      rc = 0;
      break;
...
static int refint_db_init(BackendDB *be, ConfigReply *cr) {
...
static int refint_db_destroy(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
...
static int refint_open(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  refint_data *id = on->on_bi.bi_private;
...
static int refint_close(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  refint_data *id = on->on_bi.bi_private;
...
static int refint_search_cb(Operation *op, SlapReply *rs) {
  Attribute *a;
  BerVarray b = NULL;
...
static int refint_repair(Operation *op, refint_data *id, refint_q *rq) {
  dependent_data *dp;
  ...
  if (rc != LDAP_SUCCESS) {
    Debug(LDAP_DEBUG_TRACE, "refint_repair: search failed: %d\n", rc);
  ...
  if (op->o_callback->sc_private == NULL) {
    Debug(LDAP_DEBUG_TRACE, "refint_repair: callback wiped out sc_private?!\n");
    slap_biglock_release(bl);
    return 0;
  }
...
static void *refint_qtask(void *ctx, void *arg) {
...
static int refint_response(Operation *op, SlapReply *rs) {
  refint_pre *rp;
  slap_overinst *on;
  refint_data *id;
  BerValue pdn;
 ...
 */
static int refint_preop(Operation *op, SlapReply *rs) {
