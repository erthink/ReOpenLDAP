...
static int dgroup_cf(ConfigArgs *c) {
  ...
  case SLAP_CONFIG_EMIT: {
    ...
    for (ap = on->on_bi.bi_private; ap; ap = ap->ap_next) {
      struct berval bv;
      char *ptr;
      bv.bv_len = ap->ap_mem->ad_cname.bv_len + 1 + ap->ap_uri->ad_cname.bv_len;
...
static ConfigOCs dgroupocs[] = {{"( OLcfgOvOc:17.1 "
                                 "NAME 'olcDGConfig' "
...
static int dyngroup_response(Operation *op, SlapReply *rs) {
...
static int dyngroup_destroy(BackendDB *be, ConfigReply *cr) {
