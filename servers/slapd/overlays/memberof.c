...
#define MEMBEROF_CHK(mo, f) (((mo)->mo_flags & (f)) == (f))
#define MEMBEROF_DANGLING_CHECK(mo) ((mo)->mo_flags & MEMBEROF_FDANGLING_MASK)
#define MEMBEROF_DANGLING_DROP(mo) MEMBEROF_CHK((mo), MEMBEROF_FDANGLING_DROP)
#define MEMBEROF_DANGLING_ERROR(mo) MEMBEROF_CHK((mo), MEMBEROF_FDANGLING_ERROR)
#define MEMBEROF_REFINT(mo) MEMBEROF_CHK((mo), MEMBEROF_FREFINT)
#define MEMBEROF_REVERSE(mo) MEMBEROF_CHK((mo), MEMBEROF_FREVERSE)
} memberof_t;
...
static int memberof_isGroupOrMember_cb(Operation *op, SlapReply *rs) {
 ...
 */
static int memberof_saveMember_cb(Operation *op, SlapReply *rs) {
  if (rs->sr_type == REP_SEARCH) {
    memberof_cookie_t *mc;
    Attribute *a;
 ...
 */
static int memberof_isGroupOrMember(Operation *op, memberof_cbinfo_t *mci) {
...
static int memberof_cleanup(Operation *op, SlapReply *rs) {
...
static int memberof_op_add(Operation *op, SlapReply *rs) {
  ...
  if (MEMBEROF_DANGLING_CHECK(mo) && !get_relax(op) &&
    ...
    for (ap = &op->ora_e->e_attrs; *ap;) {
      ...
      for (i = 0; !BER_BVISNULL(&a->a_nvals[i]); i++) {
        ...
        rc = be_entry_get_rw(op, &a->a_nvals[i], NULL, NULL, 0, &e);
        if (rc == LDAP_SUCCESS) {
...
static int memberof_op_delete(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
  memberof_t *mo = (memberof_t *)on->on_bi.bi_private;
...
static int memberof_op_modify(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
  memberof_t *mo = (memberof_t *)on->on_bi.bi_private;
  ...
  if (memberof_isGroupOrMember(op, &mcis) == LDAP_SUCCESS &&
    ...
    for (ml = op->orm_modlist; ml; ml = ml->sml_next) {
      if (ml->sml_desc == mo->mo_ad_member) {
        ...
        case SLAP_MOD_SOFTDEL: /* ITS#7487: can be used by syncrepl (in mirror
                                  mode?) */
          save_member = 1;
          break;
        }
  ...
  if (mmlp != NULL) {
    ...
    op->o_bd->bd_info = (BackendInfo *)on->on_info;
    rc = be_entry_get_rw(op, &op->o_req_ndn, NULL, NULL, 0, &target);
    op->o_bd->bd_info = (BackendInfo *)on;
    if (rc != LDAP_SUCCESS) {
    ...
    case SLAP_MOD_SOFTDEL: /* ITS#7487: can be used by syncrepl (in mirror
      ...
      if (ml->sml_nvalues != NULL) {
        ...
        for (i = 0; !BER_BVISNULL(&ml->sml_nvalues[i]); i++) {
          ...
          rc = be_entry_get_rw(op, &ml->sml_nvalues[i], NULL, NULL, 0, &e);
          op->o_bd->bd_info = (BackendInfo *)on;
          if (rc != LDAP_SUCCESS) {
            if (get_relax(op)) {
              continue;
    ...
    case LDAP_MOD_REPLACE:
      ...
      op->o_bd->bd_info = (BackendInfo *)on->on_info;
      /* access is checked with the original identity */
      rc = access_allowed(op, target, mo->mo_ad_memberof, NULL, ACL_WDEL, NULL);
      op->o_bd->bd_info = (BackendInfo *)on;
      if (rc == 0) {
        rc = rs->sr_err = LDAP_INSUFFICIENT_ACCESS;
        rs->sr_text = NULL;
...
static int memberof_op_modrdn(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
  slap_callback *sc;
  memberof_cbinfo_t *mci;
  OpExtra *oex;
...
static int memberof_db_init(BackendDB *be, ConfigReply *cr) {
...
static ConfigOCs mo_ocs[] = {{"( OLcfgOvOc:18.1 "
                              "NAME 'olcMemberOf' "
...
static int memberof_make_member_filter(memberof_t *mo) {
  char *ptr;
...
static int mo_cf_gen(ConfigArgs *c) {
...
static int memberof_db_open(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  memberof_t *mo = (memberof_t *)on->on_bi.bi_private;
...
static int memberof_db_destroy(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  memberof_t *mo = (memberof_t *)on->on_bi.bi_private;
...
int memberof_over_initialize(void) {
  int code, i;

  for (i = 0; as[i].desc != NULL; i++) {
    code = register_at(as[i].desc, as[i].adp, 0);
    if (code) {
      Debug(LDAP_DEBUG_ANY, "memberof_initialize: register_at #%d failed\n", i);
      return code;
    }
  }
