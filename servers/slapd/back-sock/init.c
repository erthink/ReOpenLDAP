...
int sock_back_initialize(BackendInfo *bi) {
  bi->bi_open = 0;
  bi->bi_config = 0;
  bi->bi_close = 0;
  bi->bi_destroy = 0;
...
int sock_back_db_init(Backend *be, struct config_reply_s *cr) {
  struct sockinfo *si;
...
int sock_back_db_destroy(Backend *be, struct config_reply_s *cr) {
  free(be->be_private);
  return 0;
}
