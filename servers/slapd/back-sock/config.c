...
enum { BS_EXT = 1, BS_OPS, BS_RESP, BS_DNPAT };
...
static ConfigOCs osocs[] = {{"( OLcfgDbOc:7.2 "
...
static slap_verbmasks bs_exts[] = {{BER_BVC("binddn"), SOCK_EXT_BINDDN},
...
int sock_back_init_cf(BackendInfo *bi) {
  int rc;
  bi->bi_cf_ocs = bsocs;
...
static int sock_over_response(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
