...
int meta_back_open(BackendInfo *bi) {
...
int meta_back_initialize(BackendInfo *bi) {
  bi->bi_flags =
#if 0
...
int meta_back_db_init(Backend *be, ConfigReply *cr) {
  metainfo_t *mi;
  int i;
  BackendInfo *bi;
...
int meta_back_db_open(Backend *be, ConfigReply *cr) {
  ...
  for (i = 0; i < mi->mi_ntargets; i++) {
    ...
    if (meta_target_finish(mi, mt, "meta_back_db_open", msg, sizeof(msg)))
      return 1;
  }
...
void meta_back_map_free(struct ldapmap *lm) {
...
static void target_free(metatarget_t *mt) {
  if (mt->mt_uri) {
    free(mt->mt_uri);
...
int meta_back_db_destroy(Backend *be, ConfigReply *cr) {
  metainfo_t *mi;
