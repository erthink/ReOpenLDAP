...
static int ldap_back_map_config(ConfigArgs *c, struct ldapmap *oc_map,
...
static ConfigTable metacfg[] = {
    ...
    {"acl-authcDN", "DN", 2, 2, 0,
     ARG_DN | ARG_MAGIC | LDAP_BACK_CFG_ACL_AUTHCDN, meta_back_cf_gen,
     "( OLcfgDbAt:3.2 "
    ...
    {"idassert-authzFrom", "authzRule", 2, 2, 0,
     ARG_MAGIC | LDAP_BACK_CFG_IDASSERT_AUTHZFROM, meta_back_cf_gen,
     "( OLcfgDbAt:3.9 "
    ...
    {"rebind-as-user", "true|FALSE", 1, 2, 0,
     ARG_MAGIC | ARG_ON_OFF | LDAP_BACK_CFG_REBIND, meta_back_cf_gen,
     "( OLcfgDbAt:3.10 "
    ...
    {"chase-referrals", "true|FALSE", 2, 2, 0,
     ARG_MAGIC | ARG_ON_OFF | LDAP_BACK_CFG_CHASE, meta_back_cf_gen,
     "( OLcfgDbAt:3.11 "
    ...
    {"t-f-support", "true|FALSE|discover", 2, 2, 0,
     ARG_MAGIC | LDAP_BACK_CFG_T_F, meta_back_cf_gen,
     "( OLcfgDbAt:3.12 "
     "NAME 'olcDbTFSupport' "
    ...
    {"idle-timeout", "timeout", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_IDLE_TIMEOUT,
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"conn-ttl", "ttl", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_CONN_TTL,
     meta_back_cf_gen,
     "( OLcfgDbAt:3.16 "
     "NAME 'olcDbConnTtl' "
     "DESC 'connection ttl' "
    ...
    {"network-timeout", "timeout", 2, 2, 0,
     ARG_MAGIC | LDAP_BACK_CFG_NETWORK_TIMEOUT, meta_back_cf_gen,
     "( OLcfgDbAt:3.17 "
    ...
    {"protocol-version", "version", 2, 2, 0,
     ARG_MAGIC | ARG_INT | LDAP_BACK_CFG_VERSION, meta_back_cf_gen,
     "( OLcfgDbAt:3.18 "
    ...
    {"single-conn", "true|FALSE", 2, 2, 0,
     ARG_MAGIC | ARG_ON_OFF | LDAP_BACK_CFG_SINGLECONN, meta_back_cf_gen,
     "( OLcfgDbAt:3.19 "
    ...
    {"use-temporary-conn", "true|FALSE", 2, 2, 0,
     ARG_MAGIC | ARG_ON_OFF | LDAP_BACK_CFG_USETEMP, meta_back_cf_gen,
     "( OLcfgDbAt:3.22 "
    ...
    {"conn-pool-max", "<n>", 2, 2, 0,
     ARG_MAGIC | ARG_INT | LDAP_BACK_CFG_CONNPOOLMAX, meta_back_cf_gen,
     "( OLcfgDbAt:3.23 "
    ...
    {"session-tracking-request", "true|FALSE", 2, 2, 0,
     ARG_MAGIC | ARG_ON_OFF | LDAP_BACK_CFG_ST_REQUEST, meta_back_cf_gen,
     "( OLcfgDbAt:3.24 "
    ...
    {"norefs", "true|FALSE", 2, 2, 0,
     ARG_MAGIC | ARG_ON_OFF | LDAP_BACK_CFG_NOREFS, meta_back_cf_gen,
     "( OLcfgDbAt:3.25 "
     "NAME 'olcDbNoRefs' "
    ...
    {"noundeffilter", "true|FALSE", 2, 2, 0,
     ARG_MAGIC | ARG_ON_OFF | LDAP_BACK_CFG_NOUNDEFFILTER, meta_back_cf_gen,
     "( OLcfgDbAt:3.26 "
    ...
    {"rewrite", "arglist", 2, 0, STRLENOF("rewrite"),
     ARG_MAGIC | LDAP_BACK_CFG_REWRITE, meta_back_cf_gen,
     "( OLcfgDbAt:3.101 "
     "NAME 'olcDbRewrite' "
    ...
    {"suffixmassage", "virtual> <real", 2, 3, 0,
     ARG_MAGIC | LDAP_BACK_CFG_SUFFIXM, meta_back_cf_gen, NULL, NULL, NULL},

    {"map", "attribute|objectClass> [*|<local>] *|<remote", 3, 4, 0,
     ARG_MAGIC | LDAP_BACK_CFG_MAP, meta_back_cf_gen,
     "( OLcfgDbAt:3.102 "
     "NAME 'olcDbMap' "
    ...
    {"subtree-exclude", "pattern", 2, 2, 0,
     ARG_MAGIC | LDAP_BACK_CFG_SUBTREE_EX, meta_back_cf_gen,
     "( OLcfgDbAt:3.103 "
    ...
    {"subtree-include", "pattern", 2, 2, 0,
     ARG_MAGIC | LDAP_BACK_CFG_SUBTREE_IN, meta_back_cf_gen,
     "( OLcfgDbAt:3.104 "
    ...
    {"default-target", "[none|<target ID>]", 1, 2, 0,
     ARG_MAGIC | LDAP_BACK_CFG_DEFAULT_T, meta_back_cf_gen,
     "( OLcfgDbAt:3.105 "
    ...
    {"bind-timeout", "microseconds", 2, 2, 0,
     ARG_MAGIC | ARG_ULONG | LDAP_BACK_CFG_BIND_TIMEOUT, meta_back_cf_gen,
     "( OLcfgDbAt:3.107 "
    ...
    {"pseudorootdn", "dn", 2, 2, 0,
     ARG_MAGIC | ARG_DN | LDAP_BACK_CFG_PSEUDOROOTDN, meta_back_cf_gen, NULL,
     NULL, NULL},
    {"pseudorootpw", "password", 2, 2, 0,
     ARG_MAGIC | ARG_STRING | LDAP_BACK_CFG_PSEUDOROOTDN, meta_back_cf_gen,
     NULL, NULL, NULL},
    {"nretries", "NEVER|forever|<number>", 2, 2, 0,
     ARG_MAGIC | LDAP_BACK_CFG_NRETRIES, meta_back_cf_gen,
     "( OLcfgDbAt:3.110 "
     "NAME 'olcDbNretries' "
     "DESC 'retry handling' "
     "SYNTAX OMsDirectoryString "
    ...
    {"client-pr", "accept-unsolicited|disable|<size>", 2, 2, 0,
     ARG_MAGIC | LDAP_BACK_CFG_CLIENT_PR, meta_back_cf_gen,
     "( OLcfgDbAt:3.111 "
     "NAME 'olcDbClientPr' "
...
static int meta_suffixm_config(ConfigArgs *c, int argc, char **argv,
                               metatarget_t *mt) {
  BackendDB *tmp_bd;
...
static int meta_back_cf_gen(ConfigArgs *c) {
  ...
  if (c->op == SLAP_CONFIG_EMIT) {
    ...
    case LDAP_BACK_CFG_REWRITE: {
      if (c->valx >= 0) {
        ...
        for (i = c->valx; !BER_BVISNULL(&mt->mt_rwmap.rwm_bva_rewrite[i + 1]);
             i++) {
        ...
        for (i = 0; !BER_BVISNULL(&mt->mt_rwmap.rwm_bva_rewrite[i]); i++) {
          ...
          if (!strcasecmp(ca.argv[0], "suffixmassage")) {
            ...
            rc = rewrite_parse(mt->mt_rwmap.rwm_rw, c->fname, c->lineno,
                               ca.argc, ca.argv);
          }
  ...
  case LDAP_BACK_CFG_URI: {
    ...
    mi->mi_targets = (metatarget_t **)ch_realloc(
        mi->mi_targets, sizeof(metatarget_t *) * mi->mi_ntargets);
  ...
  case LDAP_BACK_CFG_CONN_TTL: {
    ...
    if (lutil_parse_time(c->argv[1], &t)) {
      snprintf(c->cr_msg, sizeof(c->cr_msg), "unable to parse conn ttl \"%s\"",
               c->argv[1]);
      Debug(LDAP_DEBUG_ANY, "%s: %s.\n", c->log, c->cr_msg);
      return 1;
    }
...
idassert-authzFrom	"dn:<rootdn>"
...

    {
      ...
      char *cargv[] = {"idassert-bind", "bindmethod=simple",      NULL,
                       "mode=none",     "flags=non-prescriptive", NULL};
      char **oargv;
      int oargc;
      int cargc = 5;
      int rc;
      ...
      if (BER_BVISNULL(&c->be->be_rootndn)) {
        Debug(LDAP_DEBUG_ANY,
              "%s: \"pseudorootpw\": \"rootdn\" must be defined first.\n",
              c->log);
        return 1;
      }

      if (sizeof(binddn) <=
          (unsigned)snprintf(binddn, sizeof(binddn), "binddn=%s", c->argv[1])) {
      ...
      if (rc == 0) {
        ...
        if (mt->mt_idassert_authz != NULL) {
          Debug(LDAP_DEBUG_ANY,
                "%s: \"idassert-authzFrom\" already defined (discarded).\n",
                c->log);
        ...
        memcpy(&bv.bv_val[STRLENOF("dn:")], c->be->be_rootndn.bv_val,
               c->be->be_rootndn.bv_len + 1);
  ...
  case LDAP_BACK_CFG_PSEUDOROOTPW:
    ...
    if (BER_BVISNULL(&mt->mt_idassert_authcDN)) {
      Debug(LDAP_DEBUG_ANY,
            "%s: \"pseudorootpw\": \"pseudorootdn\" must be defined first.\n",
            c->log);
      return 1;
    }
...
static int ldap_back_map_config(ConfigArgs *c, struct ldapmap *oc_map,
  ...
  if ((src[0] != '\0' &&
       avl_find(map->map, (caddr_t)&mapping[0], mapping_cmp) != NULL) ||
...
int suffix_massage_config(struct rewrite_info *info, struct berval *pvnc,
