...
static int ldap_back_map_config(ConfigArgs *c, struct ldapmap *oc_map, struct ldapmap *at_map);
...
static ConfigTable metacfg[] = {
    {"uri", "uri", 2, 0, 0, ARG_MAGIC | LDAP_BACK_CFG_URI, meta_back_cf_gen,
     "( OLcfgDbAt:0.14 "
     "NAME 'olcDbURI' "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"tls", "what", 2, 0, 0, ARG_MAGIC | LDAP_BACK_CFG_TLS, meta_back_cf_gen,
     "( OLcfgDbAt:3.1 "
     "NAME 'olcDbStartTLS' "
     "DESC 'StartTLS' "
     "EQUALITY caseExactMatch "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"acl-authcDN", "DN", 2, 2, 0, ARG_DN | ARG_MAGIC | LDAP_BACK_CFG_ACL_AUTHCDN, meta_back_cf_gen,
     "( OLcfgDbAt:3.2 "
    ...
    {"binddn", "DN", 2, 2, 0, ARG_DN | ARG_MAGIC | LDAP_BACK_CFG_ACL_AUTHCDN, meta_back_cf_gen, NULL, NULL, NULL},
    {"acl-passwd", "cred", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_ACL_PASSWD, meta_back_cf_gen,
     "( OLcfgDbAt:3.3 "
     "NAME 'olcDbACLPasswd' "
    ...
    {"bindpw", "cred", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_ACL_PASSWD, meta_back_cf_gen, NULL, NULL, NULL},
    {"idassert-bind", "args", 2, 0, 0, ARG_MAGIC | LDAP_BACK_CFG_IDASSERT_BIND, meta_back_cf_gen,
     "( OLcfgDbAt:3.7 "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"idassert-authzFrom", "authzRule", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_IDASSERT_AUTHZFROM, meta_back_cf_gen,
     "( OLcfgDbAt:3.9 "
     ...
     "X-ORDERED 'VALUES' )",
     NULL, NULL},
    {"rebind-as-user", "true|FALSE", 1, 2, 0, ARG_MAGIC | ARG_ON_OFF | LDAP_BACK_CFG_REBIND, meta_back_cf_gen,
     "( OLcfgDbAt:3.10 "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"chase-referrals", "true|FALSE", 2, 2, 0, ARG_MAGIC | ARG_ON_OFF | LDAP_BACK_CFG_CHASE, meta_back_cf_gen,
     "( OLcfgDbAt:3.11 "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"t-f-support", "true|FALSE|discover", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_T_F, meta_back_cf_gen,
     "( OLcfgDbAt:3.12 "
     "NAME 'olcDbTFSupport' "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"timeout", "timeout(list)", 2, 0, 0, ARG_MAGIC | LDAP_BACK_CFG_TIMEOUT, meta_back_cf_gen,
     "( OLcfgDbAt:3.14 "
     "NAME 'olcDbTimeout' "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"idle-timeout", "timeout", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_IDLE_TIMEOUT, meta_back_cf_gen,
     "( OLcfgDbAt:3.15 "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"conn-ttl", "ttl", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_CONN_TTL, meta_back_cf_gen,
     "( OLcfgDbAt:3.16 "
     "NAME 'olcDbConnTtl' "
     "DESC 'connection ttl' "
     "EQUALITY caseIgnoreMatch "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"network-timeout", "timeout", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_NETWORK_TIMEOUT, meta_back_cf_gen,
     "( OLcfgDbAt:3.17 "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"protocol-version", "version", 2, 2, 0, ARG_MAGIC | ARG_INT | LDAP_BACK_CFG_VERSION, meta_back_cf_gen,
     "( OLcfgDbAt:3.18 "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"single-conn", "true|FALSE", 2, 2, 0, ARG_MAGIC | ARG_ON_OFF | LDAP_BACK_CFG_SINGLECONN, meta_back_cf_gen,
     "( OLcfgDbAt:3.19 "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"cancel", "ABANDON|ignore|exop", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_CANCEL, meta_back_cf_gen,
     "( OLcfgDbAt:3.20 "
     "NAME 'olcDbCancel' "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"quarantine", "retrylist", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_QUARANTINE, meta_back_cf_gen,
     "( OLcfgDbAt:3.21 "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"use-temporary-conn", "true|FALSE", 2, 2, 0, ARG_MAGIC | ARG_ON_OFF | LDAP_BACK_CFG_USETEMP, meta_back_cf_gen,
     "( OLcfgDbAt:3.22 "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"conn-pool-max", "<n>", 2, 2, 0, ARG_MAGIC | ARG_INT | LDAP_BACK_CFG_CONNPOOLMAX, meta_back_cf_gen,
     "( OLcfgDbAt:3.23 "
    ...
    {"norefs", "true|FALSE", 2, 2, 0, ARG_MAGIC | ARG_ON_OFF | LDAP_BACK_CFG_NOREFS, meta_back_cf_gen,
     "( OLcfgDbAt:3.25 "
     "NAME 'olcDbNoRefs' "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"noundeffilter", "true|FALSE", 2, 2, 0, ARG_MAGIC | ARG_ON_OFF | LDAP_BACK_CFG_NOUNDEFFILTER, meta_back_cf_gen,
     "( OLcfgDbAt:3.26 "
    ...
    {"rewrite", "arglist", 2, 0, STRLENOF("rewrite"), ARG_MAGIC | LDAP_BACK_CFG_REWRITE, meta_back_cf_gen,
     "( OLcfgDbAt:3.101 "
     "NAME 'olcDbRewrite' "
     ...
     "X-ORDERED 'VALUES' )",
     NULL, NULL},
    {"suffixmassage", "virtual> <real", 2, 3, 0, ARG_MAGIC | LDAP_BACK_CFG_SUFFIXM, meta_back_cf_gen, NULL, NULL, NULL},

    {"map", "attribute|objectClass> [*|<local>] *|<remote", 3, 4, 0, ARG_MAGIC | LDAP_BACK_CFG_MAP, meta_back_cf_gen,
     "( OLcfgDbAt:3.102 "
     "NAME 'olcDbMap' "
    ...
    {"subtree-exclude", "pattern", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_SUBTREE_EX, meta_back_cf_gen,
     "( OLcfgDbAt:3.103 "
     ...
     "SYNTAX OMsDirectoryString )",
     NULL, NULL},
    {"subtree-include", "pattern", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_SUBTREE_IN, meta_back_cf_gen,
     "( OLcfgDbAt:3.104 "
     ...
     "SYNTAX OMsDirectoryString )",
     NULL, NULL},
    {"default-target", "[none|<target ID>]", 1, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_DEFAULT_T, meta_back_cf_gen,
     "( OLcfgDbAt:3.105 "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"dncache-ttl", "ttl", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_DNCACHE_TTL, meta_back_cf_gen,
     "( OLcfgDbAt:3.106 "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"bind-timeout", "microseconds", 2, 2, 0, ARG_MAGIC | ARG_ULONG | LDAP_BACK_CFG_BIND_TIMEOUT, meta_back_cf_gen,
     "( OLcfgDbAt:3.107 "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"onerr", "CONTINUE|report|stop", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_ONERR, meta_back_cf_gen,
     "( OLcfgDbAt:3.108 "
     "NAME 'olcDbOnErr' "
     "DESC 'error handling' "
     "EQUALITY caseIgnoreMatch "
    ...
    {"pseudorootdn", "dn", 2, 2, 0, ARG_MAGIC | ARG_DN | LDAP_BACK_CFG_PSEUDOROOTDN, meta_back_cf_gen, NULL, NULL,
     NULL},
    {"pseudorootpw", "password", 2, 2, 0, ARG_MAGIC | ARG_STRING | LDAP_BACK_CFG_PSEUDOROOTDN, meta_back_cf_gen, NULL,
     NULL, NULL},
    {"nretries", "NEVER|forever|<number>", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_NRETRIES, meta_back_cf_gen,
     "( OLcfgDbAt:3.110 "
     "NAME 'olcDbNretries' "
     "DESC 'retry handling' "
     "EQUALITY caseIgnoreMatch "
     ...
     "SINGLE-VALUE )",
     NULL, NULL},
    {"client-pr", "accept-unsolicited|disable|<size>", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_CLIENT_PR, meta_back_cf_gen,
     "( OLcfgDbAt:3.111 "
     "NAME 'olcDbClientPr' "
    ...
    {"", "", 0, 0, 0, ARG_IGNORED, NULL,
     "( OLcfgDbAt:3.100 NAME 'olcMetaSub' "
    ...
    {"keepalive", "keepalive", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_KEEPALIVE, meta_back_cf_gen,
     "( OLcfgDbAt:3.29 "
     "NAME 'olcDbKeepalive' "
     "DESC 'TCP keepalive' "
     "EQUALITY caseIgnoreMatch "
    ...
    {"filter", "pattern", 2, 2, 0, ARG_MAGIC | LDAP_BACK_CFG_FILTER, meta_back_cf_gen,
     "( OLcfgDbAt:3.112 "
     "NAME 'olcDbFilter' "
...
static ConfigOCs metaocs[] = {{"( OLcfgDbOc:3.2 "
                               "NAME 'olcMetaConfig' "
...
static int meta_ldadd(CfEntryInfo *p, Entry *e, ConfigArgs *c) {
  if (p->ce_type != Cft_Database || !p->ce_be || p->ce_be->be_cf_ocs != metaocs)
...
static int meta_cfadd(Operation *op, SlapReply *rs, Entry *p, ConfigArgs *c) {
  ...
  for (i = 0; i < mi->mi_ntargets; i++) {
    bv.bv_len = snprintf(c->cr_msg, sizeof(c->cr_msg), "olcMetaSub=" SLAP_X_ORDERED_FMT "uri", i);
    c->ca_private = mi->mi_targets[i];
    c->valx = i;
    config_build_entry(op, rs, p->e_private, c, &bv, &metaocs[1], NULL);
  }

  return LDAP_SUCCESS;
}

static int meta_rwi_init(struct rewrite_info **rwm_rw) {
  char *rargv[3];
...
static int meta_suffixm_config(ConfigArgs *c, int argc, char **argv, metatarget_t *mt) {
  BackendDB *tmp_bd;
  ...
  if (dnPrettyNormal(NULL, &dn, &pvnc, &nvnc, NULL) != LDAP_SUCCESS) {
    snprintf(c->cr_msg, sizeof(c->cr_msg), "suffix \"%s\" is invalid", argv[1]);
   ...
   */
  rc = suffix_massage_config(mt->mt_rwmap.rwm_rw, &pvnc, &nvnc, &prnc, &nrnc);
...
static int slap_bv_x_ordered_unparse(BerVarray in, BerVarray *out) {
  int i;
  BerVarray bva = NULL;
  char ibuf[32], *ptr;
  struct berval idx;
...
int meta_subtree_free(metasubtree_t *ms) {
  switch (ms->ms_type) {
...
static void meta_filter_free(metafilter_t *mf) {
  regfree(&mf->mf_regex);
  ber_memfree(mf->mf_regex_pattern.bv_val);
  ch_free(mf);
}

void meta_filter_destroy(metafilter_t *mf) {
  if (mf->mf_next)
...
static int meta_subtree_unparse(ConfigArgs *c, metatarget_t *mt) {
  metasubtree_t *ms;
...
static int meta_subtree_config(metatarget_t *mt, ConfigArgs *c) {
  ...
  if (c->type == LDAP_BACK_CFG_SUBTREE_EX) {
    if (mt->mt_subtree && !mt->mt_subtree_exclude) {
      snprintf(c->cr_msg, sizeof(c->cr_msg),
               "\"subtree-exclude\" incompatible with previous "
               "\"subtree-include\" directives");
      return 1;
    }
    ...
    if (mt->mt_subtree && mt->mt_subtree_exclude) {
      snprintf(c->cr_msg, sizeof(c->cr_msg),
               "\"subtree-include\" incompatible with previous "
               "\"subtree-exclude\" directives");
      return 1;
    }
  }
      ...
      case META_ST_SUBTREE:
        ...
        case META_ST_SUBTREE:
          if (dnIsSuffix(&(*msp)->ms_dn, &ms->ms_dn)) {
            ...
            Debug(LDAP_DEBUG_CONFIG,
                  "%s: previous rule \"dn.subtree:%s\" is contained in rule "
                  "\"dn.subtree:%s\" (replaced)\n",
            ...
            *msp = (*msp)->ms_next;
            ...
            Debug(LDAP_DEBUG_CONFIG,
                  "%s: previous rule \"dn.subtree:%s\" contains rule "
        ...
        case META_ST_SUBORDINATE:
          if (dnIsSuffix(&(*msp)->ms_dn, &ms->ms_dn)) {
            metasubtree_t *tmp = *msp;
            Debug(LDAP_DEBUG_CONFIG,
                  "%s: previous rule \"dn.children:%s\" is contained in rule "
                  "\"dn.subtree:%s\" (replaced)\n",
                  c->log, pattern, (*msp)->ms_dn.bv_val);
            *msp = (*msp)->ms_next;
            tmp->ms_next = NULL;
        ...
        case META_ST_REGEX:
          if (regexec(&(*msp)->ms_regex, ms->ms_dn.bv_val, 0, NULL, 0) == 0) {
            Debug(LDAP_DEBUG_CONFIG,
                  "%s: previous rule \"dn.regex:%s\" may contain rule "
                  "\"dn.subtree:%s\"\n",
      ...
      case META_ST_REGEX:
        ...
        case META_ST_SUBORDINATE:
          if (regexec(&ms->ms_regex, (*msp)->ms_dn.bv_val, 0, NULL, 0) == 0) {
            Debug(LDAP_DEBUG_CONFIG,
                  "%s: previous rule \"dn.subtree:%s\" may be contained in "
                  "rule \"dn.regex:%s\"\n",
...
static int meta_cf_cleanup(ConfigArgs *c) {
  metainfo_t *mi = (metainfo_t *)c->be->be_private;
...
static int meta_back_cf_gen(ConfigArgs *c) {
  metainfo_t *mi = (metainfo_t *)c->be->be_private;
  ...
  if (c->op == SLAP_CONFIG_EMIT) {
    ...
    case LDAP_BACK_CFG_TIMEOUT:
      ...
      if (i) {
        bv.bv_len -= i;
        memmove(bv.bv_val, &bv.bv_val[i], bv.bv_len + 1);
      }
  ...
  case LDAP_BACK_CFG_URI: {
    ...
    mi->mi_targets = (metatarget_t **)ch_realloc(mi->mi_targets, sizeof(metatarget_t *) * mi->mi_ntargets);
    ...
    if (BER_BVISNULL(&c->be->be_nsuffix[j])) {
      snprintf(c->cr_msg, sizeof(c->cr_msg),
               "<naming context> of URI must be within the naming context of "
               "this database.");
      Debug(LDAP_DEBUG_ANY, "%s: %s.\n", c->log, c->cr_msg);
      return 1;
    }
  ...
  case LDAP_BACK_CFG_CONN_TTL: {
    ...
    if (lutil_parse_time(c->argv[1], &t)) {
      snprintf(c->cr_msg, sizeof(c->cr_msg), "unable to parse conn ttl \"%s\"", c->argv[1]);
      Debug(LDAP_DEBUG_ANY, "%s: %s.\n", c->log, c->cr_msg);
      return 1;
    }
  ...
  case LDAP_BACK_CFG_CONNPOOLMAX:
    ...
    if (c->value_int < LDAP_BACK_CONN_PRIV_MIN || c->value_int > LDAP_BACK_CONN_PRIV_MAX) {
      snprintf(c->cr_msg, sizeof(c->cr_msg),
               ...
               c->argv[1], LDAP_BACK_CONN_PRIV_MIN, LDAP_BACK_CONN_PRIV_MAX);
      Debug(LDAP_DEBUG_ANY, "%s: %s.\n", c->log, c->cr_msg);
      return 1;
    }
...
idassert-authzFrom	"dn:<rootdn>"
...

    {
      ...
      char *cargv[] = {"idassert-bind", "bindmethod=simple", NULL, "mode=none", "flags=non-prescriptive", NULL};
      char **oargv;
      int oargc;
      int cargc = 5;
      int rc;
      ...
      if (sizeof(binddn) <= (unsigned)snprintf(binddn, sizeof(binddn), "binddn=%s", c->argv[1])) {
  ...
  case LDAP_BACK_CFG_PSEUDOROOTPW:
    ...
    if (!BER_BVISNULL(&mt->mt_idassert_passwd)) {
      memset(mt->mt_idassert_passwd.bv_val, 0, mt->mt_idassert_passwd.bv_len);
  ...
  case LDAP_BACK_CFG_QUARANTINE:
    /* quarantine */
    if (META_BACK_CMN_QUARANTINE(mc)) {
      snprintf(c->cr_msg, sizeof(c->cr_msg), "quarantine already defined");
      Debug(LDAP_DEBUG_ANY, "%s: %s.\n", c->log, c->cr_msg);
      return 1;
    }
  ...
  case LDAP_BACK_CFG_REWRITE: {
    ...
    if (!strcasecmp(argv[0], "suffixmassage")) {
      ...
      rc = rewrite_parse(mt->mt_rwmap.rwm_rw, c->fname, c->lineno, argc, argv);
    }
    if (rc) {
      if (ix < cnt) {
        rewrite_info_delete(&mt->mt_rwmap.rwm_rw);
  ...
  case LDAP_BACK_CFG_MAP: {
    ...
    if (ix >= cnt || ix < 0) {
      ...
      for (i = 0; i < ix; i++) {
        ...
        ca.argv = argv;
        ca.argc++;
        rc = ldap_back_map_config(&ca, &mt->mt_rwmap.rwm_oc, &mt->mt_rwmap.rwm_at);
    ...
    /* add the new mapping */
    rc = ldap_back_map_config(c, &mt->mt_rwmap.rwm_oc, &mt->mt_rwmap.rwm_at);
    if (rc) {
      goto map_fail;
    }
...
static int ldap_back_map_config(ConfigArgs *c, struct ldapmap *oc_map, struct ldapmap *at_map) {
  ...
  if (mapping == NULL) {
    snprintf(c->cr_msg, sizeof(c->cr_msg), "out of memory");
    Debug(LDAP_DEBUG_ANY, "%s: %s.\n", c->log, c->cr_msg);
    return 1;
  }
  ...
  if (src[0] != '\0') {
    avl_insert(&map->map, (caddr_t)&mapping[0], mapping_cmp, mapping_dup);
  }
  avl_insert(&map->remap, (caddr_t)&mapping[1], mapping_cmp, mapping_dup);
...
static char *suffix_massage_regexize(const char *s) {
  char *res, *ptr;
  const char *p, *r;
  int i;
...
static char *suffix_massage_patternize(const char *s, const char *p) {
  ber_len_t len;
  char *res, *ptr;
...
int suffix_massage_config(struct rewrite_info *info, struct berval *pvnc, struct berval *nvnc, struct berval *prnc,
                          struct berval *nrnc) {
  char *rargv[5];
  int line = 0;
