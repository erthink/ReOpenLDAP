...
static void meta_back_print(metaconn_t *mc, char *avlstr) {
  int i;
...
static void meta_back_ravl_print(Avlnode *root, int depth) {
  int i;
  ...
  meta_back_print((metaconn_t *)root->avl_data, avl_bf2str(root->avl_bf));
 ...
 */
static metaconn_t *metaconn_alloc(Operation *op) {
...
retry_lock:;
  ...
  if (ispriv) {
    ...
    if (!BER_BVISEMPTY(&op->o_ndn) && SLAP_IS_AUTHZ_BACKEND(op) && isauthz) {
       ...
       */
      if (ldap_back_dn_massage(&dc, &op->o_conn->c_dn, &msc->msc_bound_ndn)) {
...
metaconn_t *meta_back_getconn(Operation *op, SlapReply *rs, int *candidate,
  ...
  if (op_type == META_OP_REQUIRE_SINGLE) {
    metatarget_t *mt __maybe_unused = NULL;
    metasingleconn_t *msc __maybe_unused = NULL;
