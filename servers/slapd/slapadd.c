    ...
    return ldifrc < 0 ? -1 : 0;
  {
    ...
    if (bd != be) {
      fprintf(stderr,
              ...
              progname, erec->lineno, dbnum, be->be_suffix[0].bv_val, e->e_dn);
      if (bd) {
        BackendDB *bdtmp;
        int dbidx = 0;
        LDAP_STAILQ_FOREACH(bdtmp, &backendDB, be_next) {
    ...
    if (SLAP_LASTMOD(be)) {
      ...
      if (attr_find(e->e_attrs, slap_schema.si_ad_createTimestamp) == NULL) {
        vals[0] = timestamp;
      ...
      if (attr_find(e->e_attrs, slap_schema.si_ad_modifyTimestamp) == NULL) {
        vals[0] = timestamp;
