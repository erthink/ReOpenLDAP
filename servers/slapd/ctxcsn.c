...
int slap_get_commit_csn(Operation *op, struct berval *maxcsn) {
...
int slap_graduate_commit_csn(Operation *op) {
  ...
  LDAP_TAILQ_FOREACH(csne, be->be_pending_csn_list, ce_csn_link) {
    if (csne->ce_op == op) {
      ...
      found = csne->ce_state;
      /* assert(found != 0); LY: op could be cancelled/abandoned before
       * slap_get_commit_csn() */
...
void slap_queue_csn(Operation *op, const struct berval *csn) {
  ...
  if (likely(pending == NULL)) {
    ...
    if (before == NULL) {
      ...
      Debug(LDAP_DEBUG_SYNC,
            "slap_queue_csn: middle-queueing %p %s, op %p\n"
            "\tbefore %p %s\n",
            pending, pending->ce_csn.bv_val, op, before, before->ce_csn.bv_val);
...
int slap_get_csn(Operation *op, struct berval *csn) {
