...
int perl_back_initialize(BackendInfo *bi) {
  ...
  /* injecting code from perl_back_open, because using function reference
   * (bi->bi_open) is not functional */
...
int perl_back_db_init(BackendDB *be, ConfigReply *cr) {
...
int perl_back_db_open(BackendDB *be, ConfigReply *cr) {
  int count;
  int return_code;
