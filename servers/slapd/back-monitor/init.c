...
static struct monitor_subsys_t known_monitor_subsys[] = {
    ...
    },
    {
        ...
        BER_BVNULL,
        BER_BVNULL,
        {BER_BVC("This subsystem contains statistics."), BER_BVNULL},
        MONITOR_F_PERSISTENT_CH,
    ...
    },
    {
        ...
        BER_BVNULL,
        BER_BVNULL,
        {BER_BVC("This subsystem contains information about TLS."), BER_BVNULL},
        MONITOR_F_NONE,
        NULL, /* init */
        NULL, /* destroy */
        NULL, /* update */
...
int monitor_subsys_is_opened(void) { return monitor_subsys_opened; }

int monitor_back_register_subsys(monitor_subsys_t *ms) {
  int i = 0;
  ...
  mss = ch_realloc(monitor_subsys, (2 + i) * sizeof(monitor_subsys_t *));
...
int monitor_back_is_configured(void) { return be_monitor != NULL; }

int monitor_back_register_subsys_late(monitor_subsys_t *ms) {
  entry_limbo_t **elpp, el = {0};
  monitor_info_t *mi;

  if (be_monitor == NULL) {
    Debug(LDAP_DEBUG_ANY, "monitor_back_register_subsys_late: "
...
int monitor_back_register_backend(BackendInfo *bi) { return -1; }

int monitor_back_register_overlay_info(slap_overinst *on) { return -1; }
...
int monitor_back_register_database_limbo(BackendDB *be, struct berval *ndn_out) {
  ...
  if (be_monitor == NULL) {
    Debug(LDAP_DEBUG_ANY, "monitor_back_register_database_limbo: "
                          "monitor database not configured.\n");
    return -1;
  }
...
int monitor_back_register_overlay_limbo(BackendDB *be, struct slap_overinst *on, struct berval *ndn_out) {
  ...
  if (be_monitor == NULL) {
    Debug(LDAP_DEBUG_ANY, "monitor_back_register_overlay_limbo: "
                          "monitor database not configured.\n");
    return -1;
  }
...
int monitor_back_register_entry(Entry *e, monitor_callback_t *cb, monitor_subsys_t *mss, unsigned long flags) {
  ...
  if (monitor_subsys_is_opened()) {
    Entry *e_parent = NULL, *e_new = NULL, **ep = NULL;
    struct berval pdn = BER_BVNULL;
    monitor_entry_t *mp = NULL, *mp_parent = NULL;
    int rc = 0;
...
int monitor_back_register_entry_parent(Entry *e, monitor_callback_t *cb, monitor_subsys_t *mss, unsigned long flags,
  ...
  if (be_monitor == NULL) {
    Debug(LDAP_DEBUG_ANY,
          "monitor_back_register_entry_parent(base=\"%s\" scope=%s "
          "filter=\"%s\"): "
          "monitor database not configured.\n",
          BER_BVISNULL(nbase) ? "" : nbase->bv_val, ldap_pvt_scope2str(scope),
  ...
  if (BER_BVISNULL(filter)) {
    /* need a filter */
    Debug(LDAP_DEBUG_ANY, "monitor_back_register_entry_parent(\"\"): "
                          "need a valid filter\n");
    return -1;
  }
  ...
  if (monitor_subsys_is_opened()) {
    Entry *e_parent = NULL, *e_new = NULL, **ep = NULL;
    struct berval e_name = BER_BVNULL, e_nname = BER_BVNULL;
    monitor_entry_t *mp = NULL, *mp_parent = NULL;
    int rc = 0;
...
static int monitor_search2ndn_cb(Operation *op, SlapReply *rs) {
...
int monitor_back_register_entry_attrs(struct berval *ndn_in, Attribute *a, monitor_callback_t *cb, struct berval *nbase,
  ...
  if ((ndn_in == NULL || BER_BVISNULL(&ndn)) && BER_BVISNULL(filter)) {
    /* need a filter */
    Debug(LDAP_DEBUG_ANY,
 ...
 */
int monitor_back_unregister_entry(struct berval *ndn) {
  monitor_info_t *mi;
  ...
  if (monitor_subsys_is_opened()) {
    ...
    for (elpp = &mi->mi_entry_limbo; *elpp; elpp = &(*elpp)->el_next) {
      ...
      if (elp->el_type == LIMBO_ENTRY && dn_match(ndn, &elp->el_e->e_nname)) {
...
int monitor_back_unregister_entry_parent(struct berval *nrdn, monitor_callback_t *target_cb, struct berval *nbase,
  ...
  if (be_monitor == NULL) {
    Debug(LDAP_DEBUG_ANY,
          "monitor_back_unregister_entry_parent(base=\"%s\" scope=%s "
          "filter=\"%s\"): "
          "monitor database not configured.\n",
          BER_BVISNULL(nbase) ? "" : nbase->bv_val, ldap_pvt_scope2str(scope),
          BER_BVISNULL(filter) ? "" : filter->bv_val);
  ...
  if ((nrdn == NULL || BER_BVISNULL(nrdn)) && BER_BVISNULL(filter)) {
    /* need a filter */
    Debug(LDAP_DEBUG_ANY, "monitor_back_unregister_entry_parent(\"\"): "
                          "need a valid filter\n");
    return -1;
  }
  ...
  if (monitor_subsys_is_opened()) {
    ...
    if (target_a != NULL) {
      ...
      for (a = target_a; a != NULL; a = a->a_next) {
        ...
        (void)modify_delete_values(e, &mod, 1, &text, textbuf, sizeof(textbuf));
      }
    }
...
monitor_subsys_t *monitor_back_get_subsys_by_dn(struct berval *ndn, int sub) {
  if (monitor_subsys != NULL) {
    int i;
...
int monitor_back_initialize(BackendInfo *bi) {
  ...
  struct m_s {
    ...
    slap_mask_t flags;
    int offset;
  } moc[] = {{"( 1.3.6.1.4.1.4203.666.3.16.1 "
              "NAME 'monitor' "
  ...
  static struct {
    char *name;
    char *oid;
  } s_oid[] = {{"olmAttributes", "1.3.6.1.4.1.4203.666.1.55"},
  ...
  for (i = 0; mat[i].schema; i++) {
    ...
    *ad = NULL;
    ...
    if (code) {
      Debug(LDAP_DEBUG_ANY, "monitor_back_db_init: register_at failed\n");
      return -1;
    }
  ...
  for (i = 0; moc[i].schema; i++) {
    int code;
    ObjectClass **Oc = ((ObjectClass **)&(((char *)mi)[moc[i].offset]));

    code = register_oc(moc[i].schema, Oc, 0);
    if (code) {
      Debug(LDAP_DEBUG_ANY, "monitor_back_db_init: register_oc failed\n");
      return -1;
    }
...
int monitor_back_db_init(BackendDB *be, ConfigReply *c) {
  int rc;
  struct berval dn = BER_BVC(SLAPD_MONITOR_DN), pdn, ndn;
  BackendDB *be2;
...
static void monitor_back_destroy_limbo_entry(entry_limbo_t *el, int dispose) {
  if (el->el_e) {
    entry_free(el->el_e);
  }
  if (el->el_a) {
...
int monitor_back_db_open(BackendDB *be, ConfigReply *cr) {
   ...
   */
  e = monitor_entry_stub(NULL, NULL, &rdn, mi->mi_oc_monitorServer, NULL, NULL);
  ...
  for (i = 0; monitor_subsys[i] != NULL; i++) {
    ...
    free(dn.bv_val);
    if (rc != LDAP_SUCCESS) {
      Debug(LDAP_DEBUG_ANY, "monitor RDN \"%s\" is invalid\n", dn.bv_val);
      return (-1);
    }
...
int monitor_back_db_destroy(BackendDB *be, ConfigReply *cr) {
  monitor_info_t *mi = (monitor_info_t *)be->be_private;
