...
int monitor_entry_update(Operation *op, SlapReply *rs, Entry *e) {
...
int monitor_entry_modify(Operation *op, SlapReply *rs, Entry *e) {
  monitor_info_t __maybe_unused *mi = (monitor_info_t *)op->o_bd->be_private;
  monitor_entry_t *mp;
...
int monitor_entry_test_flags(monitor_entry_t *mp, int cond) {
  assert(mp != NULL);

  return ((mp->mp_flags & cond) || (mp->mp_info->mss_flags & cond));
}

monitor_entry_t *monitor_back_entrypriv_create(void) {
  monitor_entry_t *mp;
