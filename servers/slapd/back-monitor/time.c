...
int monitor_subsys_time_init(BackendDB *be, monitor_subsys_t *ms) {
  monitor_info_t *mi;
  ...
  if (monitor_cache_get(mi, &ms->mss_ndn, &e_time)) {
  ...
  *ep = e;
  ...
  BER_BVSTR(&value, "0");
  attr_merge_normalize_one(e, mi->mi_ad_monitoredInfo, &value, NULL);
...
static int monitor_subsys_time_update(Operation *op, SlapReply *rs, Entry *e) {
  ...
  if (dn_match(&rdn, &bv_current)) {
    ...
    ldap_pvt_gmtime(ldap_now_steady(), &tm);
    ...
    diff = (ldap_now_steady().ns - starttime.ns) * 1e-9;
    bv.bv_len = snprintf(buf, sizeof(buf), "%lu", (unsigned long)diff);
    bv.bv_val = buf;
