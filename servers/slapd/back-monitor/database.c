...
static struct restricted_ops_t {
  struct berval op;
  unsigned int tag;
} restricted_ops[] = {{BER_BVC("add"), SLAP_RESTRICT_OP_ADD},
...
static int monitor_subsys_database_init_one(monitor_info_t *mi, BackendDB *be,
  ...
  if (oi != NULL) {
    slap_overinst *on = oi->oi_list, *on1 = on;
    ...
    for (; on; on = on->on_next) {
      ...
      ber_str2bv(on->on_bi.bi_type, 0, 0, &bv);
      attr_merge_normalize_one(e, mi->mi_ad_monitorOverlay, &bv, NULL);
...
static int monitor_back_register_database_and_overlay(BackendDB *be,
  ...
  int i, rc;
  monitor_entry_t *mp;
  monitor_subsys_t *ms_backend, *ms_database, *ms_overlay;
  struct berval bv;
  char buf[BACKMONITOR_BUFSIZE];
  ...
  if (ms_backend == NULL) {
    Debug(LDAP_DEBUG_ANY, "monitor_back_register_database: "
                          "unable to get "
    ...
    if (a) {
      ...
      for (j = 0; !BER_BVISNULL(&a->a_vals[j]); j++) {
        for (k = 0; !BER_BVISNULL(&be->be_suffix[k]); k++) {
          if (dn_match(&a->a_vals[j], &be->be_suffix[k])) {
            rc = 0;
            goto done;
          }
        }
...
int monitor_back_register_database(BackendDB *be, struct berval *ndn_out) {
...
int monitor_subsys_database_init(BackendDB *be, monitor_subsys_t *ms) {
  monitor_info_t *mi;
  Entry *e_database, **ep;
  int i, rc;
  monitor_entry_t *mp;
  monitor_subsys_t *ms_backend, *ms_overlay;
  struct berval bv;
  ...
  ms_backend = monitor_back_get_subsys(SLAPD_MONITOR_BACKEND_NAME);
  if (ms_backend == NULL) {
    Debug(LDAP_DEBUG_ANY, "monitor_subsys_database_init: "
                          "unable to get "
                          "\"" SLAPD_MONITOR_BACKEND_NAME "\" "
                          "subsystem\n");
    return -1;
 ...
 */
static int value_mask(BerVarray v, slap_mask_t cur, slap_mask_t *delta) {
...
static int monitor_back_add_plugin(monitor_info_t *mi, Backend *be,
  ...
  do {
    ...
    rc = slapi_pblock_get(pCurrentPB, SLAPI_PLUGIN_DESCRIPTION, &srchdesc);
    if (rc != LDAP_SUCCESS) {
      goto done;
    }
    if (srchdesc) {
    ...
    ber_str2bv(buf, 0, 0, &bv);
    attr_merge_normalize_one(e_database, mi->mi_ad_monitoredInfo, &bv, NULL);
