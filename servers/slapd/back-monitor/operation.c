...
struct monitor_ops_t {
  struct berval rdn;
  struct berval nrdn;
} monitor_op[] = {{BER_BVC("cn=Bind"), BER_BVNULL},
...
static int monitor_subsys_ops_destroy(BackendDB *be, monitor_subsys_t *ms);

static int monitor_subsys_ops_update(Operation *op, SlapReply *rs, Entry *e);

int monitor_subsys_ops_init(BackendDB *be, monitor_subsys_t *ms) {
  monitor_info_t *mi;
  ...
  if (monitor_cache_get(mi, &ms->mss_ndn, &e_op)) {
  ...
  for (i = 0; i < SLAP_OP_LAST; i++) {
    ...
    if (e == NULL) {
      Debug(LDAP_DEBUG_ANY,
            ...
            monitor_op[i].rdn.bv_val, ms->mss_ndn.bv_val);
      goto bailout;
    }
...
static int monitor_subsys_ops_update(Operation *op, SlapReply *rs, Entry *e) {
  ...
  ldap_pvt_mp_t nInitiated = LDAP_PVT_MP_INIT, nCompleted = LDAP_PVT_MP_INIT;
  struct berval rdn;
  int i;
  Attribute *a;
  slap_counters_t *sc;
