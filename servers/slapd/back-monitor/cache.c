 ...
 */
int monitor_cache_cmp(const void *c1, const void *c2) {
 ...
 */
int monitor_cache_dup(void *c1, void *c2) {
  monitor_cache_t *cc1 = (monitor_cache_t *)c1;
  monitor_cache_t *cc2 = (monitor_cache_t *)c2;
 ...
 */
int monitor_cache_add(monitor_info_t *mi, Entry *e) {
  monitor_cache_t *mc;
 ...
 */
int monitor_cache_lock(Entry *e) {
  monitor_entry_t *mp;
 ...
 */
int monitor_cache_trylock(Entry *e) {
  monitor_entry_t *mp;
 ...
 * with mutex locked
 */
int monitor_cache_get(monitor_info_t *mi, struct berval *ndn, Entry **ep) {
 ...
 * with mutex locked
 */
int monitor_cache_remove(monitor_info_t *mi, struct berval *ndn, Entry **ep) {
  monitor_cache_t tmp_mc, *mc;
  struct berval pndn;
  ...
  if (mc != NULL) {
    ...
    if (pmc != NULL) {
      ...
      if (entryp == NULL) {
        ...
        tmp_mc.mc_ndn = *ndn;
        tmpmc = avl_delete(&mi->mi_cache, (caddr_t)&tmp_mc, monitor_cache_cmp);
        assert(tmpmc == mc);
 ...
 * destroyed.
 */
int monitor_cache_release(monitor_info_t *mi, Entry *e) {
  monitor_entry_t *mp;
  ...
  if (mp->mp_flags & MONITOR_F_VOLATILE) {
    ...
    mc = avl_delete(&mi->mi_cache, (caddr_t)&tmp_mc, monitor_cache_cmp);
    ldap_pvt_thread_mutex_unlock(&mi->mi_cache_mutex);
    if (mc != NULL) {
      ch_free(mc);
    }
...
static void monitor_entry_destroy(void *v_mc) {
...
int monitor_cache_destroy(monitor_info_t *mi) {
  if (mi->mi_cache) {
