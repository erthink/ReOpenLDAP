...
static int monitor_subsys_log_open(BackendDB *be, monitor_subsys_t *ms);

static int monitor_subsys_log_modify(Operation *op, SlapReply *rs, Entry *e);
 ...
 */
int monitor_subsys_log_init(BackendDB *be, monitor_subsys_t *ms) {
 ...
 * opens log subentry
 */
int monitor_subsys_log_open(BackendDB *be, monitor_subsys_t *ms) {
  BerVarray bva = NULL;
...
static int check_constraints(Modification *mod, int *newlevel) {
  int i;
  ...
  for (i = 0; !BER_BVISNULL(&mod->sm_values[i]); i++) {
    ...
    memcpy(mod->sm_values[i].bv_val, bv.bv_val, bv.bv_len);
...
static int add_values(Operation *op, Entry *e, Modification *mod,
  ...
  /* no - add them */
  rc = attr_merge_normalize(e, mod->sm_desc, mod->sm_values, op->o_tmpmemctx);
  if (rc != LDAP_SUCCESS) {
    return rc;
  }
