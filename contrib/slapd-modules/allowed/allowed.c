...
static int aa_operational(Operation *op, SlapReply *rs) {
  Attribute *a, **ap;
...
#define GOT_ALL (GOT_C | GOT_CE | GOT_A | GOT_AE)
  ...
  if (atp != NULL) {
    BerVarray bv_allowed = NULL, bv_effective = NULL;
    int i, ja = 0, je = 0;
    ...
    for (i = 0, ja = 0, je = 0; atp[i] != NULL; i++) {
      ...
      if (got & GOT_AE) {
        ...
        if (access_allowed(op, rs->sr_entry, ad, NULL, ACL_WRITE, NULL)) {
...
static int aa_initialize(void) {
  ...
  for (i = 0; aa_attrs[i].at; i++) {
    ...
    if (code) {
      Debug(LDAP_DEBUG_ANY, "aa_initialize: register_at failed\n");
      return -1;
    }
  }
