...
#include "../slapd/slapconfig.h" /* not nss-ldapd slapconfig.h */
...
void nssov_find_rdnval(struct berval *dn, AttributeDescription *ad, struct berval *value) {
  ...
  do {
    next = ber_bvchr(&rdn, '+');
    if (rdn.bv_val[ad->ad_cname.bv_len] == '=' && !ber_bvcmp(&rdn, &ad->ad_cname)) {
      if (next)
...
int nssov_filter_byname(nssov_mapinfo *mi, int key, struct berval *name, struct berval *buf) {
  ...
  if (nssov_escape(name, &bv2))
    return -1;
  /* build filter */
  if (bv2.bv_len + mi->mi_filter.bv_len + mi->mi_attrs[key].an_desc->ad_cname.bv_len + 6 > buf->bv_len)
    return -1;
  buf->bv_len = snprintf(buf->bv_val, buf->bv_len, "(&%s(%s=%s))", mi->mi_filter.bv_val,
                         mi->mi_attrs[key].an_desc->ad_cname.bv_val, bv2.bv_val);
  return 0;
}
...
int nssov_filter_byid(nssov_mapinfo *mi, int key, struct berval *id, struct berval *buf) {
  /* build filter */
  if (id->bv_len + mi->mi_filter.bv_len + mi->mi_attrs[key].an_desc->ad_cname.bv_len + 6 > buf->bv_len)
    return -1;
  buf->bv_len = snprintf(buf->bv_val, buf->bv_len, "(&%s(%s=%s))", mi->mi_filter.bv_val,
                         mi->mi_attrs[key].an_desc->ad_cname.bv_val, id->bv_val);
  return 0;
}
...
static void handleconnection(nssov_info *ni, int sock, Operation *op) {
  ...
  uid_t uid;
  gid_t gid;
  char authid[sizeof("gidNumber=4294967295+uidNumber=424967295,cn=peercred,cn="
                     "external,cn=auth")];
  ...
  op->o_dn.bv_len = sprintf(authid, "gidNumber=%d+uidNumber=%d,cn=peercred,cn=external,cn=auth", (int)gid, (int)uid);
  ...
  if ((fp = tio_fdopen(sock, readtimeout, writetimeout, READBUFFER_MINSIZE, READBUFFER_MAXSIZE, WRITEBUFFER_MINSIZE,
  ...
  case NSLCD_ACTION_ALIAS_BYNAME:
    (void)nssov_alias_byname(ni, fp, op);
    break;
  case NSLCD_ACTION_ALIAS_ALL:
    (void)nssov_alias_all(ni, fp, op);
    break;
  case NSLCD_ACTION_ETHER_BYNAME:
    (void)nssov_ether_byname(ni, fp, op);
    break;
  case NSLCD_ACTION_ETHER_BYETHER:
    (void)nssov_ether_byether(ni, fp, op);
    break;
  case NSLCD_ACTION_ETHER_ALL:
    (void)nssov_ether_all(ni, fp, op);
    break;
  case NSLCD_ACTION_GROUP_BYNAME:
    (void)nssov_group_byname(ni, fp, op);
    break;
  case NSLCD_ACTION_GROUP_BYGID:
    (void)nssov_group_bygid(ni, fp, op);
    break;
  case NSLCD_ACTION_GROUP_BYMEMBER:
    (void)nssov_group_bymember(ni, fp, op);
    break;
  case NSLCD_ACTION_GROUP_ALL:
    (void)nssov_group_all(ni, fp, op);
    break;
  case NSLCD_ACTION_HOST_BYNAME:
    (void)nssov_host_byname(ni, fp, op);
    break;
  case NSLCD_ACTION_HOST_BYADDR:
    (void)nssov_host_byaddr(ni, fp, op);
    break;
  case NSLCD_ACTION_HOST_ALL:
    (void)nssov_host_all(ni, fp, op);
    break;
  case NSLCD_ACTION_NETGROUP_BYNAME:
    (void)nssov_netgroup_byname(ni, fp, op);
    break;
  case NSLCD_ACTION_NETWORK_BYNAME:
    (void)nssov_network_byname(ni, fp, op);
    break;
  case NSLCD_ACTION_NETWORK_BYADDR:
    (void)nssov_network_byaddr(ni, fp, op);
    break;
  case NSLCD_ACTION_NETWORK_ALL:
    (void)nssov_network_all(ni, fp, op);
    break;
  case NSLCD_ACTION_PASSWD_BYNAME:
    (void)nssov_passwd_byname(ni, fp, op);
    break;
  case NSLCD_ACTION_PASSWD_BYUID:
    (void)nssov_passwd_byuid(ni, fp, op);
    break;
  case NSLCD_ACTION_PASSWD_ALL:
    (void)nssov_passwd_all(ni, fp, op);
    break;
  case NSLCD_ACTION_PROTOCOL_BYNAME:
    (void)nssov_protocol_byname(ni, fp, op);
    break;
  case NSLCD_ACTION_PROTOCOL_BYNUMBER:
    (void)nssov_protocol_bynumber(ni, fp, op);
    break;
  case NSLCD_ACTION_PROTOCOL_ALL:
    (void)nssov_protocol_all(ni, fp, op);
    break;
  case NSLCD_ACTION_RPC_BYNAME:
    (void)nssov_rpc_byname(ni, fp, op);
    break;
  case NSLCD_ACTION_RPC_BYNUMBER:
    (void)nssov_rpc_bynumber(ni, fp, op);
    break;
  case NSLCD_ACTION_RPC_ALL:
    (void)nssov_rpc_all(ni, fp, op);
    break;
  case NSLCD_ACTION_SERVICE_BYNAME:
    (void)nssov_service_byname(ni, fp, op);
    break;
  case NSLCD_ACTION_SERVICE_BYNUMBER:
    (void)nssov_service_bynumber(ni, fp, op);
    break;
  case NSLCD_ACTION_SERVICE_ALL:
    (void)nssov_service_all(ni, fp, op);
    break;
  case NSLCD_ACTION_SHADOW_BYNAME:
    if (uid == 0)
      (void)nssov_shadow_byname(ni, fp, op);
    break;
  case NSLCD_ACTION_SHADOW_ALL:
    if (uid == 0)
      (void)nssov_shadow_all(ni, fp, op);
    break;
  case NSLCD_ACTION_PAM_AUTHC:
    (void)pam_authc(ni, fp, op, uid);
    break;
  case NSLCD_ACTION_PAM_AUTHZ:
    (void)pam_authz(ni, fp, op);
    break;
  case NSLCD_ACTION_PAM_SESS_O:
    if (uid == 0)
      (void)pam_sess_o(ni, fp, op);
    break;
  case NSLCD_ACTION_PAM_SESS_C:
    if (uid == 0)
      (void)pam_sess_c(ni, fp, op);
    break;
  case NSLCD_ACTION_PAM_PWMOD:
    (void)pam_pwmod(ni, fp, op, uid);
    break;
  case NSLCD_ACTION_CONFIG_GET:
    (void)nssov_config(ni, fp, op);
    break;
  default:
...
static slap_verbmasks nss_svcs[] = {
    {BER_BVC("aliases"), NM_alias},    {BER_BVC("ethers"), NM_ether},       {BER_BVC("group"), NM_group},
    {BER_BVC("hosts"), NM_host},       {BER_BVC("netgroup"), NM_netgroup},  {BER_BVC("networks"), NM_network},
    {BER_BVC("passwd"), NM_passwd},    {BER_BVC("protocols"), NM_protocol}, {BER_BVC("rpc"), NM_rpc},
    {BER_BVC("services"), NM_service}, {BER_BVC("shadow"), NM_shadow},      {BER_BVNULL, 0}};

static slap_verbmasks pam_opts[] = {{BER_BVC("userhost"), NI_PAM_USERHOST},
...
enum { NSS_SSD = 1, NSS_MAP, NSS_PAM, NSS_PAMGROUP, NSS_PAMSESS };

static ConfigDriver nss_cf_gen;

static ConfigTable nsscfg[] = {
    {"nssov-ssd", "service> <url", 3, 3, 0, ARG_MAGIC | NSS_SSD, nss_cf_gen,
     "(OLcfgCtAt:3.1 NAME 'olcNssSsd' "
     "DESC 'URL for searches in a given service' "
     "EQUALITY caseIgnoreMatch "
     "SYNTAX OMsDirectoryString )",
     NULL, NULL},
    {"nssov-map", "service> <orig> <new", 4, 4, 0, ARG_MAGIC | NSS_MAP, nss_cf_gen,
     "(OLcfgCtAt:3.2 NAME 'olcNssMap' "
     "DESC 'Map <service> lookups of <orig> attr to <new> attr' "
     "EQUALITY caseIgnoreMatch "
     "SYNTAX OMsDirectoryString )",
     NULL, NULL},
    {"nssov-pam", "options", 2, 0, 0, ARG_MAGIC | NSS_PAM, nss_cf_gen,
     "(OLcfgCtAt:3.3 NAME 'olcNssPam' "
    ...
    {"nssov-pam-defhost", "hostname", 2, 2, 0, ARG_OFFSET | ARG_BERVAL,
     ...
     NULL, NULL},
    {"nssov-pam-group-dn", "DN", 2, 2, 0, ARG_MAGIC | ARG_DN | NSS_PAMGROUP, nss_cf_gen,
     "(OLcfgCtAt:3.5 NAME 'olcNssPamGroupDN' "
    ...
    {"nssov-pam-group-ad", "attr", 2, 2, 0, ARG_OFFSET | ARG_ATDESC,
     ...
     "SYNTAX OMsDirectoryString SINGLE-VALUE )",
     NULL, NULL},
    {"nssov-pam-min-uid", "uid", 2, 2, 0, ARG_OFFSET | ARG_INT, (void *)offsetof(struct nssov_info, ni_pam_min_uid),
     "(OLcfgCtAt:3.7 NAME 'olcNssPamMinUid' "
     "DESC 'Minimum UID allowed to login' "
     "EQUALITY integerMatch "
     "SYNTAX OMsInteger SINGLE-VALUE )",
     NULL, NULL},
    {"nssov-pam-max-uid", "uid", 2, 2, 0, ARG_OFFSET | ARG_INT, (void *)offsetof(struct nssov_info, ni_pam_max_uid),
    ...
    {"nssov-pam-template", "name", 2, 2, 0, ARG_OFFSET | ARG_BERVAL,
     ...
     "SYNTAX OMsDirectoryString SINGLE-VALUE )",
     NULL, NULL},
    {"nssov-pam-session", "service", 2, 2, 0, ARG_MAGIC | NSS_PAMSESS, nss_cf_gen,
     "(OLcfgCtAt:3.11 NAME 'olcNssPamSession' "
     "DESC 'Services for which sessions will be recorded' "
     "EQUALITY caseIgnoreMatch "
     "SYNTAX OMsDirectoryString )",
     NULL, NULL},
    {"nssov-pam-password-prohibit-message", "password_prohibit_message", 2, 2, 0, ARG_OFFSET | ARG_BERVAL,
     ...
     "SYNTAX OMsDirectoryString SINGLE-VALUE )",
     NULL, NULL},
    {"nssov-pam-pwdmgr-dn", "pwdmgr_dn", 2, 2, 0, ARG_OFFSET | ARG_BERVAL,
     ...
     "SYNTAX OMsDN SINGLE-VALUE )",
     NULL, NULL},
    {"nssov-pam-pwdmgr-pwd", "pwdmgr_pwd", 2, 2, 0, ARG_OFFSET | ARG_BERVAL,
...
static ConfigOCs nssocs[] = {{"( OLcfgCtOc:3.1 "
                              "NAME 'olcNssOvConfig' "
...
static int nss_cf_gen(ConfigArgs *c) {
  ...
  if (c->op == SLAP_CONFIG_EMIT) {
    ...
    case NSS_SSD:
      ...
      for (i = NM_alias; i < NM_NONE; i++) {
        ...
        if (mi->mi_scope == LDAP_SCOPE_DEFAULT && bvmatch(&mi->mi_filter, &mi->mi_filter0) &&
        ...
        if (BER_BVISNULL(&mi->mi_base))
          ...
          base = mi->mi_base;
        ldap_pvt_scope2bv(mi->mi_scope == LDAP_SCOPE_DEFAULT ? LDAP_SCOPE_SUBTREE : mi->mi_scope, &scope);
        ssd.bv_len =
            STRLENOF(" ldap:///???") + nss_svcs[i].word.bv_len + base.bv_len + scope.bv_len + mi->mi_filter.bv_len;
        ssd.bv_val = ch_malloc(ssd.bv_len + 1);
        sprintf(ssd.bv_val, "%s ldap:///%s??%s?%s", nss_svcs[i].word.bv_val, base.bv_val, scope.bv_val,
                mi->mi_filter.bv_val);
    ...
    case NSS_MAP:
      ...
      for (i = NM_alias; i < NM_NONE; i++) {
        ...
        for (j = 0; !BER_BVISNULL(&mi->mi_attrkeys[j]); j++) {
          if (ber_bvstrcasecmp(&mi->mi_attrkeys[j], &mi->mi_attrs[j].an_name)) {
            struct berval map;

            map.bv_len =
                nss_svcs[i].word.bv_len + mi->mi_attrkeys[j].bv_len + mi->mi_attrs[j].an_desc->ad_cname.bv_len + 2;
            map.bv_val = ch_malloc(map.bv_len + 1);
            sprintf(map.bv_val, "%s %s %s", nss_svcs[i].word.bv_val, mi->mi_attrkeys[j].bv_val,
                    mi->mi_attrs[j].an_desc->ad_cname.bv_val);
  ...
  case NSS_SSD: {
    ...
    do {
      ...
      if ((lud->lud_host && *lud->lud_host) || lud->lud_attrs || lud->lud_exts) {
        rc = 1;
        break;
      }
  ...
  case NSS_PAM:
    ...
    if (i == 0) {
      ...
      if ((m & NI_PAM_USERHOST) && !nssov_pam_host_ad) {
        ...
        if (i != LDAP_SUCCESS) {
          snprintf(c->cr_msg, sizeof(c->cr_msg), "nssov: host attr unknown: %s", text);
      ...
      if ((m & (NI_PAM_USERSVC | NI_PAM_HOSTSVC)) && !nssov_pam_svc_ad) {
        ...
        if (i != LDAP_SUCCESS) {
          snprintf(c->cr_msg, sizeof(c->cr_msg), "nssov: authorizedService attr unknown: %s", text);
          Debug(LDAP_DEBUG_ANY, "%s\n", c->cr_msg);
          rc = 1;
          break;
        }
...
static int nssov_db_init(BackendDB *be, ConfigReply *cr) {
...
static int nssov_db_destroy(BackendDB *be, ConfigReply *cr) { return 0; }

static int nssov_db_open(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  nssov_info *ni = on->on_bi.bi_private;
  nssov_mapinfo *mi;
  ...
  for (i = 0; i < NM_NONE; i++, mi++) {
    ...
    for (j = 0; !BER_BVISNULL(&mi->mi_attrkeys[j]); j++) {
      ...
      if (mi->mi_attrs[j].an_desc)
        continue;
      if (slap_bv2ad(&mi->mi_attrs[j].an_name, &mi->mi_attrs[j].an_desc, &text)) {
  ...
  if ((ni->ni_pam_opts & (NI_PAM_USERSVC | NI_PAM_HOSTSVC)) && !nssov_pam_svc_ad) {
    const char *text;
    i = slap_str2ad("authorizedService", &nssov_pam_svc_ad, &text);
    if (i != LDAP_SUCCESS) {
  ...
  if (slapMode & SLAP_SERVER_MODE) {
    ...
    if (bind(sock, (struct sockaddr *)&addr, sizeof(struct sockaddr_un))) {
      Debug(LDAP_DEBUG_ANY, "nssov: bind() to " NSLCD_SOCKET " failed: %s", strerror(errno));
      if (close(sock))
        Debug(LDAP_DEBUG_ANY, "nssov: problem closing socket: %s", strerror(errno));
      return -1;
    }
...
static int nssov_db_close(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  nssov_info *ni = on->on_bi.bi_private;
  ...
  if (slapMode & SLAP_SERVER_MODE) {
    ...
    /* remove existing named socket */
    if (unlink(NSLCD_SOCKET) < 0) {
      Debug(LDAP_DEBUG_TRACE, "unlink() of " NSLCD_SOCKET " failed (ignored): %s", strerror(errno));
    }
  }
  return 0;
}
...
SLAP_MODULE_ENTRY(nssov, modinit)(int argc, char *argv[]) { return nssov_initialize(); }
#endif
