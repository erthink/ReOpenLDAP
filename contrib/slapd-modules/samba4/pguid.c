...
static int pguid_op_add(Operation *op, SlapReply *rs) {
  ...
  if (a == NULL) {
    Debug(LDAP_DEBUG_ANY,
          "%s: pguid_op_add: unable to find entryUUID of parent entry "
          "DN=\"%s\" (%d)\n",
...
static int pguid_op_rename(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
  ...
  if (a == NULL) {
    Debug(LDAP_DEBUG_ANY,
          "%s: pguid_op_rename: unable to find entryUUID of newSuperior entry "
          "DN=\"%s\" (%d)\n",
...
static int pguid_repair_cb(Operation *op, SlapReply *rs) {
  int rc;
  ...
  if (a == NULL) {
    Debug(LDAP_DEBUG_ANY,
          "%s: pguid_repair_cb: unable to find entryUUID of parent entry "
          "DN=\"%s\" (%d)\n",
          op->o_log_prefix, pdn.bv_val, rc);
...
static int pguid_repair(BackendDB *be) {
...
done_search:;
  ...
  Log(LDAP_DEBUG_STATS, LDAP_LEVEL_INFO, "pguid: repaired=%d\n", nrepaired);
...
static int pguid_initialize(void) {
  int code, i;

  for (i = 0; as[i].desc != NULL; i++) {
    code = register_at(as[i].desc, as[i].adp, 0);
    if (code) {
      Debug(LDAP_DEBUG_ANY, "pguid_initialize: register_at #%d failed\n", i);
      return code;
    }
