...
static int vernum_op_add(Operation *op, SlapReply *rs) {
...
static int vernum_op_modify(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
  vernum_t *vn = (vernum_t *)on->on_bi.bi_private;
...
static int vernum_db_init(BackendDB *be, ConfigReply *cr) {
...
static int vernum_repair_cb(Operation *op, SlapReply *rs) {
...
static int vernum_repair(BackendDB *be) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  vernum_t *vn = (vernum_t *)on->on_bi.bi_private;
...
done_search:;
  ...
  Log(LDAP_DEBUG_STATS, LDAP_LEVEL_INFO, "vernum: repaired=%d\n", nrepaired);

  return 0;
}

static int vernum_db_open(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  vernum_t *vn = (vernum_t *)on->on_bi.bi_private;
...
static int vernum_db_destroy(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  vernum_t *vn = (vernum_t *)on->on_bi.bi_private;
...
static int vernum_initialize(void) {
  int code, i;

  for (i = 0; as[i].desc != NULL; i++) {
    code = register_at(as[i].desc, as[i].adp, 0);
    if (code) {
      Debug(LDAP_DEBUG_ANY, "vernum_initialize: register_at #%d failed\n", i);
      return code;
    }
