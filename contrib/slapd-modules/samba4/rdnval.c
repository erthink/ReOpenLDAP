...
static int rdnval_is_valid(AttributeDescription *desc, struct berval *value) {
  if (desc->ad_type->sat_syntax == slap_schema.si_syn_directoryString) {
    return 1;
  }

  if (desc->ad_type->sat_syntax == syn_IA5String && !BER_BVISEMPTY(value)) {
    return 1;
  }
...
static int rdnval_unique_check_cb(Operation *op, SlapReply *rs) {
...
static int rdnval_unique_check(Operation *op, BerVarray vals) {
  ...
  if (op->o_tag == LDAP_REQ_ADD && be_issuffix(op->o_bd, &op->o_req_ndn)) {
    return LDAP_SUCCESS;
  }
  ...
  fvals = op->o_tmpcalloc(sizeof(struct berval), i + 1, op->o_tmpmemctx);
...
static int rdnval_op_add(Operation *op, SlapReply *rs) {
  Attribute *a, **ap;
  int numvals = 0;
...
static int rdnval_op_rename(Operation *op, SlapReply *rs) {
...
static int rdnval_repair_cb(Operation *op, SlapReply *rs) {
  int rc;
...
static int rdnval_repair(BackendDB *be) {
...
done_search:;
  ...
  Log(LDAP_DEBUG_STATS, LDAP_LEVEL_INFO, "rdnval: repaired=%d\n", nrepaired);
