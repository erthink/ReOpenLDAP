...
typedef struct autogroup_sc_t {
  ...
  autogroup_def_t
      *ags_def; /* Attributes definition of the group being added. */
} autogroup_sc_t;

/* Used for adding members, found when searching, to a group. */
typedef struct autogroup_ga_t {
  autogroup_entry_t
      *agg_group; /* The group to which the members will be added. */
  ...
  Modifications
      *agg_mod; /* Used in autogroup_member_search_modify_cb to hold the
                search results which will be added to the group. */

  Modifications
      *agg_mod_last; /* Used in autogroup_member_search_modify_cb so we don't
...
*/
static int autogroup_add_member_to_group(Operation *op, BerValue *dn,
                                         BerValue *ndn,
...
*/
static int autogroup_add_member_values_to_group(Operation *op,
                                                struct berval *dn,
                                                autogroup_entry_t *age,
                                                Attribute *attr) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
...
*/
static int autogroup_delete_member_from_group(Operation *op, BerValue *dn,
                                              BerValue *ndn,
                                              autogroup_entry_t *age) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
  Modifications *modlist = (Modifications *)ch_calloc(1, sizeof(Modifications));
  ...
  if (dn == NULL || ndn == NULL) {
    Debug(LDAP_DEBUG_TRACE,
          "==> autogroup_delete_member_from_group removing all members from "
          "<%s>\n",
          age->age_dn.bv_val);
...
*/
static int autogroup_delete_member_values_from_group(Operation *op,
                                                     struct berval *dn,
                                                     autogroup_entry_t *age,
                                                     Attribute *attr) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
...
*/
static int autogroup_member_search_cb(Operation *op, SlapReply *rs) {
  ...
  if (rs->sr_type == REP_SEARCH) {
    ...
    if (agf->agf_anlist) {
      Attribute *attr =
          attrs_find(rs->sr_entry->e_attrs, agf->agf_anlist[0].an_desc);
      if (attr) {
        vals = attr->a_vals;
        nvals = attr->a_nvals;
    ...
    modify_add_values(agg->agg_entry, &mod, /* permissive */ 1, &text, textbuf,
                      sizeof(textbuf));
  }
...
** NOTE: Very slow.
*/
static int autogroup_member_search_modify_cb(Operation *op, SlapReply *rs) {
  assert(op->o_tag == LDAP_REQ_SEARCH);
...
** If modify == 1, then we modify the group's entry in the database using
*be_modify.
...
** agf	- the filter
*/
static int autogroup_add_members_from_filter(Operation *op, Entry *e,
                                             autogroup_entry_t *age,
...
**	or to modify the group entry in the database (when modify == 1 and e =
*NULL and ndn != NULL).
** agi	- pointer to the groups and the attribute definitions
** agd - the attribute definition of the added group
** e	- the entry representing the group, can be NULL if the ndn is specified,
*and modify == 1
** ndn	- the DN of the group, can be NULL if we give a non-NULL e
*/
static int autogroup_add_group(Operation *op, autogroup_info_t *agi,
                               autogroup_def_t *agd, Entry *e, BerValue *ndn,
                               int scan, int modify) {
  ...
  if (e == NULL) {
    if (overlay_entry_get_ov(op, ndn, NULL, NULL, 0, &e, on) != LDAP_SUCCESS ||
        e == NULL) {
      Debug(LDAP_DEBUG_TRACE,
      ...
      if (lud->lud_attrs != NULL) {
        ...
        if (i > 1) {
          Debug(LDAP_DEBUG_ANY,
                "autogroup_add_group: too many attributes specified in url "
                "<%s>\n",
                bv->bv_val);
          /* FIXME: error? */
...
*/
static int autogroup_group_add_cb(Operation *op, SlapReply *rs) {
  assert(op->o_tag == LDAP_REQ_SEARCH);
  ...
  if (rs->sr_type == REP_SEARCH) {
    ...
    autogroup_add_group(op, ags->ags_info, ags->ags_def, rs->sr_entry, NULL, 0,
                        0);
  }
...
static int autogroup_add_entry_cb(Operation *op, SlapReply *rs) {
      ...
      for (agf = age->age_filter; agf; agf = agf->agf_next) {
        if (dnIsSuffix(&op->o_req_ndn, &agf->agf_ndn)) {
          ...
          if (rc == LDAP_COMPARE_TRUE) {
            if (agf->agf_anlist) {
              Attribute *a =
                  attr_find(aa->e->e_attrs, agf->agf_anlist[0].an_desc);
              if (a)
                autogroup_add_member_values_to_group(op, &op->o_req_dn, age, a);
            } else {
              autogroup_add_member_to_group(op, &aa->e->e_name, &aa->e->e_nname,
                                            age);
            }
            break;
          }
...
*/
static int autogroup_add_entry(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
  autogroup_info_t *agi = (autogroup_info_t *)on->on_bi.bi_private;
  ...
  sc = op->o_tmpcalloc(1, sizeof(slap_callback) + sizeof(ag_addinfo),
                       op->o_tmpmemctx);
  sc->sc_private = (sc + 1);
  ...
  for (; agd; agd = agd->agd_next) {
    if (is_entry_objectclass_or_sub(op->ora_e, agd->agd_oc)) {
      ...
      modify_delete_values(op->ora_e, &mod, /* permissive */ 1, &text, textbuf,
                           sizeof(textbuf));
...
*/
static int autogroup_delete_group(autogroup_info_t *agi, autogroup_entry_t *e) {
  autogroup_entry_t *age = agi->agi_entry, *age_prev = NULL, *age_next;
  int rc = 1;
  ...
  for (age_next = age; age_next; age_prev = age, age = age_next) {
    ...
    if (age == e) {
      autogroup_filter_t *agf = age->age_filter, *agf_next;
...
static int autogroup_delete_entry(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
  autogroup_info_t *agi = (autogroup_info_t *)on->on_bi.bi_private;
  ...
  for (age = agi->agi_entry; age; age = age->age_next) {
    ...
    for (agf = age->age_filter; agf; agf = agf->agf_next) {
      if (dnIsSuffix(&op->o_req_ndn, &agf->agf_ndn)) {
        ...
        if (rc == LDAP_COMPARE_TRUE) {
          /* If the attribute is retrieved from the entry, we don't know what to
           *delete
          ...
          if (agf->agf_anlist) {
            ...
            autogroup_delete_member_from_group(op, &e->e_name, &e->e_nname,
                                               age);
          }
          break;
        }
...
static int autogroup_response(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
  autogroup_info_t *agi = (autogroup_info_t *)on->on_bi.bi_private;
  autogroup_def_t *agd = agi->agi_def;
  autogroup_entry_t *age;
  ...
  if (op->o_tag == LDAP_REQ_DELETE || op->o_tag == LDAP_REQ_MODIFY) {
    ...
    if (rs->sr_type == REP_RESULT && rs->sr_err == LDAP_SUCCESS && !oex) {
      ...
      /* If a groups DN is modified, just update age_dn/ndn of that group with
       * the new DN. */
      for (; agd; agd = agd->agd_next) {
      ...
      /* For each group:
         ...
         2. check if the any of the group filter's base DN is a suffix of the
         new DN

         If 1 and 2 are both false, we do nothing.
         If 1 and 2 is true, we remove the old DN from the group, and add the
         new DN. If 1 is false, and 2 is true, we check the entry against the
         group's filters, and add it's DN to the group. If 1 is true, and 2 is
         false, we delete the entry's DN from the group.
      */
      ...
      for (age = agi->agi_entry; age; age = age->age_next) {
        ...
        if (age->age_modrdn_olddnmodified) {
          ...
          if (overlay_entry_get_ov(op, &age->age_ndn, NULL, NULL, 0, &group,
                                   on) != LDAP_SUCCESS ||
              group == NULL) {
            Debug(LDAP_DEBUG_TRACE,
          ...
          if (a != NULL) {
            if (value_find_ex(age->age_def->agd_member_ad,
                              SLAP_MR_ATTRIBUTE_VALUE_NORMALIZED_MATCH |
                                  SLAP_MR_ASSERTED_VALUE_NORMALIZED_MATCH,
                              a->a_nvals, ea ? ea->a_nvals : &op->o_req_ndn,
                              op->o_tmpmemctx) == 0) {
              is_olddn = 1;
            }
          }
        ...
        if (is_olddn == 1 && is_newdn == 0) {
          if (ea)
            ...
            autogroup_delete_member_from_group(op, &op->o_req_dn,
                                               &op->o_req_ndn, age);
        } else if (is_olddn == 0 && is_newdn == 1) {
          Entry etmp;
          struct berval odn, ondn;
          ...
          op->o_dn = odn;
          op->o_ndn = ondn;
        } else if (is_olddn == 1 && is_newdn == 1 && dn_equal != 0) {
          if (ea) {
            /* group refresh */
            autogroup_delete_member_from_group(op, NULL, NULL, age);
  ...
  if (op->o_tag == LDAP_REQ_MODIFY) {
    if (rs->sr_type == REP_RESULT && rs->sr_err == LDAP_SUCCESS && !oex) {
      ...
      /* If we modify a group's memberURL, we have to delete all of it's
         members,
         and add them anew, because we cannot tell from which memberURL a member
         was added. */
      for (; agd; agd = agd->agd_next) {
            ...
            if (match == 0) {
              for (; m; m = m->sml_next) {
                if (m->sml_desc == age->age_def->agd_member_url_ad) {
                  ...
                  Debug(LDAP_DEBUG_TRACE,
                        "autogroup_response MODIFY changing memberURL for "
                        "group <%s>\n",
                        op->o_req_dn.bv_val);
                  ...
                  autogroup_add_group(op, agi, group_agd, NULL, &op->o_req_ndn,
                                      1, 1);
      ...
      /* When modifying any of the attributes of an entry, we must
         ...
         If the entry exists in a group, but the modified attributes do
              not match any of the group's filters, we delete the entry from
         that group. If the entry doesn't exist in a group, but matches a
         filter, we add it to that group.
      */
      attrs = attrs_dup(e->e_attrs);
      overlay_entry_release_ov(op, e, 0, on);
      etmp.e_name = op->o_req_dn;
      ...
      for (age = agi->agi_entry; age; age = age->age_next) {
        ...
        if (is_olddn == 1 && is_newdn == 0) {
          if (ea)
            autogroup_delete_member_values_from_group(op, &op->o_req_dn, age,
                                                      ea);
          else
            autogroup_delete_member_from_group(op, &op->o_req_dn,
                                               &op->o_req_ndn, age);
          ...
          if (ea)
            ...
            autogroup_add_member_to_group(op, &op->o_req_dn, &op->o_req_ndn,
                                          age);
        }
...
*/
static int autogroup_memberOf_filter(Filter *f, BerValue *dn,
                                     AttributeDescription *memberof_ad) {
  int result = 0;
  if (f == NULL)
    return 0;
...
/*
** When modifying a group, we must deny any modifications to the member
*attribute,
** because the group would be inconsistent.
*/
static int autogroup_modify_entry(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
  autogroup_info_t *agi = (autogroup_info_t *)on->on_bi.bi_private;
  autogroup_def_t *agd = agi->agi_def;
  autogroup_entry_t *age;
  ...
  /* Must refresh groups if a matching member value is modified OR filter
   * contains memberOf=DN */
  for (age = agi->agi_entry; age; age = age->age_next) {
    autogroup_filter_t *agf;
    for (agf = age->age_filter; agf; agf = agf->agf_next) {
      if (agf->agf_anlist) {
      ...
      if (autogroup_memberOf_filter(agf->agf_filter, &op->o_req_ndn,
                                    agi->agi_memberof_ad)) {
        age->age_mustrefresh = 1;
      }
    }
        ...
        if (match == 0) {
          for (; m; m = m->sml_next) {
            if (m->sml_desc == age->age_def->agd_member_ad) {
              ...
              ldap_pvt_thread_mutex_unlock(&agi->agi_mutex);
              Debug(LDAP_DEBUG_TRACE,
                    "autogroup_modify_entry attempted to modify group's <%s> "
                    "member attribute\n",
                    op->o_req_dn.bv_val);
              send_ldap_error(
                  op, rs, LDAP_CONSTRAINT_VIOLATION,
...
/*
** Detect if the olddn is part of a group and so if the group should be
*refreshed
*/
static int autogroup_modrdn_entry(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
  autogroup_info_t *agi = (autogroup_info_t *)on->on_bi.bi_private;
  autogroup_entry_t *age;
  Entry *e;
...
*/
static int autogroup_build_def_filter(autogroup_def_t *agd, Operation *op) {
  char *ptr;
...
enum { AG_ATTRSET = 1, AG_MEMBER_OF_AD, AG_LAST };

static ConfigDriver ag_cfgen;

static ConfigTable agcfg[] = {
    {"autogroup-attrset", "group-oc> <URL-ad> <member-ad", 4, 4, 0,
     ARG_MAGIC | AG_ATTRSET, ag_cfgen,
     "( OLcfgCtAt:2.1 NAME 'olcAGattrSet' "
     "DESC 'Automatic groups: <group objectClass>, <URL attributeDescription>, "
    ...
    {"autogroup-memberof-ad", "memberOf attribute", 2, 2, 0,
...
static ConfigOCs agocs[] = {{"( OLcfgCtOc:2.1 "
...
static int ag_cfgen(ConfigArgs *c) {
  ...
  if (c->op == SLAP_CONFIG_EMIT) {
    ...
    if (c->valx < 0) {
      ...
      for (i = 0, agdp = &agi->agi_def; i < c->valx; i++) {
        if (*agdp == NULL) {
          return 1;
        }
        agdp = &(*agdp)->agd_next;
      ...
      *agdp = agd->agd_next;
      ...
      for (age_next = age, age_prev = NULL; age_next;
           age_prev = age, age = age_next) {
        age_next = age->age_next;
  ...
  switch (c->type) {
  case AG_ATTRSET: {
    autogroup_def_t **agdp, *agd_next = NULL;
    ObjectClass *oc = NULL;
    AttributeDescription *member_url_ad = NULL, *member_ad = NULL;
    const char *text;
    ...
    if (!is_at_subtype(member_url_ad->ad_type,
    ...
    for (agdp = &agi->agi_def; *agdp; agdp = &(*agdp)->agd_next) {
      ...
      if ((*agdp)->agd_member_url_ad == member_url_ad &&
  ...
  case AG_MEMBER_OF_AD: {
    ...
    if (!is_at_syntax(memberof_ad->ad_type, SLAPD_DN_SYNTAX) /* e.g. "member" */
        && !is_at_syntax(memberof_ad->ad_type,
...
*/
static int autogroup_db_open(BackendDB *be, ConfigReply *cr) {
...
static int autogroup_db_close(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  ...
  if (on->on_bi.bi_private) {
    autogroup_info_t *agi = on->on_bi.bi_private;
    autogroup_entry_t *age = agi->agi_entry, *age_next;
    autogroup_filter_t *agf, *agf_next;
...
static int autogroup_db_destroy(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  ...
  if (on->on_bi.bi_private) {
    autogroup_info_t *agi = on->on_bi.bi_private;
    autogroup_def_t *agd = agi->agi_def, *agd_next;
...
static int autogroup_initialize(void) {
  int rc = 0;
