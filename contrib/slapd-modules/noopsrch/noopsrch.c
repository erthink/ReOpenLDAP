...
static int noopsrch_parseCtrl(Operation *op, SlapReply *rs, LDAPControl *ctrl) {
...
static int noopsrch_response(Operation *op, SlapReply *rs) {
...
static int noopsrch_cleanup(Operation *op, SlapReply *rs) {
...
static int noopsrch_op_search(Operation *op, SlapReply *rs) {
  if (op->o_noopsrch != SLAP_CONTROL_NONE) {
    slap_callback *sc;
    noopsrch_cb_t *nc;
...
static int noopsrch_db_init(BackendDB *be, ConfigReply *cr) {
  if (noopsrch_cnt++ == 0) {
    int rc;
...
static int noopsrch_db_destroy(BackendDB *be, ConfigReply *cr) {
...
static int noopsrch_initialize(void) {
