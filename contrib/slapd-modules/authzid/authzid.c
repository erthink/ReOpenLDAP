...
static int authzid_conn_cmp(const void *c1, const void *c2) {
...
static int authzid_conn_dup(void *c1, void *c2) {
...
static authzid_conn_t *authzid_conn_find(Connection *c) {
...
static authzid_conn_t *authzid_conn_get(Connection *c) {
...
static void authzid_conn_release(authzid_conn_t *ac) {
  ldap_pvt_thread_mutex_lock(&authzid_mutex);
  ac->refcnt--;
  ldap_pvt_thread_mutex_unlock(&authzid_mutex);
}

static int authzid_conn_insert(Connection *c, char flag) {
  authzid_conn_t *ac;
  int rc;
...
static int authzid_conn_remove(Connection *c) {
...
static int authzid_response(Operation *op, SlapReply *rs) {
  LDAPControl **ctrls;
...
static int authzid_cleanup(Operation *op, SlapReply *rs) {
  if (rs->sr_ctrls) {
    LDAPControl *ctrl;
...
static int authzid_op_bind(Operation *op, SlapReply *rs) {
  slap_callback *sc;
...
static int parse_authzid_ctrl(Operation *op, SlapReply *rs, LDAPControl *ctrl) {
...
static int authzid_db_init(BackendDB *be, ConfigReply *cr) {
  if (!SLAP_ISGLOBALOVERLAY(be)) {
    /* do not allow slapo-ppolicy to be global by now (ITS#5858) */
    if (cr) {
      snprintf(cr->msg, sizeof(cr->msg), "slapo-authzid must be global");
 ...
 */
static int authzid_db_destroy(BackendDB *be, ConfigReply *cr) {
...
static int authzid_initialize(void) {
