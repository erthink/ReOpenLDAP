...
static int cloak_cfgen(ConfigArgs *c) {
...
static int cloak_search_response_cb(Operation *op, SlapReply *rs) {
  slap_callback *sc;
  cloak_info_t *ci;
  Entry *e = NULL;
  Entry *me = NULL;
  ...
  for (ci = (cloak_info_t *)sc->sc_private; ci; ci = ci->ci_next) {
    ...
    if (ci->ci_oc != NULL && !is_entry_objectclass_or_sub(e, ci->ci_oc))
      continue;
  ...
  for (ci = (cloak_info_t *)sc->sc_private; ci; ci = ci->ci_next) {
    ...
    for (pa = NULL, a = me->e_attrs; a; pa = a, a = a->a_next) {
...
static int cloak_search_cleanup_cb(Operation *op, SlapReply *rs) {
...
static int cloak_search(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
  cloak_info_t *ci = (cloak_info_t *)on->on_bi.bi_private;
  slap_callback *sc;

  if (op->ors_attrsonly || op->ors_attrs || get_manageDSAit(op))
    return SLAP_CB_CONTINUE;
...
static int cloak_db_destroy(BackendDB *be, ConfigReply *cr) {
...
static ConfigOCs cloakocs[] = {{"( OLcfgCtOc:4.1 "
                                "NAME 'olcCloakConfig' "
...
static int cloak_initialize(void) {
  int rc;
