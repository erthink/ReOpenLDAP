...
static int vc_conn_cmp(const void *c1, const void *c2) {
...
static int vc_conn_dup(void *c1, void *c2) {
...
static int vc_cb(Operation *op, SlapReply *rs) {
...
static int vc_exop(Operation *op, SlapReply *rs) {
  int rc = LDAP_SUCCESS;
  ber_tag_t tag;
  ber_len_t len = -1;
  BerElementBuffer berbuf;
  ...
  if (!BER_BVISNULL(&cookie)) {
    ...
    memcpy((char *)&tmp.conn, (const char *)cookie.bv_val, cookie.bv_len);
  ...
  if (conn) {
    if (conn->op->o_conn->c_sasl_bind_in_progress) {
      if (conn->conn == NULL) {
        ...
        conn->refcnt--;
        ldap_pvt_thread_mutex_lock(&vc_mutex);
        rc = avl_insert(&vc_tree, (caddr_t)conn, vc_conn_cmp, vc_conn_dup);
        ldap_pvt_thread_mutex_unlock(&vc_mutex);
        assert(rc == 0);
