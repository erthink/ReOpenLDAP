...
static int slapd_argon2_hash(const struct berval *scheme, const struct berval *passwd, struct berval *hash,
                             const char **text) {
  ...
  int rc = crypto_pwhash_argon2i_str(encoded.bv_val, passwd->bv_val, passwd->bv_len, iterations, memory);
...
static int slapd_argon2_verify(const struct berval *scheme, const struct berval *passwd, const struct berval *cred,
                               const char **text) {
...
SLAP_MODULE_ENTRY(pw_argon2, modinit)(int argc, char *argv[]) {
  ...
  if (rc == -1)
    return -1;
  return lutil_passwd_add((struct berval *)&slapd_argon2_scheme, slapd_argon2_verify, slapd_argon2_hash);
}
