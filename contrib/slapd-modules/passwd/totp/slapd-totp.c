...
static int totp_b32_pton(char const *src, u_char *target, size_t targsize) {
  ...
  while ((ch = *src++) != '\0') {
    ...
    case 1:
      if (target) {
        ...
        target[tarindex + 1] = ((pos - Base32) & 0x3) << 6;
      }
      tarindex++;
      state = 2;
      break;
    ...
    case 3:
      if (target) {
        ...
        target[tarindex + 1] = ((pos - Base32) & 0xf) << 4;
      }
      tarindex++;
      state = 4;
      break;
    case 4:
      if (target) {
        if ((size_t)tarindex + 1 >= targsize)
          return (-1);
        target[tarindex] |= (pos - Base32) >> 1;
        target[tarindex + 1] = ((pos - Base32) & 0x1) << 7;
      }
      tarindex++;
      state = 5;
      break;
    ...
    case 6:
      if (target) {
        if ((size_t)tarindex + 1 >= targsize)
          return (-1);
        target[tarindex] |= (pos - Base32) >> 3;
        target[tarindex + 1] = ((pos - Base32) & 0x7) << 5;
      }
      tarindex++;
      state = 7;
      break;
...
static void do_hmac(const void *hash, myval *key, myval *data, myval *out) {
  TOTP_HMAC_CTX ctx;
...
static int totp_op_bind(Operation *op, SlapReply *rs) {
...
static int totp_db_open(BackendDB *be, ConfigReply *cr) {
  int rc = 0;
