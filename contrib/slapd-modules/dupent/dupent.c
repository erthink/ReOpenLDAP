/* dupent.c - LDAP Control for a Duplicate Entry Representation of Search
 * Results */
/* $ReOpenLDAP$ */
...
static int dupent_parseCtrl(Operation *op, SlapReply *rs, LDAPControl *ctrl) {
  ber_tag_t tag;
...
DuplicateEntryRequest ::= SEQUENCE {
  ...
  if (cnt == 0) {
    ...
    for (i = 0, c = 0; i < cnt; i++) {
      ...
      if (bvmatch(&AttributeDescriptionList[i], slap_bv_all_user_attrs)) {
      ...
      for (j = 0; j < c; j++) {
        if (ds->ds_an[c].an_desc == ds->ds_an[j].an_desc) {
          rs->sr_text = "Dupent control: AttributeDescription must be unique "
                        "within AttributeDescriptionList";
          rs->sr_err = LDAP_PROTOCOL_ERROR;
          goto done;
        }
      }
...
static int dupent_response_done(Operation *op, SlapReply *rs) {
  BerElementBuffer berbuf;
  BerElement *ber = (BerElement *)&berbuf;
...
static int dupent_response_entry_1level(Operation *op, SlapReply *rs, Entry *e, valnum_t *valnum, int nattrs,
  ...
  for (i = 0; i < valnum[level].ap->a_numvals; i++) {
    ...
    if (level < nattrs - 1) {
      rc = dupent_response_entry_1level(op, rs, e, valnum, nattrs, level + 1);
      if (rc != LDAP_SUCCESS) {
        break;
      }
...
static int dupent_response_entry(Operation *op, SlapReply *rs) {
  ...
  for (i = 0; i < dc->dc_ds->ds_nattrs; i++) {
    ...
    ap = attr_find(rs->sr_entry->e_attrs, dc->dc_ds->ds_an[i].an_desc);
...
static int dupent_response(Operation *op, SlapReply *rs) {
  dupent_cb_t *dc = (dupent_cb_t *)op->o_callback->sc_private;
...
static int dupent_cleanup(Operation *op, SlapReply *rs) {
...
static int dupent_op_search(Operation *op, SlapReply *rs) {
  if (op->o_dupent != SLAP_CONTROL_NONE) {
    slap_callback *sc;
    dupent_cb_t *dc;
