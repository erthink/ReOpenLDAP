...
enum { ADREMAP_CASE = 1, ADREMAP_DNV };
...
static int adremap_cf_dnv(ConfigArgs *c) {
  slap_overinst *on = (slap_overinst *)c->bi;
  adremap_info *ai = on->on_bi.bi_private;
  adremap_dnv *ad, **a2;
  int rc = ARG_BAD_CONF;
...
static int adremap_search_resp(Operation *op, SlapReply *rs) {
...
static int adremap_refsearch(Operation *op, SlapReply *rs) {
  if (rs->sr_type == REP_SEARCH) {
...
static adremap_dnv *adremap_filter(Operation *op, adremap_info *ai) {
...
static int adremap_search(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
...
static int adremap_db_init(BackendDB *be, ConfigReply *cr) {
...
static int adremap_db_destroy(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  adremap_info *ai = (adremap_info *)on->on_bi.bi_private;
  adremap_case *ac;
  adremap_dnv *ad;
