 ...
 * Support for sambaPwdMustChange and sambaPwdCanChange added by Marco
 * D'Ettorre. Support for shadowLastChange added by SATOH Fumiyasu @ OSS
 * Technology, Inc.
 */
...
static const unsigned SMBK5PWD_F_ALL = 0
#ifdef DO_KRB5
                                       | SMBK5PWD_F_KRB5
#endif
#ifdef DO_SAMBA
...
static void lmPasswd_to_key(const char *lmPasswd, DES_cblock *key) {
...
static void hexify(const char in[HASHLEN], struct berval *out) {
  int i;
  char *a;
  unsigned char *b;
...
static void lmhash(struct berval *passwd, struct berval *hash) {
...
static void nthash(struct berval *passwd, struct berval *hash) {
...
static int smbk5pwd_op_cleanup(Operation *op, SlapReply *rs) {
  slap_callback *cb;
...
static int smbk5pwd_op_bind(Operation *op, SlapReply *rs) {
...
static int smbk5pwd_exop_passwd(Operation *op, SlapReply *rs) {
  ...
  do {
    ...
    if (!is_entry_objectclass(e, oc_krb5KDCEntry, 0))
      break;
...
static ConfigOCs smbk5pwd_cfocs[] = {{"( OLcfgCtOc:1.1 "
...
static int smbk5pwd_initialize(void) {
  int rc;
