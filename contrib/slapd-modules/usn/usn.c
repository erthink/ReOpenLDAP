...
static int usn_func(Operation *op, SlapReply *rs) {
...
static int usn_operational(Operation *op, SlapReply *rs) {
  slap_overinst *on = (slap_overinst *)op->o_bd->bd_info;
    ...
    if (SLAP_OPATTRS(rs->sr_attr_flags) || ad_inlist(ad_usnChanged, rs->sr_attrs)) {
      ...
      if (!ap) {
        if (rs_entry2modifiable(op, rs, on)) {
          a = attr_find(rs->sr_entry->e_attrs, ad_usnChanged);
        }
 ...
 */
static int usn_db_open(BackendDB *be, ConfigReply *cr) {
...
static int usn_db_init(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  usn_info_t *ui;

  if (SLAP_ISGLOBALOVERLAY(be)) {
    Debug(LDAP_DEBUG_ANY, "usn must be instantiated within a database.\n");
    return 1;
  }
...
static int usn_db_close(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  usn_info_t *ui = on->on_bi.bi_private;
  Connection conn = {0};
  OperationBuffer opbuf;
...
static int usn_db_destroy(BackendDB *be, ConfigReply *cr) {
  slap_overinst *on = (slap_overinst *)be->bd_info;
  usn_info_t *ui = on->on_bi.bi_private;
...
static int usn_init(void) {
  ...
  for (i = 0; as[i].desc; i++) {
    ...
    if (code) {
      Debug(LDAP_DEBUG_ANY, "usn_init: register_at #%d failed\n", i);
      return code;
    }
  }
...
SLAP_MODULE_ENTRY(usn, modinit)(int argc, char *argv[]) { return usn_init(); }
