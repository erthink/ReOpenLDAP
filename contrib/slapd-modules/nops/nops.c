...
static void nops_rm_mod(Modifications **mods, Modifications *mod) {
  Modifications *next;
...
static int nops_modify(Operation *op, SlapReply *rs) {
  ...
  if ((m = op->orm_modlist) == NULL) {
    ...
    send_ldap_error(op, rs, LDAP_INVALID_SYNTAX, "nops() got null orm_modlist");
    return (rs->sr_err);
  }
...
static int nops_initialize(void) {
