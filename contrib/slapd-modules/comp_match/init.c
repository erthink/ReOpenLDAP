...
int load_derived_matching_rule(char *cfg_path) {}

AttributeAliasing *comp_is_aliased_attribute(void *in) {
...
char *pre_processed_comp_filter[] = {
    /*1*/ "item:{ component \"toBeSigned.issuer.rdnSequence\", rule "
          "distinguishedNameMatch, value xxx }",
    /*2*/
    "item:{ component \"toBeSigned.serialNumber\", rule integerMatch, value "
    "xxx }",
    /*3*/
    "and:{ item:{ component \"toBeSigned.serialNumber\", rule integerMatch, "
    "value xxx }, item:{ component \"toBeSigned.issuer.rdnSequence\", rule "
    "distinguishedNameMatch, value xxx } }"};

static int init_attribute_aliasing_table() {
  int rc;
  int index = 0;
...
void init_component_description_table() {
  ...
  for (id = BASICTYPE_BOOLEAN; id != ASNTYPE_END; id++) {
    ...
    asntype_to_compdesc_mapping_tbl[id].atcd_cd.cd_comp_type =
        (AttributeType *)&asntype_to_compType_mapping_tbl[id].ac_comp_type;
  }
}
...
void *comp_convert_attr_to_comp(Attribute *a, Syntax *syn, struct berval *bv) {
  char *peek_head;
...
void comp_free_component(void *mem_op) {
...
void eat_bv_whsp(struct berval *in) {
...
int comp_test_one_component(void *attr_mem_op, void *assert_mem_op, ComponentSyntaxInfo *csi_attr,
  ...
  if (mr->smr_usage & SLAP_MR_COMPONENT) {
    ...
    return csi_attr->csi_comp_desc->cd_all_match(oid, csi_attr, csi_assert);
...
void comp_nibble_memory_free(void *nm) { ShutdownNibbleMemLocal(nm); }

void *comp_get_component_description(int id) {
