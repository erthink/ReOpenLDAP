...
if [ -d $HERE/.git ]; then
	...
	BUILD_ID=".${BUILD_NUMBER}-$(git describe --abbrev --always --long --tags)"
...
if [ -d $HERE/.git ]; then
	(git log --no-merges --dense --date=short --pretty=format:"%cd %s" "$PREV_RELEASE".. \
		...
		&& /bin/echo -e "\nPackage version: $PACKAGE\nSource code tag: $(git describe --abbrev=15 --long --always --tags)$(git show -s --abbrev=15 --format=-t%t | head -n 1)" ) > changelog.txt \
	|| failure "changelog"
else
	notice "No git repository, unable to provide changelog.txt" 2>&1 | tee changelog.txt >&2
fi
...
if [ -s Makefile ]; then
	...
	configure \
		...
		--enable-mdb=${MOD} --with-buildid=$BUILD_ID \
